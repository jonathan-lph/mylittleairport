// let arr = []; for (let i of document.getElementsByTagName('p')) { arr.push(i.innerText.split('\n'))}; console.log(arr);

const albumList = [[
  '在動物園散步才是正經事',
  'The OK Thing to Do on Sunday Afternoon Is to Toddle in the Zoo',
  2004, 10,
  "retail",
  "維港唱片", "Habour Records"
], [
  '只因當時太緊張',
  'Becoz I Was Too Nervous at That Time',
  2005, 10,
  "retail",
  "維港唱片", "Habour Records"
], [
  '我們在炎熱與抑鬱的夏天，無法停止抽煙',
  'We Can\'t Stop Smoking in the Vicious and Blue Summer',
  2007, 17,
  "retail",
  "維港唱片", "Habour Records"
], [
  '介乎法國與旺角的詩意',
  'Poetics - Something Between Montparnasse and Mongkok',
  2009, 16,
  "retail",
  "維港唱片", "Habour Records"
], [
  '香港是個大商場',
  'Hong Kong Is One Big Shopping Mall',
  2011, 15,
  "retail",
  "維港唱片", "Habour Records"
], [
  '寂寞的星期五',
  'Lonely Friday',
  2012, 17,
  "retail",
  "維港唱片", "Habour Records"
], [
  '適婚的年齡',
  'The Right Age for Marriage',
  2014, 15,
  "retail",
  "維港唱片", "Habour Records"
], [
  '火炭麗琪',
  'Fo Tan Lai Ki',
  2016, 17,
  "retail",
  "維港唱片", "Habour Records"
], [
  '你說之後會找我',
  'You Said We\'d Be Back',
  2018, 12,
  "retail",
  "維港唱片", "Habour Records"
], [
  'SABINA 之淚',
  "Sabina's Tears",
  2021, 16,
  "retail",
  "維港唱片", "Habour Records"
], [
  "催淚的滋味 (Live)",
  "TASTE OF TEARS (Live)",
  2020, 23,
  "online",
  "my little airport", "my little airport",
], [
  "我都唔知道 (Live)",
  "I ALSO DON'T KNOW (Live)",
  2022, 33,
  "online",
  "my little airport", "my little airport",
], [
  '跟你開玩笑',
  "Just Kidding",
  2022, 10,
  "retail",
  "維港唱片", "Habour Records"
], [
  'Singing in the Telephone',
  'Singing in the Telephone',
  2003, 6,
  "limited",
  "my little airport", "my little airport", 
], [
  "When I Listen to the Field Mice",
  "When I Listen to the Field Mice",
  2005, 2,
  "limited",
  "維港唱片", "Habour Records"
], [
  "Gi Gi Leung Is Dead",
  "Gi Gi Leung Is Dead",
  2005, 4,
  "limited",
  "默契音樂", "Silent Agreement"
], [
  "Zoo Is Sad, People Are Cruel",
  "Zoo Is Sad, People Are Cruel",
  2007, 14,
  "limited",
  "Elefant Records", "Elefant Records", 
], [
  "Stay As Sweet As You Are",
  "Stay As Sweet As You Are",
  2008, 17,
  "limited",
  "트라이앵글 뮤직은", "Triangle Music"
], [
  "為你含情",
  "Hum Ching for You",
  2008, 10,
  "collaboration",
  "LYFE Music", "LYFE Music"
]]

const a1list = {
  og: [
    "coka, i'm fine",
    "在動物園散步才是正經事",
    "victor, fly me to stafford",
    "edward, had you ever thought that the end of the world would come on 20.9.01?",
    "柯德莉，關於風景",
    "my little banana",
    "josephine's shop",
    "王菲，關於你的眉",
    "you don't wanna be my girlfriend, phoebe",
    "dee, it may all end tomorrow"
  ],
  en: [
    "Coka, I'm Fine",
    "The OK Thing to Do on Sunday Afternoon Is to Toddle in the Zoo",
    "Victor, Fly Me To Stafford",
    "Edward, Had You Ever Thought That the End of the World Would Come On 20.9.01?",
    "Audrey, About the Landscape",
    "My Little Banana",
    "Josephine's Shop",
    "Faye Wong, About Your Eyebrows",
    "You Don't Wanna Be My Girlfriend, Phoebe",
    "Dee, It May All End Tomorrow"
  ]
}

const a1meta = [
  [
    1, 50,
    "P", "P", null, "N", null
  ],
  [
    2, 15,
    "P", "P", null, "N", null
  ],
  [
    2, 14,
    "P", "P", null, "N", null
  ],
  [
    2, 21,
    "P", "P", null, "N", null
  ],
  [
    1, 38,
    "P", "P,N", null, "N", null
  ],
  [
    1, 59,
    "N", "馬克斯（Marcus）", null, "N", null
  ],
  [
    2, 46,
    "P", "P", null, "N", null
  ],
  [
    2, 25,
    "P", "P", null, "N", null
  ],
  [
    2, 18,
    "P", "P", null, "N", null
  ],
  [
    1, 46,
    "P", "P,N", null, "N", null
  ]
]

const a1lyrics = [
  [
    "I am so sorry I have no time to call you",
    "I've been so busy recently and I don't want",
    "Do you recall the park where we spent for whole night?",
    "We walked and talked till the sunrise",
    "",
    "Public exam's coming, you've got to be hard working",
    "You should not be as lazy as I'm being",
    "I will always stand by you, so don't be afraid",
    "Though I can't give you any note",
    "",
    "Coka, Coka, Coka, Coka",
    "If you still remember me",
    "Just call me when you are free"
  ],
  [
    "我間中仍會想我們會見面",
    "在那間紅磡近黃埔的商店",
    "你若然還記得那諾言",
    "曾說今天我們流浪到海邊",
    "",
    "你照片留在一封情信裡面",
    "每一天仍是照舊看它一遍",
    "我仍然還記得那笑臉",
    "曾在當天我們流浪到夏天",
    "",
    "我都曾寫你在每天的日記",
    "但放棄你卻像再沒可能夢想",
    "像一個沉悶的獨唱",
    "",
    "你照片留在一封情信裡面",
    "每一天仍是照舊看它一遍",
    "兩年前遊過的動物園",
    "來到今天我們流浪到哪邊？",
    "",
    "我都曾寫你在每天的日記",
    "但放棄你卻像再沒可能夢想",
    "像一個沉悶的獨唱",
    "",
    "但放棄你卻像再沒可能夢想",
    "像失去甜蜜的合唱"
  ],
  [
    "This is the last song I write for you",
    "This is the last night I think of you",
    "Your name is Victor Ching, how do you do?",
    "",
    "I phoned you this morning you couldn't hear me",
    "I sent you letter you couldn't receive",
    "It's my favorite game, but I have changed",
    "",
    "It's too late to say I miss you Victor Ching",
    "And you are on your way to Stafford in UK",
    "I know it's too late to say I'm fond of you",
    "You've got a girl from Singapore who you didn't like before",
    "",
    "I love you when you no longer love me",
    "I need you when you no longer need me",
    "You wanted me, but you have changed already",
    "",
    "It's too late to say I miss you Victor Ching",
    "And you are on your way to Stafford in UK",
    "I know it's too late to say I'm fond of you",
    "You've got a girl from Singapore who you didn't like before",
  ],
  [
    "Well it happened a few years ago",
    "We're still pen friends at 17 years old",
    "I waited for you everyday, did you know?",
    "I scared to call you anyway, you didn't know",
    "",
    "Then we met in a rainy day",
    "I remember how you dressed that day",
    "I forgot what I wanted to say when I saw your face",
    "It's just like happened yesterday",
    "But now you fall away",
    "Oh now you fall away",
    "",
    "Where are you now?",
    "Why can't we be ourselves just like yesterday?",
    "Edward, we were good pen friends",
    "And did you ever believe me?",
    "When I say \"I don't need to be free",
    "If you could only",
    "Could only stay with me\"",
    "I don't need to be free",
    "If you could stay with me",
    "If you could stay with me"
  ],
  [
    "沒什麼秘密",
    "可以及現在更秘密",
    "月亮將下沉",
    "可是末日未會如常降臨",
    "",
    "看著天和海",
    "仿似連成一物",
    "但是不曾接近",
    "你像片大海",
    "柯德莉藍的海",
    "願望看著天",
    "包括樹木　唱片",
    "皮鞋　情侶　公路",
    "石頭　河流　商舖"
  ],
  [
    "My little beautiful pretty pretty banana",
    "You are all that I'd like and I want to share you",
    "With all my friends, family and loved one",
    "I just open all your skin then I take a small bite",
    "Once I have a wonderful dream about you",
    "We were dancing in the sea",
    "And the fishes all swimming around us",
    "Come with me so I can show you big boat that I built",
    "Let's go for a ride",
    "I promise it will fly to the sky where no one cries",
    "Banana you will see the clouds beneath our feet",
    "To the space where twinkle stars",
    "Saying hello and smile",
    "They're shinning bright happily",
    "",
    "Will we see each other any more after ten years?",
    "Why don't you tell me you're falling ill and not happy?",
    "Please stay well and be tasty",
    "I don't want to see you be grumpy",
    "Please stay strong and be healthy",
    "We still have a lot to do like exploring the whole world",
    "You're the only one can't be replaced",
    "Hey you, listen",
    "Banana you're the best, man!"
  ],
  [
    "'Cause running business is not your gift",
    "Why don't you close it",
    "Why don't say \"Damn it\"?",
    "Depression in your shop of Mong Kok",
    "Silence on Monday, no change on Thursday",
    "",
    "But Josephine",
    "You may think your pals cannot lose you",
    "Probably they need you so much more than they love you",
    "",
    "So why don't you abandon them?",
    "Let yourself be free",
    "This world is bigger than",
    "What we always believe in",
    "So why don't you try to fade out",
    "From your dream of dreams?",
    "Try to take the other risks",
    "Your life should not be like this",
    "Every color we can be",
    "",
    "The only thing I've bought in your shop",
    "Was a $10 candle with some nice smelling",
    "It couldn't help you to pay the rent",
    "Couldn't resist hazards nor prevent disease",
    "",
    "But Josephine",
    "You know your shop is cute and beautiful",
    "The truth is",
    "You know this beauty can't make you happy",
    "",
    "So why don't you abandon them?",
    "Let yourself be free",
    "This world is bigger than",
    "What we always believe in",
    "So why don't you try to fade out",
    "From your dream of dreams?",
    "Try to take the other risks",
    "Your life should not be like this",
    "Every color we can be",
  ],
  [
    "若要　將這音樂配上字句",
    "描述你的臉",
    "關於你的眼　耳　頭髮",
    "能用些什麼字與什麼詞",
    "描述你在夢中那樣美艷？",
    "",
    "夢中　夢中你跟我是情侶",
    "一起執迷",
    "一起呼叫在太陽裡",
    "然後我在七時五十六分醒時",
    "無法再睡著覺　我便要上學",
    "",
    "可不可以話你像個大南瓜和紅豆沙般美？",
    "沒有玉蝴蝶形容　王菲都很美",
    "沒有漂亮的詞仍認得出你",
    "la la la..."
  ],
  [
    "You don't wanna be my girlfriend, Phoebe",
    "You just wanna show your beauty",
    "Oh my Phoebe",
    "Can you see that I'm crying?",
    "",
    "All the boys think that you are the best",
    "All the eyes are just looking at your breast",
    "Oh my Phoebe",
    "You just wanna show your beauty",
    "",
    "You don't wanna kiss me",
    "You just wanna show your beauty",
    "You don't wanna know me",
    "You know you're not just a love song to me.",
    "",
    "You've ever sent your photos to me",
    "Watching your face is an interesting journey",
    "Oh my Phoebe",
    "I wanna tell you my little story",
    "",
    "Can you talk to me when I'm lonely?",
    "Can we talk not only when you're lonely?",
    "I know you Phoebe",
    "I think I trend to know you completely.",
    "",
    "You don't wanna kiss me",
    "You just wanna show your beauty",
    "You don't wanna know me",
    "You will never send all your love to me",
    "",
    "So please don't say \"Maybe we may meet one day\"",
    "This sentence could not last more than one day",
    "'Cause you know I'm not the only one be missed by you",
    "As same as I don't only miss you"
  ],
  [
    "But how can I forget?",
    "But how can I forget your tender smile?",
    "But how can you forgive?",
    "But how can you forgive my unstable mind?",
    "One day we may meet again",
    "One day we may kiss again very splendid",
    "One day you may get married",
    "One day you may have baby, oh my dear Dee"
  ]
]

const a2list = {
  og: [
    "gi gi leung is dead",
    "i don't know how to download good av like iris does",
    "只因當時太緊張",
    "失落沮喪歌",
    "就當我是張如城",
    "白田購物中心",
    "leo, are you still jumping out of windows in expensive clothes?",
    "你的微笑像朵花",
    "春天在車廂裡",
    "my little fish"
  ],
  en: [
    "Gi Gi Leung Is Dead",
    "I Don't Know How to Download Good AV Like Iris Does",
    "Because I Was Too Nervous at That Time",
    "The Lost and Frustration Song",
    "Just Pretend I Am Jacky Cheung",
    "Park Tin Commercial Centre",
    "Leo, Are You Still Jumping Out of Windows in Expensive Clothes?",
    "You Smile Like A Blossom",
    "Spring in the Bus",
    "My Little Fish"
  ]
}

const a2meta = [
  [
    1,16,
    "P", "P", null, "N", null
  ],
  [
    2,26,
    "P", "P", null, "N", null
  ],
  [
    2,35,
    "P", "P", null, "N", null
  ],
  [
    2,27,
    "P", "P", null, "N", null
  ],
  [
    1,49,
    "P", "P", null, "P", null
  ],
  [
    1,10,
    "P", "阿雪", null, "N", null
  ],
  [
    2,9,
    "P", "P", null, "N", null
  ],
  [
    2,38,
    "P", "P", null, "N", null
  ],
  [
    2,40,
    "P", "P", null, "N", null
  ],
  [
    2,17,
    "N", "N", null, "N", null
  ]
]

const a2lyrics = [
  [
    "On Monday",
    "I don't wanna go to school",
    "I just wanna stay with you",
    "All I want is you",
    "",
    "On Tuesday",
    "I don't wanna go to school",
    "I just wanna sing with you",
    "Sing your songs with you",
    "",
    "When I listen to your songs",
    "I feel extremely happy",
    "You will be loving me",
    "I know you just can't leave me",
    "You just can't leave me",
    "",
    "On Sunday",
    "You're no longer in my heart",
    "Maybe I got old enough",
    "Yeah yeah yeah yeah yeah",
    "",
    "Gi Gi Leung is dead",
    "Gi Gi Leung is dead",
    "Gi Gi Leung is dead",
    "Gi Gi Leung is dead"
  ],
  [
    "I wanna watch some AV and you got plenty",
    "I'm so dry, I'm so dry, I'm so dry",
    "I enjoy watching girls and boys in uniforms",
    "Send them to me, oh Iris, send them to me",
    "",
    "I don't know any others who has plenty of AV",
    "Please send those good AV to me",
    "",
    "I don't know how to download good AV like yours",
    "I don't know, I don't know, I don't know",
    "You know I like those AV telling no story",
    "Only you, only you, you know me",
    "",
    "I don't know any others who has plenty of AV",
    "please send those good AV to me",
    "",
    "(Send me good AV)",
    "(Please send me good AV)",
    "(Saw some AV)",
    "(AV, got plenty)",
    "(I want some good AV)",
    "(AV telling no story)",
    "(Send me, send me)",
    "(I wanna watch some AV)",
    "(Yeah I watch AV)",
    "(I know you got plenty)",
    "(AV)",
    "(AV)"
  ],
  [
    "只因當時我太緊張",
    "令你覺得我很異常",
    "看到你令我有很多遐想",
    "最好以後都這樣",
    "",
    "那次不小心掂了你手掌",
    "令我覺得很有營養",
    "與你出街是我的夢想",
    "人生充滿新方向",
    "",
    "別人說你很囂張",
    "但是我覺得你很善良",
    "我已沒有心機再細想",
    "怎麼告訴你真相？",
    "",
    "別人說你太囂張",
    "但是我覺得你很正常",
    "我已沒有心情再去聯想",
    "怎麼能令你心癢？",
    "",
    "只因當時我太慌張",
    "沒發揮到我的擅長",
    "我最想與你去數綿羊",
    "每夜美夢很歡暢",
    "",
    "結果我與你去了足球埸",
    "然後再逛白購商場",
    "順便看了一些明星相",
    "但願你沒高修養",
    "",
    "別人說你很囂張",
    "但是我覺得你很善良",
    "我已沒有心機再細想",
    "怎麼告訴你真相？",
    "",
    "別人說你太囂張",
    "但是我覺得你很漂亮",
    "我已沒有心情再去聯想",
    "怎麼能令你心癢？",
    "",
    "我似漸變抑鬱與感傷",
    "都不能令你歡暢"
  ],
  [
    "我又有心事",
    "自從看了太宰治",
    "我問誰",
    "誰都不會在意死亡的意義",
    "",
    "世界像似崩裂",
    "我倆都無可救治",
    "我但求",
    "乘風飛到別處再重新開始",
    "",
    "願你可以給我寫首詩",
    "紀念我們約會過六次",
    "",
    "我沒法想像",
    "看過的童話故事",
    "有陣時",
    "成長不過是個悲劇的開始",
    "",
    "我做過的事",
    "沒太多是有意義",
    "接近誰",
    "誰都不會快樂這些我都知",
    "",
    "願我可變歡笑的天使",
    "我們便會每日見十次",
    "",
    "願我可變優雅的詩篇",
    "這樣你便願看數十遍"
  ],
  [
    "說我是張如城吧",
    "說我是張如城吧",
    "已經豁了出來",
    "都反正已經豁了出來",
    "年輕的你不要怕",
    "",
    "說我是夏金城吧",
    "說我是夏金城吧",
    "怎麼要批評他的差",
    "怎麼要批評",
    "夢想你都有過嗎？",
    "",
    "說我是 la la la la la",
    "說我是 la la la la la",
    "la la la la la la",
    "la la la",
    "la la la la la la",
    "什麼使你會害怕？"
  ],
  [
    "En fin, il est arrivé",
    "Après les trois questions",
    "Le premier fois",
    "La première personne",
    "Tu es si peur comme un petit lapin",
    "Tu es si timide comme une petite fille",
    "Est-il trop vite?",
    "Y-a-t-il trop d'amour?",
    "Tu ne sais plus",
    "Tu ne sais rien",
    "Jusqu'à quelqu'un t'a dit",
    "« Il y a toujours le premier fois. »",
    "« Ne triste pas »",
    "(Jusqu'à quelqu'un t'a dit)",
    "(« Il y a toujours le premier fois. »)",
    "(« Ne triste pas. »)"
  ],
  [
    "I hope you can see what I can see",
    "I hope you can hear what I can hear",
    "Because it's not easy",
    "Because it's not easy",
    "",
    "I hope you can feel what I can feel",
    "I hope you believe what I believe",
    "Because I feel lonely",
    "Because I'm so lonely",
    "",
    "I don't wanna go home",
    "I still can recall",
    "All the days in F.4 we played in our school",
    "Now I stay in Mei Foo",
    "But where are you?",
    "Leo, where are you?",
    "",
    "I hope you can taste what I can taste",
    "I hope you can think what I can think",
    "Because I'm so crazy",
    "Because I'm so crazy",
    "",
    "'Cause I know you know what I don't know",
    "I know you can think what I can't think",
    "Because you are special",
    "Because you're so special",
    "",
    "How I wish you were here",
    "So I called your home",
    "But your mom said that you've gone somewhere",
    "I have never heard before",
    "Why didn't you call me",
    "When you leave here?",
    "",
    "And all the things changed",
    "After we repeated a year in school"
  ],
  [
    "已經過了兩星期",
    "沒有與你食雪梨",
    "我與你確係離奇",
    "年紀相差五十里",
    "",
    "那天你要去外地",
    "剩下我獨自皺眉",
    "你的個性最迷離",
    "讓我不可以忘記",
    "",
    "願明日能再見",
    "你迷人的嘴臉",
    "我現時就快要",
    "找你對住我笑",
    "",
    "你知嗎　你最迷人",
    "是你回望那帶笑眼神",
    "那刻已被你迷魂",
    "為了你甘願受困",
    "",
    "可惜我另有愛人",
    "實在讓我最頭暈",
    "怎可以共你蜜運",
    "望你可給我熱吻",
    "",
    "願明日能再見",
    "你迷人的嘴臉",
    "我現時就要叫",
    "想起你就要跳"
  ],
  [
    "可否試一次",
    "與我閒時亂搭巴士",
    "由上環到天后",
    "拖著我手到我話夠",
    "",
    "請跟我一次",
    "與我二時搭到十時",
    "什麼都也可以",
    "搭到累時吃百力滋",
    "",
    "我們這件事",
    "沒有人知",
    "流浪到商店",
    "在這日子",
    "離別時別掛念",
    "明年今天再繼續試",
    "",
    "我們這件事",
    "沒有人知",
    "流浪到商店",
    "在這日子",
    "離別時侯約定",
    "明年今天再繼續試"
  ],
  [
    "Dear skinny fish",
    "Listen to me for a while",
    "We have no more tears",
    "No more struggle, no more wine",
    "",
    "You're my little fish",
    "Swim to me and take some cheese",
    "Do you still believe",
    "He would come back? He's a freak",
    "",
    "And then my fish she says",
    "\"I thought he belongs to me",
    "I know I should not mind",
    "I cannot tell lies",
    "He puts me aside\"",
    "",
    "Fish starts to cry",
    "Other fishes cry and cry.",
    "Fish keeps on crying",
    "\"wu wu wu…\"",
    "",
    "Little baby fish",
    "Listen to me for a while",
    "We have no more tears",
    "No more struggle, no more wine",
    "",
    "From now on",
    "From now on",
    "From now on"
  ]
]

const a3list = {
  og: [
    "j'ai peur",
    "japan 實瓜",
    "我愛官恩娜，都不及愛你的哨牙",
    "馬田的心事",
    "讓我搭一班會爆炸的飛機",
    "indie 悲歌",
    "奇人的離職",
    "frankie wants to be alone",
    "悲傷的採購",
    "荔枝角公園",
    "je pense à toi",
    "畢業變成失業",
    "美孚根斯堡與白田珍寶金",
    "和陳五 msn",
    "debbie, don't send this song to others",
    "dolly and bb will not wake up anymore",
    "my little k and j",
  ],
  en: [
    "J'ai Peur",
    "Japan Sat-Gwaa",
    "I Love Your Whistling Teeth More Than Ella Koon",
    "Martin's Worries",
    "Let Me Get On an Exploding Plane",
    "Indie Elegy",
    "Weirdo's Resignation",
    "Frankie Wants to Be Alone",
    "Sad Purchase",
    "Lai Chi Kok Park",
    "Je Pense à Toi",
    "Graduation Equals Jobless",
    "Mei Foo Gainsbourg and Pak Tin Pirkin",
    "MSN with Ng Chan",
    "Debbie, Don't Send This Song to Others",
    "Dolly and BB Will Not Wake Up Anymore",
    "My Little K and J",
  ]
}

const a3meta = [
  [
    0,21,
    null, null, null, null, {
      poem: '阿雪',
      recite: '阿雪'
    }
  ],
  [
    1,31,
    'P', 'P', 'P', 'P,N', null 
    // { guitar: 'P' }
  ],
  [
    2,6,
    'P', 'P', 'P', 'N', {
      // casio: 'P',
      // programming: 'P',
      poem: '阿雪',
      recite: '阿雪'
    }
  ],
  [
    3,1,
    'P', 'P', 'P', 'P,N', {
      // programming: 'P',
      recite: 'N'
    }
  ],
  [
    2,21,
    'P', 'P', 'P,N,阿賢,阿科', 'N', null
    // {
    //   casio: 'P',
    //   chorus: 'P',
    //   guitar: 'P',
    //   bass: '阿賢',
    //   drum: '阿科'
    // }
  ],
  [
    1,57,
    'P', 'P', 'P,N,阿賢,阿科', 'N', null 
    // {
    //   casio: 'P',
    //   guitar: 'P',
    //   bass: '阿賢',
    //   drum: '阿科'
    // }
  ],
  [
    2,14,
    'P', 'P', 'P', 'N', {
      // casio: 'P',
      // programming: 'P',
      // flute: 'P',
      poem: '阿雪',
      recite: '阿雪'
    }
  ],
  [
    2,16,
    'P', 'P', 'P', 'N', null,
    // {
    //   casio: 'P',
    //   programming: 'P',
    //   guitar: 'P',
    // }
  ],
  [
    3,3,
    'P', 'P', 'P', 'N', null,
    // {
    //   casio: 'P',
    //   programming: 'P',
    // }
  ],
  [
    1,9,
    null, 'P', 'P', null, {
      // programming: 'P',
      poem: '阿雪',
      recite: 'Vincent Tigreat'
    }
  ],
  [
    0,55,
    'P', 'P', 'P,N,阿賢,阿科', 'P', null,
    // {
    //   guitar: 'P',
    //   bass: '阿賢',
    //   drum: '阿科'
    // }
  ],
  [
    2,16,
    'P', 'P', 'P,N,阿賢,阿科', 'N', null,
    // {
    //   chorus: 'P',
    //   guitar: 'P',
    //   bass: '阿賢',
    //   drum: '阿科'
    // }
  ],
  [
    2,13,
    'The Lee\'s', 'P', 'P,N,The Lee\'s', 'The Lee\'s', {
      // casio: 'P',
      // melodica: 'P',
      // guitar: 'P,The Lee\'s',
      // celesta: 'N',
      recite: 'P,N'
    }
  ],
  [
    2,35,
    'P', 'P', 'P', 'P,N', null,
    // {
    //   casio: 'P',
    //   programming: 'P',
    // }
  ],
  [
    3,14,
    'P,N,Jerry,Josephine,Tim', 'P', 'P', 'P', null,
    // {
    //   programming: 'P',
    // }
  ],
  [
    2,14,
    'N', 'N', 'N', 'N', null,
    // {
    //   guitar: 'N'
    // }
  ],
  [
    4,14,
    'N', 'N', 'P,N,Reonda C.,吳卓基,LV,Sing', 'N', 
    // {
    //   casio: 'P',
    //   organ: 'N',
    //   banjo: 'Reonda C.',
    //   guitar: '吳卓基',
    //   viola: 'LV',
    //   celesta: 'LV',
    //   percussion: 'Sing',
    // }
  ],
]

const a3lyrics = [
  [
    "J'ai peur que tu ne m'aime plus",
    "J'ai peur que tu ne me parle plus",
    "J'ai peur que tu as une autre belle fille",
    "J'ai peur que tu veux me quitter",
    "J'ai si peur que je ne peux pas dormir",
    "Ce sera finir bientôt",
    "Je sais, oui je sais"
  ],
  [
    "You teach me French",
    "But I cannot speak well",
    "You teach me French",
    "But I cannot speak well",
    "You said \"Je pense à toi\"",
    "But I said \"Japan 實瓜\"",
    "You said \"Je pense à toi\"",
    "But I said \"Japan 實瓜\"",
    "",
    "I teach you French",
    "But you cannot speak well",
    "I teach you French",
    "But you cannot speak well",
    "I said \"Je pense à toi\"",
    "But you said \"Japan 實瓜\"",
    "I said \"Je pense à toi\"",
    "But you said \"Japan 實瓜\""
  ],
  [
    "（咦，原來你鍾意官恩娜㗎）",
    "",
    "難道你已經不再喜歡我",
    "踫見面也都不嗌吓我",
    "難道你已經不再關心我",
    "看見亦要直行直過",
    "",
    "其實我已經算很喜歡你",
    "對住第二個都無咁好氣",
    "其實我已經算很欣賞你",
    "看住電視亦會想起你",
    "",
    "我就算喜愛官恩娜",
    "都不及愛你的哨牙",
    "我就算喜愛何韻詩",
    "她都不及你的法文詩",
    "",
    "如若你真的這麼喜歡我",
    "到最後你不應該放棄我",
    "難道你也見一個鍾意一個",
    "每個月看中數十個",
    "",
    "On partage la musique",
    "Dans la rue, sur le lit",
    "On n'a pas besoin de parler",
    "Et la main dans la main",
    "Comme ça, juste toi et moi",
    "Ensemble, toujours",
    "",
    "我就算喜愛薛凱琪",
    "都不及愛你的皺眉",
    "我就算喜愛張柏芝",
    "她都不及你咁得意",
    "",
    "我就算喜愛官恩娜",
    "都不及愛你的哨牙",
    "我就算喜愛何韻詩",
    "她都不及你的法文詩",
    "她都不及你咁得意",
    "她都不及你咁真摰"
  ],
  [
    "我　很想　很想打俾你",
    "我　很想　很想打俾你",
    "但我知道　如果我　真的打俾你。",
    "你就會　驚我想約你",
    "",
    "我不想　你知道　我想你",
    "我不想　你知道　我想你",
    "若你知道　我真的　很想　很想你",
    "你就會　不再跟我睇戲",
    "",
    "我愈想你　我就愈想你",
    "做什麼都沒有心機",
    "我愈想你　我就愈憎你",
    "為什麼你要將我捨棄？",
    "",
    "在樓梯的轉角　我只能偷看你",
    "在茶水間的雪櫃旁　我們擦身而過",
    "你的眼睛　你的頭髮",
    "你的身高　我都不敢細看",
    "",
    "我愈想你　我就愈想你",
    "做什麼都沒有心機",
    "我愈想你　我就愈憎你",
    "為什麼你要將我捨棄？",
    "我愈憎你　我就愈生氣",
    "為什麼你不給我驚喜？",
    "我愈生氣　我就愈小器",
    "為什麼我要將你念記？"
  ],
  [
    "讓我搭一班會爆炸的飛機",
    "去到台灣之前被炸死",
    "讓我喝一杯會吐血的香檳",
    "在喝醉之前可斷氣",
    "",
    "我以後也不會再打電話給你",
    "再沒法對電話唱Ｋ",
    "你要是孤單便去找好友睇戲",
    "他們一定會氹返你",
    "",
    "讓我搭一班會爆炸的飛機",
    "回到香港之前被炸死",
    "讓我喝一杯會吐血的香檳",
    "在喝醉之前擁抱你",
    "",
    "我以後也不會再打電話給你",
    "再沒法對電話唱Ｋ",
    "你要是孤單便去找好友睇戲",
    "他們一定會氹返你",
    "",
    "讓我搭一班會爆炸的飛機",
    "回到香港之前被炸死",
    "讓我喝一杯會吐血的香檳",
    "在喝醉之前可斷氣",
  ],
  [
    "專業音樂人",
    "不想音樂有 indie 之分",
    "彷似些有錢人",
    "從不理會貧窮的心",
    "但我沒有怨恨",
    "即使滿心傷痕",
    "不管主流傳媒多黑暗",
    "",
    "討厭你的人",
    "只會繼續火滾",
    "喜愛你的人",
    "不會個個現身",
    "但你要繼續前行",
    "多遠請不要問",
    "總有些人陪在你左近",
    "",
    "我要這首歌",
    "在大氣電波飛行",
    "讓 DJ 可播另一種聲音",
    "但佢地沒有責任",
    "肯播是我的運",
    "只怪我的聲音太啃",
    "只怪我的公司無銀",
    "我與大眾永不會拉近",
    "",
    "就算沒法流行",
    "總想繼續前行",
    "出碟你記得幫襯"
  ],
  [
    "到最後你都係要走",
    "我沒有挽留",
    "到最後你都係要分手",
    "你沒有回頭",
    "我沒有強求",
    "你在我眼前",
    "我在你背後",
    "看你離開帶走你的小狗",
    "",
    "J'ai peur que tu ne m'aime plus",
    "J'ai peur que tu ne me parle plus",
    "J'ai peur que tu as une autre belle fille",
    "J'ai peur que tu veux me quitter",
    "J'ai si peur que je ne peux pas dormir",
    "Ce sera finir bientôt",
    "",
    "到最後我都沒有開口",
    "我只有垂頭",
    "到最後我都沒有跳樓",
    "我只有淚流",
    "對於愛情",
    "對於友情",
    "我都會任由",
    "任由它自己飄走",
    "",
    "J'ai peur que tu ne m'aime plus",
    "J'ai peur que tu ne me parle plus",
    "J'ai peur que tu as une autre belle fille",
    "J'ai peur que tu veux me quitter",
    "J'ai si peur que je ne peux pas dormir",
    "Ce sera finir bientôt",
  ],
  [
    "His name is Frankie",
    "likes to drink green tea",
    "And I noticed him, when I was 16",
    "We discussed about Christy",
    "She was the goddess to me",
    "",
    "He knows who is Tim",
    "but not closed to him",
    "Though they both like film",
    "they have different dreams",
    "Frankie wants to be lonely",
    "he needs no one to agree",
    "",
    "He walks alone in school",
    "no one think he is cool",
    "He never studies hard",
    "but he always gets high mark",
    "People think he's so strange",
    "but he doesn't wanna change",
    "All he need is daydreaming",
    "No one really knows his thinking",
    "",
    "He walks alone in school",
    "no one think he is cool",
    "He never studies hard",
    "but he always gets high mark",
    "People think he's so strange",
    "but he doesn't wanna change",
    "All he need is daydreaming",
    "No one really knows his thinking"
  ],
  [
    "訂了一季　又到下季",
    "為何人大了就要成為工作的奴隸",
    "最愛作的不可發揮",
    "我的感覺　逐漸流逝",
    "從前曾話過要如何欣賞世界的美麗",
    "現在只懂得放假去消費",
    "",
    "如果繼續這樣",
    "我還是我嗎？",
    "我還喜歡我嗎？",
    "或者我應該死去吧",
    "總好過變得更差",
    "如果繼續這樣",
    "你還願意嗎？",
    "和我奏著結他？",
    "日子不應該這樣吧",
    "而現實就是這樣可怕",
    "",
    "如果繼續這樣",
    "我還是我嗎？",
    "我還喜歡我嗎？",
    "或者我應該死去吧",
    "總好過變得更差",
    "如果繼續這樣",
    "你還願意嗎？",
    "和我奏著結他？",
    "日子不應該這樣吧",
    "而現實就是這樣可怕",
  ],
  [
    "Chaque fois, quand je te vois, je pleure",
    "Toutes les petites choses me font pleurer",
    "Et après toutes les larmes",
    "J'ai déjà oubile pourquoi je t'aime",
    "Le temp est arrive",
    "Il faut que je te dise: « Je m'en vais. »"
  ],
  [
    "You teach me French",
    "But I cannot speak well",
    "You teach me French",
    "But I cannot speak well",
    "You said \"Je pense à toi\"",
    "But I said \"Japan 實瓜\"",
    "You said \"Je pense à toi\"",
    "But I said \"Japan 實瓜\"",
    "",
    "You teach me French",
    "But I cannot speak well",
    "You teach me French",
    "But I cannot speak well",
    "You said \"Je pense à toi\"",
    "But I said \"Japan 實瓜\"",
    "You said \"Je pense à toi\"",
    "But I said \"Japan 實瓜\"",
  ],
  [
    "一畢業就等於失業",
    "一畢業就等於失業",
    "",
    "Shoo do do do do do",
    "shoo do do do do do do",
    "shoo do do do do do",
    "shoo do do do do do",
    "", 
    "七個月見左三十九份工",
    "搵份工仲難過搵老公",
    "我今日只想留在家中",
    "",
    "Shoo do do do do do",
    "shoo do do do do do do",
    "shoo do do do do do",
    "shoo do do do do do",
    "", 
    "見工見到入左去東涌",
    "點知份工要去掘山窿",
    "我今日只想留在家中",
    "",
    "聽見同學逐個搵到工",
    "但我仍在尋尋覓覓中",
    "沒有人比我更加燶",
    "不幸正與我結伴中",
    "",
    "Shoo do do do do do",
    "shoo do do do do do do",
    "shoo do do do do do",
    "shoo do do do do do",
    "",
    "之後見o岩份工以為好輕鬆",
    "因為份工其實基本上係無人工",
    "我今日只想留在家中",
    "",
    "Shoo do do do do do",
    "shoo do do do do do do",
    "shoo do do do do do",
    "shoo do do do do do do",
    "",
    "Shoo do do do do do",
    "shoo do do do do do do",
    "shoo do do do do do",
    "shoo do do do do do",
    "",
    "Shoo do do do do do",
    "shoo do do do do do do",
    "shoo do do do do do",
    "shoo do do do do do do do",
    "shoo do do do do do do do",
    "do do do do do",
    "",
    "一畢業就等於失業",
    "一畢業就等於失業",
  ],
  [
    "(Look at me and love again)",
    "我們在炎熱與抑鬱的夏天　無法停止抽煙",
    "(Look at me and love again)",
    "我們在炎熱與抑鬱的辦公室　無法停止寫詩",
    "(Fiction said that let me down)",
    "我們是美孚根斯堡與白田珍寶金",
    "(What you said that what's in my hand)",
    "金錢對於我們來說　輕如鴻毛",
    "",
    "(da da da, da da da)",
    "",
    "(I can see that love again)",
    "我們是香港最後一群缺乏社交技巧的詩人",
    "(You can see the love in my hand)",
    "我們是演奏家　思想家",
    "(I can say that love is blind)",
    "我們是迷失在森林裏的旅人",
    "(I don't care if love is blind)",
    "在同樣不仁慈的善良與邪惡之間　與潮人抗衡",
    "",
    "(da da da, da da da)",
    "",
    "(I can't be that lonely again)",
    "我們在嶺南之風　美孚之巔",
    "(I can't change the world)",
    "在公園　電影院　商場",
    "送別所有成長的記憶",
    "(I don't care about --)",
    "我們喝著涼茶聽著音樂",
    "(Love me is right)",
    "大口大口地把煙噴到天上",
    "(I don't see a thing)",
    "日出日落　我們如在巴黎法國",
    "",
    "(I can see the love is gone)",
    "我們在炎熱與抑鬱的夏天　無法停止抽煙",
    "(I don't care the love you are)",
    "我們在炎熱與抑鬱的辦公室　無法停止寫詩",
    "(You can say that love is true)",
    "(I think she let it down)",
    "",
    "(da da da, da da da)",
    "",
    "(I can't be that lonely again)",
    "(We can't change the world)",
    "(I can't see that love in my hand)",
    "(I can't change the world)"
  ],
  [
    "我愛你　我要帶你返屋企",
    "我不要等你",
    "我不要在 MSN 等你",
    "我要你　我要撟住你的手臂",
    "像那晚撟住你的手臂",
    "閉著眼行了一條街",
    "你說這是測驗對對方信心的遊戲",
    "",
    "這個世界已沒有將來",
    "我在這裏被悶到炸開",
    "我要與你流亡到海外",
    "我說我們不如一同去到越南食魚蛋",
    "我說我們不如一同去到月球釣泥猛",
    "我說我們不如一同去到去到去到去到……",
    "",
    "其實我知道你不開心",
    "其實我知道你不想一個人",
    "其實我都不想一個人",
    "我說我們不如我們不如結婚",
    "但結婚之後要分開住分開訓",
    "然後你問「咁點解要結婚？」",
    "我說和不太熟的人結婚應該會很開心",
    "",
    "這個世界已沒有將來",
    "這裏永遠也沒法變改",
    "我要與你流亡到海外",
    "或者我們都不需要真的戀愛",
    "或者我們只不過需要間唔中去一次諾士佛臺",
    "或者我們只不過需要一種新的戀愛"
  ],
  [
    "Where are you?",
    "Oh my dear Debbie",
    "Don't you know me?",
    "I am your little P",
    "Yeah...",
    "",
    "I dance alone",
    "You just stand near Jerry",
    "Never give me fantasy",
    "Oh my darling Debbie",
    "",
    "Oh my Debbie, can you imagine",
    "If one day I disappear",
    "And will you cry for me?",
    "Will you miss me?",
    "",
    "Oh my Debbie, you make me crazy",
    "If you still talk with Jerry",
    "I could not sleep",
    "How could I sleep?",
    "",
    "Na…",
    "",
    "Oh my Debbie, Jerry laughed at me",
    "He said that I was in a dream",
    "Do you think it's a dream to me?",
    "Oh my dear Debbie",
    "",
    "I have a dream",
    "Holding hands with Debbie",
    "Kissing her lips",
    "Touching her skin",
    "",
    "Where are you?"
  ],
  [
    "Dolly my little dog",
    "Fur in black and brown",
    "She met her boyfriend Rocky",
    "They played in the ground",
    "One winter night",
    "A baby dog was born",
    "Unfortunately",
    "Dolly died next morning",
    "I cried in the morning",
    "",
    "BB the baby dog",
    "Lost her mother Dolly",
    "Thus we fed her",
    "Pretend to be her mother",
    "BB grew up",
    "And had babies",
    "Maybe that's fate",
    "One day the wolf dog bit her",
    "The wolf dog bit her",
    "",
    "The wolf dog bit her neck",
    "And BB couldn't breathe",
    "She's puling on the floor",
    "Her blood sketched on her chin",
    "Her son's watching and barking",
    "He was too small to do fighting",
    "I didn't save my darling",
    "I was sleeping",
    "",
    "Dolly my little dog",
    "Fur in black and brown",
    "She met her boyfriend Rocky",
    "They played in the ground",
    "One winter night",
    "A baby dog was born",
    "Her name is BB",
    "Her name is BB",
    "Her name is BB"
  ],
  [
    "It's Kate's first time to a friend's wedding",
    "Everyone looked nice with their nice dresses",
    "Kate hugged the bride and gave her best blessing",
    "She was arranged to sit next to Jane",
    "Jane smiled to Kate with no talking",
    "Kate was confused and Jane felt interesting",
    "",
    "When you see her smile you'll understand why",
    "see-through blouse and perfume, heels and silk stockings",
    "Jane don't wear those things, she's simply brightening",
    "So that is life not just for a night",
    "Champagne hidden our secrets of lives",
    "And that is it you need no wondering",
    "You're no more a little girl",
    "",
    "After 2 days Jane messaged Kate",
    "\"Mon cheri, Mon cheri,\" Jane called Kate",
    "Oh who could help Kate she'd fallen to Jane",
    "She can't explain why her taste has changed",
    "They didn't kiss in their first dating",
    "Felt shy and smiling, forgot everything",
    "",
    "Oh little Kate, our tastes can change",
    "See how it started in the crowded wedding",
    "So that is it you cannot quit the game",
    "And that is love not just for a night",
    "Champagne hidden their secrets of pink",
    "And that is it they cannot quit the game",
    "",
    "Kate's no more a little girl",
    "Jane's no more a little girl",
    "You're no more a little girl",
    "You're no more a little girl"
  ]
]

const a4list = {
  og: [
    "介乎法國與旺角的詩意",
    "濕濕的夢",
    "北歐是我們的死亡終站",
    "窮人賣屎忽",
    "社會主義青年",
    "荔枝角網球場宣言",
    "瓜分林瑞麟三十萬薪金",
    "藍白紅風格練習",
    "my little st. valentine's day",
    "失業抗爭歌",
    "給 face 雜誌的記者 ivy",
    "when the party is over, i miss my dear porn star",
    "浪漫九龍塘",
    "阿波馬草結婚了",
    "邊一個發明了返工",
    "donald tsang, please die",
  ],
  en: [
    "Poetics - Something Between Montparnasse and Mongkok",
    "Wet Dream",
    "Scandinavia Is Our Final Destination",
    "Work Our Poor Asses Off",
    "Socialist Youths",
    "Manifesto on the Lai Chi Kok Park Tennis Courts",
    "Splitting Stephen Lam's $300,000 Payroll",
    "Exercicse de Style",
    "My Little St. Valentine's Day",
    "Anthem of the Unemployment Resistance",
    "To FACE Weekly's Ivy",
    "When the Party Is Over, I Miss My Dear Porn Star",
    "Romantic Kowloon Tong",
    "Por and Macho Got Married",
    "Who Invented Working",
    "Donald Tsang, Please Die"
  ]
}

const a4meta = [
  [
    2,27,
    'P', 'P', 'P,阿賢,阿科', 'N,P,Jackie,Sot Kanin', null
  ],
  [
    3,2,
    'P,觀鳥家果仔', 'P', 'P,N,阿賢,阿科,LWTV,Jackie,Sot Kanin', 'N,P,Jackie,Sot Kanin', null
  ],
  [
    2,9,
    'P', '阿雪', 'P', null, {
      recite: '阿雪'
    }
  ],
  [
    0,43,
    'P', 'P', 'P', 'N,阿賢', null
  ],
  [
    2,22,
    'P', 'P', 'P', 'P', null
  ],
  [
    1,6,
    'P', 'N', 'P,N', null, {
      recite: 'P,阿雪'
    }
  ],
  [
    2,13,
    'P', 'P', 'P', 'P,阿賢,Thomas,Sot Kanin,阿花', null
  ],
  [
    0,58,
    '陳寧', 'P', 'P', null, {
      recite: '陳寧'
    }
  ],
  [
    2,13,
    'N', 'N', 'P,N,LWTV', 'N', null
  ],
  [
    1,57,
    'P', 'P', 'P', 'P,阿賢,Thomas,Sot Kanin', null
  ],
  [
    1,57,
    'P', 'P', 'P', 'P', null
  ],
  [
    2,36,
    'P', 'P', 'P', 'P', null
  ],
  [
    2,6,
    'P', 'P', 'P', 'P,N', null
  ],
  [
    1,38,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,58,
    'P', 'P', 'P', 'P,阿賢,Thomas', null
  ],
  [
    1,53,
    'P', 'P', 'P', 'P', null
  ]
]

const a4lyrics = [
  [
    "俾我再約多你一次",
    "再約多一次　再多一次",
    "你我這件事　回到公司",
    "不會有人知",
    "",
    "俾我再交換多一次",
    "交換多一次　換禮物的心意",
    "放進去雪櫃　用黃色的膠袋包起",
    "不會有人懷疑",
    "",
    "其實我不願離開這公司",
    "全都因為對你太鍾意",
    "",
    "俾我再講多一次",
    "再講多一次　我的心事",
    "我最愛你是　介乎法國與旺角的詩意",
    "",
    "俾我再送多你一次",
    "再送多一次　到你家多一次",
    "送到去門前　再行多一圈",
    "再兜多一轉",
    "",
    "其實我不願離開這公司",
    "全都因為對你太鍾意",
    "",
    "其實我心願離開這公司",
    "然之後一同與你搞生意",
    "誰都知是對你太鍾意"
  ],
  [
    "這晚　要與你做個濕濕的夢（濕濕的夢）",
    "與你　去創造這深深的痛（深深的痛）",
    "這次　要跌入你黑黑的洞（黑黑的洞）",
    "你教我　要學會盡量放鬆（盡量放鬆）",
    "",
    "噢　來跟我做趟運動",
    "還請你帶我配合你節奏之中",
    "誰使我再次自覺是有用",
    "全因你流出了澎湃洶湧",
    "噢……",
    "",
    "每次　遇上你也會一起衝動（一起衝動）",
    "每晚　也要與你輕輕相碰（輕輕相碰）",
    "再次　再次共創濕濕的夢（濕濕的夢）",
    "可否　帶著我盡量去衝（盡量去衝）",
    "",
    "噢　來跟我做趟運動",
    "還請你帶我配合你節奏之中",
    "誰使我再次自覺是有用",
    "全因你流出了澎湃洶湧",
    "噢……",
  ],
  [
    "這個抑鬱的星期天",
    "我們在討論自殺的方法",
    "你說嗑藥不錯",
    "我說萬一死不去還要洗胃很辛苦",
    "割脈也太痛　不能接受",
    "燒炭　你我都認同最舒適的方法",
    "但未免太消極可悲",
    "你說不如凍死",
    "穿汗衣在北極凍死",
    "我滿心歡喜",
    "提議先到芬蘭旅遊",
    "然後穿汗衣凍死",
    "你說其實可以先環遊世界",
    "然後到芬蘭凍死",
    "好主意　我說北歐是我們的死亡終站",
    "很浪漫的想法",
    "",
    "你問打算何時進行",
    "不如就二十九歲",
    "活在永遠的二十九歲",
    "我說時間無多　要開始儲錢",
    "為了我們的計劃",
    "你說你有積蓄可以資助我",
    "我感動地　幻想",
    "二十九歲",
    "我們一起到北歐去"
  ],
  [
    "世界經濟絕望",
    "窮人賣屎忽救香港",
    "我們的積蓄都已大解放",
    "幫人做音樂最適當",
    "",
    "我們都不是收很貴",
    "我們都很等錢駛",
    "器材上亦甘願花駛費",
    "是要令客人覺得抵"
  ],
  [
    "基本上我只是一個隱蔽的青年",
    "如果你決定要和我發展",
    "我們也許不會經常見　如果見",
    "我也只會帶你流浪到海邊",
    "然之後告訴你我的一個重點",
    "我的重點是我沒有錢",
    "所以先至帶你到海邊",
    "",
    "基本上我經已不算是一個青年",
    "而且我都不適合表演",
    "這一點也應該很明顯",
    "表演時不知望向邊",
    "表演時不知應該望向邊",
    "",
    "基本上我是一個傾向社會主義的青年",
    "雖然馬克思的共產黨宣言只看過一遍",
    "但世界需要改變",
    "不要讓我失業超過半年"
  ],
  [
    "電車　電車路",
    "失業青年　白兔",
    "工人　露體狂",
    "龐克信徒　紛紛撤出電車路",
    "",
    "電影院　天台",
    "散落傳單　網球場宣言",
    "一傳十　十傳一千",
    "二十年　五十年　九十年",
    "",
    "陽光普照的下午　太多清甜的回憶",
    "觀鳥家　學生",
    "途人　合唱團",
    "神秘主義者　長頸鹿",
    "機械人　孔雀",
    "攝影師　日記簿和詩集",
    "雙雙陣亡"
  ],
  [
    "瓜分　讓我們瓜分",
    "林瑞麟每個月三十萬元的薪金",
    "讓我們的新碟會有更好的錄音",
    "攤分　讓我們攤分",
    "除開三十份　每人都仲有萬幾蚊",
    "讓我們每個月都會有覺好瞓",
    "這件事雖不能發生　但諗下亦開心",
    "林瑞麟本身都應被瓜分",
    "",
    "然後再瓜分　讓我們瓜分",
    "警察對我們社運人士的兇狠",
    "告上法庭　不會被當襲警再監禁",
    "攤分　終極地攤分",
    "攤分香港政治權力的核心",
    "權力歸於人民　領袖應該由人民普選產生",
    "",
    "這件事不知幾時發生",
    "或者到時已經過身",
    "或者我已經不能返大陸探親"
  ],
  [
    "Tu me manques, tous les jours, tous les soirs",
    "我把你的名字　栽種在一株李樹下",
    "讓枝葉伸展　向天空表白",
    "我無從訴說的愛",
    "",
    "Je vais traverser la rue pour te retrouver",
    "你給的功課　我都做完了",
    "學會了　如何撒嬌",
    "如何獨自睡覺",
    "如何點煙",
    "如何和不相識的人聊天",
    "如何離去　不驚動任何人",
    "",
    "Je me souviens de toi, comme ça",
    "我記得一些周日的早上",
    "我記得一些醒來的感覺",
    "我記得一些城市的清晨",
    "我記得你長長的眼睫毛",
    "我記得他安靜的呼吸聲",
    "我記得對街樓房的光線",
    "我記得陽台上的灰鴿子",
    "我記得深夜的便利商店",
    "我記得戲院裡你的側影",
    "我記得　我怕我將不記得"
  ],
  [
    "Feeling sad when I woke up, I thought of you at noon",
    "What I have done to you makes me feel so bad",
    "Please don't cry baby, please don't cry baby",
    "You told me that on the St. Valentine's Day",
    "",
    "Tell me I'll be ok, without you by my side",
    "Tell me you'll be happy, without the rose we planted",
    "The days have gone, yeah, the days have gone",
    "You told me that on the St. Valentine's Day",
    "",
    "Listen to the little voice, it tells you what to do",
    "What is right and what is wrong; desire's just an animal thing",
    "We're all pathetic, we try to hide it",
    "And you told me that on the St. Valentine's Day",
    "",
    "Tell me..."
  ],
  [
    "來到世上的意義是為了抗衡",
    "為了正義必要時犯法抗爭",
    "這是場戰爭但勝利是不可能",
    "犯法為彰顯法制不公允",
    "",
    "全世界也在欺壓著社會低下層",
    "年過五十低技能無法翻身",
    "大財團最終亦會炒哂你啲人",
    "小政府唔會理你幾窮困",
    "",
    "來到最後只有最後的搖滾精神",
    "我們有四五六七八九十個人",
    "這是場戰爭但勝利是不可能",
    "犯法為彰顯法制不公允"
  ],
  [
    "五月十八號你打嚟約我做訪問",
    "話想知道關於我哋首〈please die, donald tsang〉",
    "聽到你係《FACE》已經覺得有啲甘",
    "同你三唔識七受害應該都係我本人",
    "",
    "我話唔得閒亦唔想做你訪問",
    "但你打好幾次嚟又態度好誠懇",
    "所以我都俾你喺個電話度問",
    "總好過你完全亂作我個心係咁諗",
    "",
    "開始嘅時候你都係正常咁問",
    "除咗無啦啦問我有無女友同佢點諗",
    "我再三叮囑唔好寫我正職嘅身份",
    "你話都唔知我返乜工又點會有可能",
    "",
    "第日我個 friend 打嚟話見到個訪問",
    "而且當中仲問埋我間公司嘅高層",
    "嗰個仲要係陳志雲",
    "問佢點樣睇呢一位員工嘅呢一首作品",
    "",
    "Ivy　我知你本身不是這種人",
    "我們都一樣低薪兼出賣靈魂",
    "但公司裁員時不會講社會責任",
    "你又何必賣力到賣埋自己嘅良心",
    "",
    "Ivy　我知近年畢業是很不幸",
    "讀書時你應該無諗要過呢種人生",
    "仲要寫多幾多期同害多幾多個人",
    "夏天來了　不如轉吓份工唔好再等",
    "當你仲可以嘅時候要趁早起行"
  ],
  [
    "When the party is over, I miss my dear porn star",
    "When the party is over, I miss my dear porn star",
    "Do you have any boyfriend in Beijing?",
    "Do you have any boyfriend in Beijing?",
    "I don't think you have boyfriend in Beijing",
    "And I am the same",
    "",
    "I can only sing \"Sha la la\"",
    "Because I can't sing Putonghua",
    "I can only sing \"Sha La La\"",
    "Because I don't know Putonghua",
    "You will be queen of indie in Beijing",
    "I will be Hong Kong indie king",
    "Oh what am I saying?",
    "I am just kidding",
    "",
    "Sha la la la la la la la la la la la",
    "Sha la la la la la la la la la la la",
    "I know tomorrow you will be leaving",
    "And your post punk music is going",
    "And your \"mushroom head\" is going",
    "Back to Beijing"
  ],
  [
    "I want to sing you a song,",
    "About me and you went to Kowloon Tong,",
    "We have to be very strong,",
    "If we want to do something very wrong.",
    "",
    "我們終於去到了九龍塘",
    "在城大旁的酒店爆房",
    "你臉上沒有一點點的徬徨",
    "我們都似是很開放",
    "",
    "I want to sing you a song,",
    "About me and you went to Kowloon Tong,",
    "We have to be very strong,",
    "If we want to do something very wrong.",
    "",
    "我們打開房便見到一張床",
    "然後才開始覺有點慌",
    "我在想應否真的行房",
    "這樣會否破壞了交往",
    "我們會否繼續再交往"
  ],
  [
    "阿波終於跟馬草",
    "進了教堂結婚",
    "馬草都相信阿波",
    "會照顧佢一生",
    "",
    "那一整天多美好",
    "個個到來慶祝",
    "競飲比賽最開心",
    "玩到大家唔想瞓",
    "",
    "恭喜阿波找到心上人",
    "佢又會肯跟你結婚",
    "恭喜馬草找到個好男人",
    "佢為人搞笑但認真",
    "",
    "婚宴結束再去飲",
    "飲到男女唔識分",
    "卒之阿雪飲醉酒",
    "未入廁所已經嘔"
  ],
  [
    "邊一個發明了返工",
    "我要給佢米田共（米田共）",
    "邊一個發明了返工",
    "返到我愈嚟愈窮（愈嚟愈窮）",
    "為了薪金一萬元",
    "令每天都沒了沒完",
    "一萬元一萬元一萬元",
    "靈魂賣給了大財團",
    "",
    "邊一個發明了返工",
    "以為自己好有用（好有用）",
    "邊一個發明了返工",
    "阻礙我藝術發展重重（發展重重）",
    "到了薪金兩萬元",
    "我的青春就快用完",
    "兩萬元兩萬元兩萬元",
    "我有更多事沒法做完",
    "",
    "我渴望有一個平原",
    "沒有政府沒有治權",
    "這平原這平原這平原",
    "有好多個美女舞蹈團",
    "",
    "邊一個發明了返工",
    "唔係八婆就係八公（就係八公）",
    "邊一個發明了返工",
    "返工喺度玩接龍（玩接龍）",
    "到了薪金三萬元",
    "我哋都應該唔會有三萬元",
    "三萬元三萬元三萬元",
    "三萬元啲人通常都有啲串",
    "",
    "我渴望這一個平原",
    "畢業之後啲同學見唔完",
    "見面唔會講買股票",
    "買車　結婚　生仔　買樓　買船"
  ],
  [
    "Donald Tsang, please die.",
    "我哋實上街",
    "Donald Tsang, please die.",
    "When will you be fired?",
    "假設 Donald 今日你俾人斬咗隻手",
    "二十年後嗰個人發咗達又做埋特首",
    "你會否因為佢嘅成就",
    "然後叫自己不要追究？",
    "",
    "Donald Tsang, please die.",
    "我哋實上街",
    "Donald Tsang, please die.",
    "We're all poor guys.",
    "董建華雖然做野係渣",
    "但係良心都唔會好似你咁差",
    "你任內最驕人嘅成就",
    "就係強化市民對民主嘅要求",
    "",
    "Donald Tsang, please die.",
    "When will you die?",
    "Donald Tsang, please die.",
    "When will you be fired?"
  ],
]

const a5list = {
  og: [
    "九龍公園游泳池",
    "迷人的頸巾",
    "通利的阿賢",
    "我是為了兩千蚊才到這裡表演",
    "豬隻在城中逐一消失",
    "terence 的心事",
    "你是浪子，別泊岸",
    "if you bring me to the party",
    "milan",
    "rm1210",
    "公司裁員三百人",
    "搭的士上班去",
    "西西弗斯之歌",
    "給金鐘地鐵站車廂內的人",
    "直到人類滅亡",
  ],
  en: [
    "Kowloon Park Swimming Pool",
    "Fascinating Scarf",
    "Tom Lee's Ah Yin",
    "I Came Here to Preform Just for the $2000",
    "Pigs Are Vanishing One By One",
    "Terence's Worries",
    "You're a Drifter, Don't Dock",
    "If You Bring Me to the Party",
    "Milan",
    "Rm1210",
    "Laid Off 300",
    "I Go to Work by Taxi",
    "The Hymn of Sisyphus",
    "To Those On Board at Admiralty Station",
    "Until We Go Extinct"
  ]
}

const a5meta = [
  [
    2,39,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,19,
    'P', 'P', 'P', 'N', {
      poem: '阿雪',
      recite: 'Pierre Martin'
    }
  ],
  [
    2,12,
    'P', 'P', 'P,N,阿賢,阿科', 'N,Jackie,Sot Kanin', null
  ],
  [
    1,51,
    'P', 'P', 'P', 'N', null
  ],
  [
    1,41,
    null, 'P', 'P', null,  {
      poem: '阿雪',
      recite: '阿雪',
    }
  ],
  [
    2,25,
    'P', 'P', 'P', 'N', null
  ],
  [
    1,48,
    'P', 'P', 'P', 'N', null
  ],
  [
    1,33,
    'P', 'P', 'P', 'Michhy @ The Evening Primrose', null
  ],
  [
    1,44,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,2,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,18,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,5,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,15,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,13,
    'P', 'P', 'P', 'P', null
  ],
  [
    1,19,
    null, 'P', 'P', null,  {
      poem: '阿雪',
      recite: '阿雪',
    }
  ]
]

const a5lyrics = [
  [
    "我喜歡九龍公園游泳池",
    "那個戲水池有個瀑布位置",
    "瀑布下站著能忘記煩惱事",
    "每個星期我都會去一次",
    "",
    "我喜歡九龍公園游泳池",
    "那裡我不再執著一些往事",
    "我原是世間其中的粒子",
    "如何沖擊我都可以",
    "",
    "就在那時我變得不再幼稚",
    "就在那時我想重新開始",
    "二百年後這裡什麼也都不是",
    "宇宙裡有什麼不是暫時？",
    "",
    "就在我要離開這個瀑布時",
    "我突然游得更加輕易",
    "晚上折射到池底的燈光很美　但是",
    "我都要離去不留戀到八時"
  ],
  [
    "如果只是朋友　為什麼織條頸巾",
    "俾我在冬天戴　又常同我出街？",
    "如果只是朋友　為什麼俾我拖手",
    "每天放工之後　在調景嶺等候？",
    "但我始終都不能　在情人節跟你一起瞓",
    "",
    "Je pense à toi, tous les jours et tous les soirs.",
    "J'espère que tu es toujours là, assis à côté de moi.",
    "Je te donne le thé et je prends le vin.",
    "Je te regarde en silence,",
    "Tes yeux, tes mains, tes lèvres, ton sourire.",
    "Aujourd'hui est le jour de l'amour,",
    "je veux disparaître avec toi.",
    "Dis moi, dis moi, est-ce que tu penses à moi?",
    "Ì non, il faut que j'arrête,",
    "Que je m'arrête avant que je ne t'aime trop.",
    "",
    "如果無男朋友　你會否跟我飲酒？",
    "每當我看著你　我也無法開口",
    "如果無男朋友　或者我會更怕醜",
    "更加擔心你會　被更多人搶走",
    "但我始終都不能　在情人節跟你一起瞓"
  ],
  [
    "通利的阿賢　她今晚不發言",
    "這個晚上　她在我們後面彈著結他",
    "她想著另一個他　他可能在我們前面",
    "心裡面嘻嘻哈哈　等佢送花",
    "",
    "但係等咗一晚　等咗兩晚",
    "等到最後的今晚",
    "都沒有送到花　辜負了大家",
    "",
    "通利的阿賢　我哋相識都有五年",
    "再過五六年　你會唔會同鼓手阿科拍拖？",
    "通利的阿賢　我經常失落沮喪於人前",
    "但每次見面　你都笑容滿面",
    "到底你是怎樣度過你的童年？"
  ],
  [
    "老實說我是為了兩千蚊",
    "才來到這個地方表演",
    "玩了音樂經已六年",
    "這些年我有許多改變",
    "",
    "譬如說我已知這音樂不會有發展",
    "譬如說我的旋律愈寫愈不自然",
    "譬如說我已預備在某個夏天",
    "會回到我的起點",
    "",
    "那一年的一個夏天",
    "我和十二個朋友建立了維港唱片",
    "那一年是人生過得最快的一年",
    "那些清甜的回憶",
    "我仍能看見",
    "",
    "有些人過多五年會變很有錢",
    "有些人過多五年窮到不再露面",
    "而你會不會記得這一天？",
    "我為了兩千蚊來到這裡表演"
  ],
  [
    "豬隻在城中逐一消失",
    "昨日在北角　今日在鰂魚涌",
    "牠們一隻一隻　迷失了路途",
    "四處閒遊",
    "牠們一隻一隻　闖進了禁區",
    "大禍臨頭",
    "",
    "人類普遍怯懦",
    "一旦遇見豬隻",
    "便如臨大敵　先發制人",
    "一擊　二擊　三擊",
    "不求活捉　只求了結",
    "牠們驚叫　逃亡　無力反抗",
    "最後倒地身亡",
    "",
    "人類終究地位崇高",
    "可以主宰一切",
    "豬隻感到無力",
    "只能默默承受一切的不公平",
    "對於豬隻",
    "社會對牠們做過甚麼？",
    "我們對牠們做過甚麼？"
  ],
  [
    "我已經中了你愛情的箭",
    "我正等你為我拉開褲鏈",
    "你的眼神猶如兩支火箭",
    "眨一眨我便會飛到上天",
    "是你　是你　是你",
    "是你先可以令我沒法睡眠",
    "是你　是你　是你",
    "是你將愛的感覺再次燃點",
    "",
    "我的少男情懷已早改變",
    "但你卻會令我再紅了臉",
    "我不可以被你見太多遍",
    "我怎可以被你知我發癲",
    "是你　是你　是你",
    "是你先可以令我沒法睡眠",
    "是你　是你　是你",
    "是你將愛的感覺再次燃點",
    "",
    "是你　是你　是你",
    "是你先可以令我沒法睡眠",
    "是你　是你　是你",
    "是你將愛的感覺再次燃點",
  ],
  [
    "你是浪子別泊岸",
    "就算她笑得多好看",
    "間中回味是最適當",
    "泊岸以後誰想再看？",
    "",
    "你是浪子別泊岸",
    "一般人都難以抵抗",
    "眾生都墮入情網",
    "你是我們唯一寄望",
    "",
    "每當看見路上的愛侶擁抱互望",
    "我只看得見絕望",
    "「離婚的最主因是結婚」這不是亂講",
    "一百年前已流傳西方",
    "",
    "你是浪子別泊岸",
    "清風是你的翅膀",
    "太陽帶給你光芒",
    "泊岸卻令你枯乾"
  ],
  [
    "If you bring me once more to the party,",
    "I promise that I'll behave like a lady.",
    "I understand I'm no longer twenty,",
    "but I'm still looking for the sweetie.",
    "do do do..."
  ],
  [
    "你告訴我一個　昆德拉的故事",
    "關於一對情侶　假裝陌生男女",
    "再勾引對方去　酒店裡一起睡",
    "最終勾引到分不到對方和自己本身是誰",
    "",
    "故事聽到心醉　我都想請你考慮",
    "假裝你我是一對　知你有男友伴隨",
    "你我不必一起睡　睡了什麼都會失去",
    "我最多只是想拖你的手　過一條馬路去",
    "",
    "但我沒說這幾句　我不可說這幾句",
    "因我再也不是　你現在這個年歲",
    "「什麼都會過去　什麼都總會過去」",
    "我像一個失落的港女　唱Ｋ最喜歡唱這幾句"
  ],
  [
    "或者是過了今年　或者是過了明年",
    "你說會與他結為夫妻　永遠不變",
    "最好是今年　我說不要等明年",
    "既然你已說好　從那天起我們不再見",
    "",
    "然後我步入中年　然後我步入老年",
    "我或已經忘記　有否到過你婚宴",
    "而我不想忘記那天　我們遇見的那年",
    "那年吃過幾多薯片　看過幾多套港產片？",
    "",
    "知你們不會改變　你們計劃了多年",
    "就趁這之前　我仍願繼續發展",
    "然後我會等你來電　不會給他發現",
    "就算你為兒女改名　我都會給意見"
  ],
  [
    "裁員潮又再次出現",
    "據說這次要裁走三百件",
    "但員工的心態已經改變",
    "反而擔心自己不在名單裡面",
    "反而擔心裁員只不過是傳言",
    "員工都驚怕夢想不能兌現",
    "他們都想逃脫企業的魔掌",
    "他們都不想在這裡留得長",
    "要離開又順便取得一個月賠償",
    "入職三年都只係加到一萬蚊一個月糧",
    "",
    "公司傳聞　每到凌晨",
    "樓梯轉角　有樂隊四人",
    "疑似鬼魂　但歌曲感人",
    "副歌部份　大概係咁：",
    "「我要你裁走我　我要你裁走我",
    "但是你每次都裁錯　裁親的人其實都不錯",
    "我要你裁走我　我要你裁走我",
    "但是你每次都裁錯",
    "我才是公司最應該被裁走的那個」"
  ],
  [
    "你說你總在一星期　有一天真的不想返工",
    "我說這並不稀奇　亦符合人性的一種",
    "反而我有一種想法　常被朋友指摘",
    "我說要每個星期　找一天搭的士返工",
    "",
    "搭的士返工　讓你減輕傷痛",
    "即使已很窮　但這個世界已經失控",
    "既然你的錢最後亦被剝削在每月的人工",
    "為何不改善在一天的交通？",
    "",
    "你改變不了現在",
    "你改變不了將來",
    "你改變不了這時代",
    "大學時你的銳氣現在已不再",
    "你怎能每日每夜也繼續在打工",
    "直到死咗都未搭過的士返工"
  ],
  [
    "呢年嚟我有幾份兼職　其中嘅一份",
    "係幫啲馬迷落注　喺電話投注中心",
    "喺裡面我識唔到朋友　返工食飯都係一個人",
    "我已經悶到抽筋　但要維生",
    "我諗到一個方法抗衡",
    "",
    "開閘前半分鐘　總有好急嘅客人",
    "落注嘅時侯夾雜粗口　聽到我一嚿雲",
    "但佢哋愈係忟　我就愈係斯文",
    "我話：「麻煩你重複一次」",
    "搞到夠鐘開閘　佢哋就問候我屋企人",
    "",
    "佢鬧我正一懵撚　話要叫個經理出嚟問",
    "我都好有禮貌咁回應：「先生，麻煩你等等」",
    "我既緊張又興奮　同時又扮晒殷勤",
    "喺呢個心情咁複雜嘅搏鬥裡面",
    "我開始搵到工作嘅快感",
    "",
    "希臘神話有一個故事",
    "講述西西弗斯受到諸神嘅懲罰",
    "要喺地獄不斷推一塊巨石上山",
    "上到山頂　塊巨石又會自己碌返落山腳",
    "佢每日都要重複呢種徒勞無功嘅工作",
    "直到永恆",
    "",
    "後人有一個講法",
    "話諸神並唔係用「推石頭」嚟懲罰西西弗斯",
    "而係用觀念",
    "用「我永世都要推石頭實在太慘」嘅呢個觀念",
    "西西弗斯知道自己改變唔到命運",
    "佢唯一可以做嘅　就係繼續推石頭",
    "直到有一日",
    "佢發現佢可以蔑視自己嘅命運",
    "甚至用享受呢個過程嚟去否定諸神對佢嘅懲罰",
    "於是　佢感覺到自己係快樂嘅"
  ],
  [
    "曾經你都是夾在月台上的人",
    "當時你覺得入了車廂內的人",
    "有責任盡量前行",
    "",
    "如今你經已是進入了車廂內的人",
    "但你忘記了四班車之前",
    "你的月台人身份",
    "",
    "機會來了你變更",
    "你不再為後面的人諗",
    "曾經你是月台上有理想的人",
    "對月台人充滿憐憫",
    "",
    "金鐘地鐵站車廂內的人",
    "為什麼你們不行入啲呢？",
    "你們曾經都是夾在月台上的人",
    "曾經都希望前面的人行入啲",
    "但光陰似箭日月如梭時移勢易",
    "等咗四班車之後你們的機會來了",
    "入了車廂你們就不再行入啲",
    "歷史不會原諒你們　渣滓",
    "你們曾經是夾在月台上有理想的人",
    "理想是什麼？",
    "人們說理想是在彼岸",
    "但你知道理想就是逼前面啲人行入啲",
    "入啲　再行入啲",
    "",
    "改變在入車門時發生",
    "你發覺搭兩個站無謂入咁深",
    "所以站近門邊諗住容易啲走人",
    "不再前行為他人犧牲",
    "這事每日也發生"
  ],
  [
    "親愛的　末日來了",
    "你說怎麼辦？",
    "我們開始儲糧吧",
    "你喜歡吃的　都為你備好",
    "趁假日一起到野外去",
    "預早尋找神秘的洞穴",
    "好等末日前藏起你與我",
    "",
    "你說你還未去過歐洲",
    "還未看透我的臉",
    "還未與我好好瘋狂過",
    "我說九月一起去巴黎吧",
    "在末日前嚐一口香榭麗榭的熱朱古力",
    "或到王爾德墓前畫一幅畫",
    "一起模仿波德來爾的遊蕩生活",
    "",
    "親愛的　末日來了",
    "你說怎麼辦？",
    "我說讓我們緊貼著",
    "在高山盡頭　看城市殞落",
    "你說你害怕從高處墮下",
    "我說我會牽著你手",
    "保護你到最後",
    "直到人類滅亡"
  ]
]

const a6list = {
  og: [
    "我們一起離開吧",
    "回到中學的暑假",
    "牛頭角青年",
    "how can you fall in love with a guy who doesn't know Gainsbourg?",
    "印度",
    "my little angel",
    "憂傷的嫖客",
    "爺就是一名辭職撚",
    "廣州足浴一夜",
    "大象",
    "寂寞的星期五",
    "菊花的味道",
    "去信和賣碟",
    "羅曼蒂克",
    "RE：迷惘的人生",
    "love is not a romantic song",
    "悼二零一二年的夏",
  ],
  en: [
    "Let's Leave Together",
    "Back to the Summer in Secondary School",
    "Youths of Ngau Tau Kok",
    "How Can You Fall in Love With a Guy Who Doesn't Know Gainsbourg?",
    "India",
    "My Little Angel",
    "Sorrowful Whoremonger",
    "I am a Fucking Quitter",
    "A Night of GuangZhou Foot Massage",
    "Elephant",
    "Lonely Friday",
    "The Taste of Chocolate Starfish",
    "Selling CDs in Sino Centre",
    "Romantic",
    "RE: A Perplexed Life",
    "Love Is Not a Romantic Song",
    "Mourning the Summer of 2012"
  ]
}

const a6meta = [
  [
    2,38,
    'P', 'P', 'P', 'N', null,
    // {
    //   bass: '阿賢'
    // }
  ],
  [
    2,4,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Jackie,Karena',
      // bass: '阿賢',
      // guitar: 'Mable @ 在草地上'
    }
  ],
  [
    2,39,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Jackie,Karena',
      // bass: '阿賢'
    }
  ],
  [
    2,0,
    'P', 'P', 'P', 'N', null
  ],
  [
    1,13,
    null, null, 'P', null, {
      poem: '阿雪',
      recite: '阿雪'
    }
  ],
  [
    1,37,
    'P', 'P', 'P,N', 'N', {
      bass: '阿賢',
      // slide_guitar: 'N'
    }
  ],
  [
    1,58,
    'P', 'P', 'P', 'P,N', {
      recite: 'P,N',
      // bass: '阿賢'
    }
  ],
  [
    2,45,
    'P', 'P', 'P', 'N,P', null,
    // {
    //   bass: '阿賢'
    // }
  ],
  [
    2,20,
    'P', 'P', 'P', 'N', null
  ],
  [
    1,15,
    null, null, 'P', null, {
      poem: '阿雪',
      recite: '阿雪'
    }
  ],
  [
    2,16,
    'P', 'P', 'P', 'N', null,
    // {
    //   bass: '阿賢'
    // }
  ],
  [
    2,8,
    'P', 'P', 'P', 'N', null,
    // {
    //   bass: '阿賢'
    // }
  ],
  [
    2,9,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,13,
    'P', 'P', 'P', 'P,N', null
  ],
  [
    2,5,
    'N', 'P,N', 'P', 'N', null
  ],
  [
    1,4,
    'P', 'P', 'P', 'N', null
  ],
  [
    1,30,
    null, null, 'P', null, {
      poem: '阿雪',
      recite: '阿雪',
      // singing_bowl: '阿雪'
    }
  ]
]

const a6lyrics = [
  [
    "我想變做個木偶",
    "我想變做個石頭",
    "我想變做山丘",
    "我想變做小狗",
    "",
    "點先可以沒憂愁？",
    "點先可以沒淚流？",
    "點先可以沒內疚？",
    "點先可以自由？",
    "",
    "我想帶你去歐洲",
    "我想帶你去月球",
    "想帶你環遊宇宙",
    "會否把生命看透？",
    "",
    "那天你睡著顫抖",
    "我的心裡在淚流",
    "上天為何不保佑？",
    "為什麼要我們接受？",
    "",
    "我家不在這星球",
    "太深業力的地球",
    "每當我望見星宿",
    "我都不想逗留"
  ],
  [
    "或者就這樣吧",
    "不要再去找她",
    "你經已失去她",
    "便由她去吧",
    "",
    "就這樣算了吧",
    "或者你不夠喜歡她",
    "如果你夠喜歡她",
    "不會暗地喜歡這牽掛",
    "",
    "是很久沒這感覺吧",
    "似回到中學的暑假",
    "那年第一次失去她",
    "還未學會瀟灑",
    "",
    "是結束得不夠浪漫吧",
    "你才這麼不捨得她",
    "如果永遠也離不開她",
    "你便會更害怕",
    "",
    "現在不是更好嗎",
    "她永遠變了一幅畫",
    "你只需要間中掛起她",
    "",
    "原諒一下自己吧",
    "你對自己一向這麼差",
    "帶來了這麼多惡果",
    "要你慢慢消化"
  ],
  [
    "牛頭角都玩厭了",
    "我們還可以去邊？",
    "全世界都有暴動的青年",
    "但香港幾時先出現？",
    "",
    "再過春天　再過秋天",
    "這裡都不會改變",
    "或者永遠都不變",
    "如果我們都只願做旁觀的青年",
    "",
    "牛頭角的日出都看厭",
    "時間不站在你身邊",
    "沉迷過的樂隊解散了",
    "還記得穿橙色衫的阿 Tim",
    "",
    "「在吐煙」　「在發癲」",
    "首歌叫〈究竟應該點？〉",
    "事隔差不多二十年",
    "不知你是否已不是個「討厭的青年」？",
    "",
    "而我想寫首法文歌叫「Jamais」",
    "「永遠不會」的意思",
    "« Jamais je ne t'ai dit que je t'aimerai toujour »",
    "",
    "過了秋天　再過冬天",
    "我都回不到你身邊",
    "然後這一首歌的最尾一句叫「adieu」",
    "意思是「永遠的再見」"
  ],
  [
    "How can you fall in love with a guy who doesn't know Gainsbourg?",
    "How can you fall in love with a guy who doesn't know Gainsbourg?",
    "If you expect him to be sweet with you,",
    "I will say good luck to you.",
    "",
    "You can fall in love with someone who dislikes Gainsbourg.",
    "But you can't fall in love with a guy who doesn't know Gainsbourg.",
    "If you expect him to entertain you,",
    "your expectations will fool you.",
    "",
    "I translated all the lyrics of Gainsbourg with an online French dictionary.",
    "Finally I understand the secret of romance. Do you know the secret of romance?",
    "The secret of romance is to do something very man.",
    "",
    "After you fall in love with a guy who loves Gainsbourg.",
    "You won't be satisfied with someone who doesn't know Gainsbourg.",
    "Especially someone who looks very cool,",
    "and all the girls loved him in school.",
    "",
    "If you expect him to be sweet with you,",
    "I will say good luck to you.",
  ],
  [
    "我沒有名字",
    "沒有臉孔",
    "沒有手腳",
    "沒有焦點",
    "我是湧向海洋的流水",
    "我是陶瓷之前的泥土",
    "我是吹過你身邊的微風",
    "有時候　我是全世界",
    "有時候　我甚麼也不是"
  ],
  [
    "再過兩日你便要走",
    "新的他會接手",
    "與你下次再對偶",
    "我相信在以後",
    "",
    "上世與你不能放手",
    "今世總算有邂逅",
    "放眼望這大氣世界",
    "我倆永心有相溝",
    "",
    "請你不要傷憂",
    "我也不淚流",
    "如若想我",
    "可以夢裡合奏"
  ],
  [
    "［客］",
    "我就這樣痛哭一場",
    "在陌生女子懷內失常",
    "［鳳］",
    "你說我長得跟你舊愛很像",
    "才令你突然這樣憂傷",
    "",
    "［鳳］",
    "「你不用那麼傷心，",
    "早晚你會找到一個喜歡你的人。」",
    "［客］",
    "「不是，是我自己不想再跟人一起，",
    "我覺得我跟其他人不能相處」",
    "［鳳］",
    "「如果不合適就早點分開，",
    "以後的日子還長呢」",
    "",
    "［鳳］",
    "我說人生的經歷總無常",
    "你又何必介懷心上？",
    "一切苦與樂最終都一樣",
    "是為旅途增添花樣",
    "",
    "［客］",
    "我一路喊",
    "你一路安慰我",
    "［鳳］",
    "「你跟她一齊多久？",
    "為什麼要分開？」",
    "［客］",
    "我越講越激動",
    "直到走嘅時候　你問……",
    "［鳳］",
    "「哎，小費可以多給一點嗎？」",
    "［客］",
    "我先突然無咁傷心",
    "",
    "由不相識再到交心一場",
    "然後又回復正常",
    "四十分鐘的關係似夢一樣",
    "人生經歷總無常",
    "你又何必介懷心上？"
  ],
  [
    "爺就是一名辭職撚",
    "錢不需要搵亦可維生",
    "爺今生是有任務在身",
    "就是遠離瘋狂的人群",
    "",
    "爺有的是顆自由的心",
    "風雨中不需要同行",
    "爺有時是戀愛顧問",
    "但從不交出真心",
    "",
    "爺就是這麼一名負心撚",
    "算你不幸如果你行太近",
    "爺愛當女人是飄過的塵",
    "誰會對微塵傷心",
    "",
    "爺曾經都有人擔心",
    "如今經已無人過問",
    "爺就是喜愛做這一種人",
    "各人有各人的修行",
    "",
    "爺夜深時曾經問心",
    "有沒有太過傷害別人",
    "才發覺世事總有原因",
    "沒有爺她們學不到死心",
    "",
    "爺是她們要修的行"
  ],
  [
    "人生是什麼？",
    "我今晚不知道喎",
    "只想你不要難過",
    "甚麼都不算甚麼",
    "",
    "明天我不會來過",
    "快樂不捨得用太多",
    "只知我今晚如何過",
    "明天你已忘記我",
    "",
    "你話你未離開過廣州",
    "你話你未見過海",
    "你問香港係唔係好繁榮",
    "你問香港嘅海係點",
    "你話上年返鄉下種田之後覺得",
    "人應該留喺城市",
    "我話香港有啲年青人開始學種田",
    "你話香港咁繁榮",
    "點會有人去種田",
    "",
    "努力又是什麼？",
    "如果目標已經出錯",
    "這裡有沒有人清楚",
    "越努力越錯得多",
    "",
    "人生是什麼？",
    "我今晚不知道喎",
    "這裡有沒有人可",
    "將一切告訴我"
  ],
  [
    "聽說　每日傍晚時分",
    "大象便會在對岸出現",
    "為了見牠　我每日",
    "同一時間　同一地點",
    "凝望著對岸　盼望大象",
    "一日　一星期　一個月",
    "大象始終沒有出現",
    "到我放棄等待的那天",
    "牠施施然走到河邊飲水",
    "趕去時",
    "只瞥見牠背影一眼"
  ],
  [
    "這一個星期五",
    "你說不會再上來坐",
    "我知日後的星期五",
    "我將要一個人過",
    "",
    "這些年我犯過的錯",
    "如果可以再來過",
    "我都會揀這樣過",
    "我就是這麼卑劣的我",
    "",
    "吃了一個「如果」",
    "再吃一個還肚餓",
    "如你現在覺得罪過",
    "只因你開始對厭我",
    "",
    "這天你說你和我",
    "如像兩個小孩闖了禍",
    "現在你覺得他真不錯",
    "只因你看清楚了我"
  ],
  [
    "La la la...",
    "",
    "我記不起多久沒試過菊花的味道",
    "只知道沒你我不會嚐到",
    "我要怎麼答謝你給我的引導",
    "為我帶來這奇異的喜好",
    "",
    "La la la...",
    "",
    "這世間的道理我未能逐一參透到",
    "只知太快樂是一種虛無",
    "只有菊花的味道最能令我驕傲",
    "含淚的快樂才是高分數",
    "",
    "你給了我這一種菊花的味道",
    "亦從此為我帶來煩惱",
    "我不知道可以跟誰來傾訴",
    "只知黑暗不再像從前恐怖",
    "",
    "每一分每一刻你我都步向衰老",
    "還要堅守甚麼純潔的信徒",
    "如未真的試過這奇異的喜好",
    "又如何確定它的不好？",
    "",
    "La la la...",
  ],
  [
    "到你再過多兩年",
    "差不多三十歲的某天",
    "你會去最後一次二手唱片店",
    "",
    "賣去過往十數年",
    "買落的幾百張唱片",
    "這個時候你要賣得有技巧一點",
    "",
    "技巧是要知道有兩種唱片店",
    "一種是每張碟報不同價錢",
    "一種是一個價錢報一堆唱片",
    "",
    "你要以第一種店為先",
    "它不會收下所有唱片",
    "餘下的就再賣去第二種店",
    "",
    "這次序當中的重點",
    "不只可整體賣高一倍價錢",
    "還可讓你盡量忘掉眼前",
    "",
    "事關一下子失去所有唱片",
    "會有把夢燒光的感覺出現",
    "忙著分析價錢有助讓這感覺不見"
  ],
  [
    "因我的臉並不好看（不好看）",
    "因我的事業亦不好講（不好講）",
    "但如你能陪我遊一趟",
    "浪漫的巴士河 woo",
    "",
    "我會聽你心事",
    "我知你想講",
    "你心裡的計劃是離開香港",
    "但未有勇氣去闖",
    "",
    "剛好我想有一些釋放（一些釋放）",
    "剛好你的味道是多麼芬芳（多麼芬芳）",
    "你坐在我身旁",
    "令我滿心飄盪",
    "我就不妨直講 woo",
    "",
    "我已經不再是當初的文藝腔",
    "我已經不再是每天都覺沮喪",
    "我只想摸你乳房",
    "我只想摸你乳房",
    "我只想摸你乳房",
    "我只想摸你乳房",
    "摸你乳房"
  ],
  [
    "我最親愛的朋友",
    "請您不用憂愁",
    "傷悲說不出口",
    "可以把眼淚流",
    "",
    "更深厚的情愛",
    "總有受挑戰時候",
    "前世您們纏糾",
    "今世愛過夠",
    "",
    "呼出您的困憂",
    "世事只是水流",
    "轉世前決定必修",
    "學會後回到宇宙",
    "",
    "等到了時候",
    "就自然能看透",
    "學會把執著放手",
    "自在自然足夠",
    "",
    "無須內疚",
    "無須牽求",
    "若要人分憂",
    "隨時打俾我呼救"
  ],
  [
    "Love is not just a song.",
    "Love is not just you and me.",
    "Love is not the kiss you're looking for so long.",
    "Love is not just words and melody.",
    "Love is not one night in a week.",
    "Love is not missing someone with tears.",
    "Love is not the passion you think you got a lot.",
    "Love is just the years.",
    "Love is just the thing you fear to give."
  ],
  [
    "這個夏天很冷",
    "陽光很猛",
    "卻失去暖人的溫度",
    "你的離開不合情理",
    "我常在默想",
    "那幾分鐘之前",
    "你在幹甚麼？",
    "那幾分鐘之前",
    "一切有預兆麼？",
    "你以砍頭的力量追尋自由",
    "我們腳踏實地為你延續理想",
    "今天",
    "約定彼岸的你",
    "在下一個炎夏",
    "看繁花盛放"
  ]
]

const a7list = {
  og: [
    "土瓜灣情歌",
    "給親戚看見我一個人食吉野家",
    "愛情 disabled",
    "今晚講嘢夜唔夜",
    "男神與寇比力克",
    "年輕的茶餐廳老闆娘",
    "我在暗中儲首期",
    "海心公園",
    "美麗新香港",
    "月映之審判者",
    "京都民宿夜",
    "已婚男人",
    "阿叔今年四十歲",
    "五點鐘去天光墟",
    "四句",
  ],
  en: [
    "To Kwa Wan Ballad",
    "My Relatives Saw Me Eating in Yoshinoya",
    "Love Disabled",
    "Is It Too Late to Chat Tonight",
    "Boygod and Kubrick",
    "Cha Chaan Teng's Young Boss Lady",
    "I'm Saving the Down Payment in Secret",
    "Hoi Sham Park",
    "Brand New Hong Kong",
    "Reflector's Judgement",
    "A Night in Kyoto Guesthouse",
    "Married Men",
    "Uncle is 40 This Year",
    "To Dawn Market at 5AM",
    "Four Lines"
  ]
}

const a7meta = [
  [
    2,23,
    'P', 'P', 'P,N,阿賢,阿科', 'N', {
      backup_vocal: 'Karena',
    }
  ],
  [
    2,35,
    'P', 'P', 'P,N,阿賢,阿科', 'N', null 
  ],
  [
    2,44,
    'P,陳寧', 'P', 'P,N,阿賢,阿科', 'N', {
      recite: 'N'
    }
  ],
  [
    2,41,
    'P', 'P', 'P,N,阿賢,阿科,陳銘澤 @ 在草地上', 'P,N', null
  ],
  [
    1,56,
    '阿雪', 'P', 'P,阿賢,阿科', null, {
      recite: '阿雪'
    }
  ],
  [
    3,1,
    'P', 'P', 'P,N,阿賢,阿科', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,22,
    'P', 'P', 'P,N,阿賢,阿科', 'N', {
      backup_vocal: 'Karena'
    } 
  ],
  [
    2,47,
    'P', 'P', 'P,N,阿賢,阿科,陳銘澤 @ 在草地上', 'P,N', null 
  ],
  [
    1,57,
    'P', 'P', 'P,N,阿賢,阿科,陳銘澤 @ 在草地上', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    3,21,
    'N', 'P,N,阿賢,阿科', 'P,N,阿賢,阿科', null, {
      recite: 'N'
    } 
  ],
  [
    2,17,
    'P', 'P', 'P,N,阿賢,阿科', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,35,
    'P', 'P', 'P,N,阿賢,阿科,陳銘澤 @ 在草地上', 'P,N', null
  ],
  [
    2,24,
    'P', 'P', 'P,N,阿賢,阿科,陳銘澤 @ 在草地上', 'P,N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    4,0,
    'P', 'P', 'P,N,阿賢,阿科', 'P,N', null
  ],
  [
    1,42,
    'N', 'N', 'P,N,阿賢,阿科', 'N', null
  ],
]

const a7lyrics = [
  [
    "這一個下雨的夜晚",
    "你是否正在空閒",
    "我此刻仍留在土瓜灣",
    "想著未來怎麼辦",
    "只希望沙中線的那個站",
    "可以起得更慢",
    "再貴的租我已不能負擔",
    "",
    "這一個下雨的夜晚",
    "我是否仍在你心間",
    "已有多少年沒有上班",
    "煩惱為何卻未減",
    "只想歲月過得平淡",
    "知這是最困難",
    "願你我能留在黑暗中多一陣間",
    "",
    "這一個下雨的夜晚",
    "我躺在沙發矇矓間",
    "收音機說北韓已射出導彈",
    "地球另一邊被炸爛",
    "所有的事都不夠虛幻",
    "因現實未到最爛",
    "要到絕望才望見希望在人間",
    "",
    "到了某天你我不再浪漫",
    "有不喜歡的工要返",
    "你會否便記起曾笑得這麼燦爛",
  ],
  [
    "給親戚看見我一個人食吉野家",
    "差點想講句你認錯人吧",
    "但我已經懂得怎麼說些客套話",
    "即使想放棄牛肉飯趕回家",
    "",
    "早點看見他也可改叫外賣吧",
    "他已跟太太一同坐下",
    "若假裝有同伴座位要留給他",
    "講出來自己都嫌假",
    "",
    "給親戚看見我一個人食吉野家",
    "記得童年曾與他同玩耍",
    "我與他應該年紀也差不多吧",
    "今晚發覺他已成了家",
    "",
    "多年來踫到親戚我亦有點怕",
    "近況如何盡量講得不太差",
    "給親戚看見我一個人食吉野家",
    "還有什麼以後可以怕"
  ],
  [
    "當我向政府申請津貼時職員看著我然後",
    "問我申請津貼到底是基於一個什麼的理由",
    "(love disabled)",
    "當我搭巴士向司機展示我張優惠證時候",
    "司機看著我彷彿想問我究竟是什麼因由",
    "love disabled  love disabled",
    "love disabled  love disabled",
    "",
    "當我想找一個好友得到心靈上的解救",
    "總是沒有一個可以找得到我心靈的入口",
    "(love disabled)",
    "當我見到戲院的觀眾開始投入到淚流",
    "我都想知道故事有什麼值得大家憂愁",
    "love disabled  love disabled",
    "love disabled  love disabled",
    "",
    "這個世界是否真的有這種 love disabled？",
    "心裡的這個問號 誰又能解答得到？",
    "",
    "耶穌說的愛是無條件的、獻身的，",
    "奧修說的愛是能量的互動、是自由的、無束縛的，",
    "昆德拉說的愛是機遇的、偶然的、命定的，",
    "高達說的愛是刺激的、好玩的、有今生沒來世的、哲學的，",
    "小津安二郎說的愛是溫柔的、隱藏的、非愛的，",
    "畢卡索說的愛是經驗的、性慾的、美好的，",
    "夏卡爾說的愛是聖潔的、救贖的、唯一的。",
    "L'amour, mes amants, mon amour, aimer.",
    "愛情，愛人們，我的愛，去愛。",
    "而我將要說的是,",
    "L'impossibilité d'aimer dans notre temps.",
    "我們時代的愛無能。"
  ],
  [
    "［阿Ｐ］",
    "或者你可以坐得更近",
    "靠在我耳邊講嘢不會過份",
    "",
    "［Nicole］",
    "但我天生就是這麼拘謹",
    "還未有那份自信心",
    "",
    "［阿Ｐ］",
    "若你問我什麼先叫做緣份",
    "我會說緣份只留給夠進取的人",
    "",
    "［Nicole］",
    "好既然依家個情況係咁",
    "過咗今晚之後我先至再做好人",
    "",
    "［Chorus］",
    "今晚講嘢夜唔夜",
    "咁啱我就有部車",
    "車你去山頂感受個山有幾斜",
    "望你千萬不要太心邪",
    "",
    "［阿Ｐ］",
    "或者等我表白內心嘅黑暗",
    "其實我從未放低過舊愛人",
    "所以先每晚搵不同人瞓",
    "希望你不要太過黐身",
    "",
    "［Nicole］",
    "我又點會唔明白你嘅內心",
    "我又何嘗唔係想搵人瞓一瞓",
    "所以先扮拘謹等你勾引",
    "又估唔到你真係會咁易當真",
    "",
    "［Chorus］",
    "今晚講嘢夜唔夜",
    "咁啱我就有部車",
    "車你返屋企先飲杯咖啡",
    "再傾返陣先至好扯",
    "望你千萬不要太早扯"
  ],
  [
    "那個潮濕的晚上",
    "我與男神去看 2001",
    "漆黑的電影院",
    "空洞洞的兩個人",
    "十多年前看不懂的畫面",
    "巨石　星球",
    "今天終於明白了",
    "內心激動地跳著",
    "為寇比力克的時空",
    "穿梭宇宙的靈魂",
    "也為男神衣領上的迷迭幽香",
    "散場　男神體貼地送我回家",
    "我一邊訴說對寇比力克的仰慕",
    "他一邊點頭沒有搭話",
    "好看嗎？　我說",
    "看不懂。　他說",
    "為甚麼？　為甚麼？",
    "完美的男神",
    "看不懂寇比力克",
    "完美的男神",
    "缺陷能原諒嗎？"
  ],
  [
    "這是我最後一次",
    "來吃午飯吃到四點",
    "以後或不會再見",
    "到底我也是高傲的成年",
    "",
    "已考慮不知幾多遍",
    "才決定給你這信件",
    "寫了電話等你致電",
    "明知你看了只會笑一百遍",
    "",
    "這些年裡面",
    "你都知我很少到別的店",
    "但若你最終給我來電",
    "你可知道這感覺會點",
    "",
    "像一支小蠟燭的光點",
    "讓漆黑的房滿佈光線",
    "像打開考試試卷發現",
    "突然所有答案都看得見",
    "",
    "對上的一次冬至",
    "來吃晚飯吃到八點",
    "但你卻沒有出現",
    "出現我也不敢看你正面",
    "",
    "已幻想不知幾多遍",
    "旅行有你在我身邊",
    "多次見你髮型轉變",
    "又是為吸引那個他的視線",
    "",
    "這些年裡面",
    "你都知我很少到別的店",
    "但若你最終給我來電",
    "你可知道這感覺會點",
    "",
    "像一支小蠟燭的光點",
    "讓漆黑的房滿佈光線",
    "像打開考試試卷發現",
    "突然所有答案都看得見",
    "",
    "這些年裡面",
    "我也不知道我可以去邊",
    "但若你最終給我來電",
    "我可給你一整個的春天"
  ],
  [
    "可否可否等我",
    "先把計劃說好",
    "再決定穿婚紗跟他擁抱",
    "還是跟我到老",
    "我沒有他的生意頭腦",
    "但會努力讓你減去苦惱",
    "與你走過那一段路途",
    "不願這樣就算數",
    "",
    "我已快將首期儲好",
    "小小單位勉強可供到",
    "又或者繼續在工廈交平租",
    "再不斷旅遊更多回憶可以儲",
    "",
    "可否可否跟我",
    "一起私奔到寶島",
    "你不是卡列尼娜",
    "我沒佛倫斯基那麼糟",
    "不用怕會像想像得到",
    "文藝悲劇的老套",
    "去到台南風景會做到",
    "治療我們的虛無",
    "",
    "到這天先知道什麼是最美好",
    "婚姻有多好現在才知道",
    "可否不要到某年後悔沒揀好",
    "懇請你接受這份愛的 proposal",
    "",
    "不要重演西西的〈感冒〉"
  ],
  [
    "告訴李慧琼和那班新住客",
    "我是海心公園的一個阿伯",
    "千億個晚上在這裡唱歌我沒犯法",
    "投訴是來自那間新豪宅？",
    "",
    "今晚是我和朋友表演的時刻",
    "把我的愛情還給我立刻",
    "到了六十八　厭倦所有迫壓",
    "年輕才有資格被糟蹋",
    "",
    "六歲嗰年我唔小心嘈醒咗",
    "瞓緊晏覺嘅阿爸",
    "我俾佢捉咗去個島度留一日一夜",
    "島上面有好多巨型嘅怪石",
    "啲人話啲怪石好靈",
    "拜過佢嘅情侶",
    "最終都會白頭到老",
    "但點解　點解最後我無？",
    "",
    "今晚是我和朋友表演的時刻",
    "把我的愛情還給我立刻",
    "到了六十八　厭倦所有迫壓",
    "年輕才有資格被糟蹋",
    "",
    "告訴李慧琼和那班新住客",
    "最近土瓜灣變得很抑壓",
    "不知你向來有什麼政策",
    "但根本沒有我　我很明白"
  ],
  [
    "自你決定要走以後",
    "沒人知我有多難受",
    "再沒勇氣在派對逗留",
    "踫見朋友不敢問候",
    "",
    "自你遠去不再回頭",
    "青山綠水不再依舊",
    "再沒有早起身的理由",
    "再沒可安睡的枕頭",
    "",
    "這世界只有一種鄉愁",
    "就是沒有你的時候",
    "這香港已不是我的地頭",
    "就當我在外地旅遊",
    "",
    "自美夢再不可尋求",
    "我便學會清風兩袖",
    "命運要令我學識奮鬥",
    "但我只懂得荒謬",
    "",
    "這世界只有一種鄉愁",
    "是你不在身邊的時候",
    "這香港已不是我的地頭",
    "就當我在外地飄流",
    "",
    "我知已走到盡頭",
    "為何還要擔憂？",
    "這世界也不是我的地頭",
    "就當我在宇宙飄流"
  ],
  [
    "反映者林鵬",
    "世界就在你內在呈現",
    "我驚訝著",
    "甚至為之落淚",
    "為你寫的歌",
    "不為它的失落沮喪",
    "為的是萬物恆常變幻之精密之美",
    "由「你」這個有機複合體",
    "體現出來",
    "是萬有的折射",
    "是宇宙的投射",
    "然而",
    "這是不著痕跡的",
    "而你剛開始時也不知道",
    "自身的設計功能和作用",
    "",
    "親愛的反映者",
    "你就是宇宙的縮影",
    "你的任務就是反映周遭",
    "這樣就可以了",
    "在你尋找愛的真諦的道路上",
    "你所反映的",
    "正正是",
    "在學習「什麼是愛」的",
    "萬有狀態"
  ],
  [
    "七月某夜　在京都遊玩到累",
    "我在床上　十時已準備睡",
    "每朝早已　獨自四周圍去",
    "每隔兩天　住不同的民宿裡",
    "",
    "那夜聽見　房外老闆娘輕聲傾吐",
    "不知與誰　我突然回到六歲",
    "只有童年　才會比家人更早睡",
    "聽到母親　在客廳隱約的字句",
    "",
    "那夜只想　在被窩聽落去",
    "我怕忘記　從前各樣瑣碎",
    "只有那夜　才能片刻重聚",
    "謝謝來到　京都這裡",
    "",
    "ありがとうございます"
  ],
  [
    "Du...",
    "",
    "一早都告訴你女人就是女人",
    "你說不同地方的女人都是不同類人",
    "直到你的外遇開始要求你離婚",
    "你是否了解女人最終只想關係安穩？",
    "",
    "那時告訴過你但你沒有放在心",
    "我說免費的性愛是最昂貴的講真",
    "寧願每次都付費但你不同意如今",
    "便為短暫的興奮付出最昂貴的教訓",
    "",
    "最後那位外遇在網上向你太太現身",
    "那夜朋友們由食花生變到為你痛心",
    "據聞你挽救婚姻時說「孩子才剛出生，",
    "不可以離婚」然後沒人再打聽到下文",
    "",
    "其實都只因為你實在太早結婚",
    "但內心仍很想證明自己的吸引",
    "正是不夠標致又不肯安份",
    "有太多秘密但又藏得不夠深",
    "",
    "世事看似只是不幸　但都不是沒原因",
  ],
  [
    "阿叔今年已經四十歲",
    "都算係食過無數咁多女",
    "辜負過嘅點止一大堆",
    "但你話人生無悔會係幾咁無趣",
    "",
    "所以最緊要學識謙虛",
    "無咁大個頭就忍住唔好亂咁吹",
    "想分享一句人生嘅金句",
    "就係生得靚仔其實真係無罪",
    "",
    "聽眾係咁打嚟爆啲秘密出去",
    "係咪世上所有嘅朋友都已經失去",
    "我係就係唱得有啲聲衰",
    "但係就係勝在唱得有啲 Sam Hui",
    "",
    "阿叔其實過晒四十歲",
    "仲想繼續喺度玩落去",
    "對於婚姻從來都不抗拒",
    "如果有個女人可以令我心碎",
    "",
    "聽眾係咁打嚟爆啲秘密出去",
    "係咪世上所有嘅朋友都已經失去",
    "今夜讓我做住你伴侶",
    "同你一齊去尋找上太空嘅樂趣"
  ],
  [
    "do do do...",
    "",
    "我最近有一個夢想",
    "係同朋友五點鐘去天光墟",
    "我經已好耐無同朋友出去",
    "而且去一個未去過嘅地區",
    "",
    "但我點可能五點鐘就起身出去？",
    "除非我十點鐘開始入睡",
    "但香港有無人可以十點鐘入睡？",
    "除非喺屋企無端端飲醉",
    "",
    "Terence、阿雪、Peter 同阿 Cza",
    "你哋會陪我去嗎？",
    "之後再去飲早茶",
    "佢哋都應該會陪我啩",
    "",
    "do do do...",
    "",
    "但我最大嘅一個夢想",
    "其實已一早失去",
    "香港都正在死去",
    "我都經已不再唏噓",
    "",
    "這個世界叫人類堅持落去",
    "世界叫窮人奮鬥落去",
    "但我淨係可以繼續喺度",
    "無聊落去",
    "",
    "我有一隻公仔",
    "兩年嚟朝夕相對",
    "我有段日子唔想出街",
    "主要都係因為佢",
    "",
    "但我最近發現",
    "佢唔再講嘢食嘢同飲水",
    "而我亦都開始瞓唔著",
    "就算飲到好醉",
    "",
    "do do do...",
    "",
    "我諗緊好唔好等到五點鐘",
    "然後就去天光墟",
    "我諗緊瞓唔著係咪因為",
    "以前做過一啲事太衰",
    "",
    "我起返身開返部機聽鄧麗君",
    "聽佢唱到呢幾句：",
    "「不知天上宮闕 今夕是何年",
    "我欲乘風歸去……」",
    "",
    "do do do..."
  ],
  [
    "(hmm hmm hmm...)",
    "",
    "對不起　我傷害了你",
    "請原諒我對你造成的傷害",
    "謝謝你之前所做的一切",
    "我愛你　一直都愛著你",
    "",
    "對不起　我傷害了你",
    "請原諒我對你造成的傷害",
    "謝謝你之前所做的一切",
    "我愛你　一直都愛著你",
  ]
]

const a8list = { 
  og: [
    "明天不要賴床 baby",
    "愛人有新歡",
    "驗孕的下晝",
    "麥記最後一夜",
    "獨身的理由",
    "海",
    "下亞厘畢道",
    "my dear friend ka hei",
    "火炭麗琪",
    "第十支煙",
    "昨晚夢見你",
    "今天沒大麻在身",
    "離席，抽一根煙",
    "某夜電影中心",
    "我以後不再去長洲",
    "今夜到干諾道中一起瞓",
    "攻陷你的西",
  ],
  en: [
    "Don't Sleep In Tomorrow, Baby",
    "Lover Has a New Lover",
    "Afternoon of Pregnancy Testing",
    "The Last Night at McDonald's",
    "Reasons to Be Single",
    "Sea",
    "Lower Albert Road",
    "My Dear Friend Ka Hei",
    "Fo Tan Lai Ki",
    "The Tenth Cigarette",
    "Dreamt of You Last Night",
    "No Weed on Me Today",
    "Leave the Table, Smoke a Cigarette",
    "A Night at Cinema Centre",
    "I Won't Go to Cheung Chau Again",
    "Let's Sleep on Connaught Road Central Tonight",
    "Dominating Your Cunt"
  ]
}

const a8meta = [
  [
    2,25,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫,銘銘', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,5,
    'P', 'P', 'P,阿賢,阿科,銘銘', 'N,Karena', null
  ],
  [
    2,26,
    'P', 'P', 'P,阿賢,阿科,銘銘', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    3,26,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,28,
    'P', 'P', 'P,阿賢,阿科', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    1,33,
    null, 'P', 'P,N,阿賢,阿科,銘銘', null, {
      poem: '阿雪',
      recite: '阿雪'
    }
  ],
  [
    2,46,
    'P', 'P', 'P,夏日浪漫', 'N', {
      backup_vocal: 'Jackie'
    }
  ],
  [
    2,35,
    'P', 'P', 'P,銘銘', 'N', null
  ],
  [
    2,9,
    'P', 'P', 'P,阿賢,阿科', 'N', {
      backup_vocal: 'Karena,Jackie'
    }
  ],
  [
    3,56,
    'P', 'P', 'P,阿賢,阿科,銘銘', 'N', {
      backup_vocal: 'Karena,Jackie'
    }
  ],
  [
    2,27,
    'N', 'N', 'P,阿賢,阿科,夏日浪漫,銘銘', 'N', null
  ],
  [
    2,25,
    'P', 'P', 'P,阿賢,阿科', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    1,13,
    'P', null, null, null, null
  ],
  [
    2,18,
    'P', 'P', 'P,阿賢,阿科,銘銘', 'N', null
  ],
  [
    1,46,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫', 'N', {
      backup_vocal: 'Jackie'
    }
  ],
  [
    1,45,
    'P', 'P', 'P', 'P,N', null
  ],
  [
    2,5,
    'P', 'P', 'P,銘銘', 'N,P,Porpor Channel,Kit @ 22cats,Ling ling ling', null
  ]
]

const a8lyrics = [
  [
    "我有過很多瘋狂往事",
    "例如網上認識一位女子",
    "互相傾訴了數小時",
    "便結伴旅遊廈門市",
    "",
    "當然看過她照片多次",
    "四日旅程覺得可以一試",
    "若不好也是四天的事",
    "讓人生多添個故事",
    "",
    "那天我先到廈門市",
    "機場麵店內等愛情飄至",
    "又開始後悔怎麼可以",
    "轉眼她已到達面前",
    "",
    "她長得比照片還得意",
    "然後我們上了一架的士",
    "司機介紹情侶要到那兒",
    "早上出發不要遲",
    "",
    "我說明天不要賴床 baby",
    "突然飾演另一個人彷似",
    "但跟你才見面第一次",
    "我故意講多聲 baby",
    "",
    "某天她說有新對象心儀",
    "餘下我獨自回望往事",
    "事後多想時間可以",
    "就留在那程的士",
    "",
    "事後多想時間可以",
    "讓我再講多聲 baby",
    "明天不要賴床 baby",
  ],
  [
    "沒所謂的　沒所謂",
    "坦白告訴我　就沒問題",
    "沒所謂的　沒所謂",
    "他是哪一位？是何時一齊？",
    "",
    "是沒所謂",
    "都知這件事　最不可控制",
    "沒有所謂",
    "在這裡我有很多朋友仔",
    "",
    "沒所謂的　沒所謂",
    "這個大時勢　兒女私情不要計",
    "只是給你寫的第一首歌仔",
    "便以分手為題",
    "",
    "是沒所謂",
    "你做了我　從前的一切",
    "沒有所謂",
    "高高拎起　輕輕放低",
    "",
    "坦白告訴我 就沒問題",
  ],
  [
    "當我知這一次或會改變我前程",
    "應要繼續我路拒絕命運的邀請",
    "或已到某個階段應該順應天命",
    "為你的到來而高興",
    "",
    "想你應將得到眾多姨姨的垂青",
    "想你會使我努力　為這關係打拼",
    "縱是我未認清你母親的個性",
    "但婚姻無關愛情反正",
    "",
    "不會責備你若你什麼都跟我傾",
    "包括青春期會遇到的各種類型",
    "世界要你努力去考取功名",
    "但是真誠才是最大本領",
    "",
    "總會有很多事令你失去熱情",
    "總在青春過後留不住舊髮型",
    "只願驗孕結果能在此刻呈陰性",
    "不負如來不負卿",
  ],
  [
    "就在那夜送你歸家以後",
    "你再打給我說　你沒有上樓",
    "沿著我背後一路走　因見面不敢開口",
    "「明天要返英國」　電話裡淚流",
    "",
    "就到家的我即轉身回頭",
    "寒夜跟你到麥記　朱古力當是酒",
    "我若全力追究　你也會來去自由",
    "那種金牛座特性你全沒有",
    "",
    "麥記的最後一夜",
    "白燈、廣東歌與露宿者",
    "想起廟街某夜 相士說你屬於那些",
    "「矛盾波動的千里馬」般狂野",
    "",
    "就在八月知你住我家附近之後",
    "那夜經過大角咀你細訴小時候",
    "然後我們隔一兩週 便結伴公園夜遊",
    "潛入「嶺南之風」看不見四周",
    "",
    "但是我亦走不進你心裡頭",
    "某夜親吻到似變了男女朋友",
    "但是你從不愛拖手　只是放手進我衣袖",
    "代表只想取暖　是否？",
    "",
    "麥記的最後一夜",
    "白燈、廣東歌與露宿者",
    "這惡俗的不捨　比一起更好過些",
    "才是你遠走的主因　或者？",
    "",
    "麥記的最後一夜",
    "橙汁、朱古力與露宿者",
    "合照留下一些 轉地方睡最後今夜",
    "完結這一場冬天的狂野",
    "",
    "電捲風馳　從此各自上車",
  ],
  [
    "告訴我你婚後的事兒",
    "我與你已很久沒見面",
    "網上看見一篇文章說",
    "情侶分手後若相處依然",
    "代表　已是沒有愛意",
    "或者　愛意從沒有改變",
    "但我卻更覺得適用於",
    "形容情侶分手以後從沒有再見",
    "",
    "我會在 IG 看你一生的轉變",
    "在 IG 等你 BB 的出現",
    "星期天見你遛狗到山邊",
    "晚上與你姊妹的合照片",
    "偶爾也會見到他露面",
    "與你遊歷的異國景點",
    "時光飛逝　如幻似煙",
    "何時先可以停止懷緬？",
    "",
    "Muss es sein?",
    "Es muss sein!",
    "",
    "你與我唯一的接觸點",
    "是我仍用你編號買書在誠品店",
    "像「非如此不可」的德國語言",
    "埋藏在貝多芬的四重奏經典裡面",
  ],
  [
    "十年後",
    "你還會記得我們到海邊的那個晚上嗎？",
    "一整夜的風　一整天的霧",
    "短暫的快樂",
    "帶給我翌日猛烈的頭痛",
    "你笑說　這是海邊的代價",
    "我苦笑　代價未免太大",
    "春霧中　你告訴我",
    "「回憶很奇怪",
    "唯有兩個人同時記得",
    "回憶才是真實」",
    "十年後　你才年過三十",
    "我已芳華漸暮",
    "你還會記得",
    "我們第一次見面是到海邊嗎？",
    "走過的路還存在嗎？",
    "看過的風景還美麗嗎？",
    "假如你不記得",
    "那場頭痛是我虛無的幻想嗎？",
  ],
  [
    "和你兩個月前每日 inbox 傾訴",
    "初次約會要設定一種暗號",
    "如你我碰面後若想離開提早",
    "用暗號避免直接令對方失措",
    "",
    "一杯 moscato　再一杯 moscato",
    "你教我講冰島語如何問好",
    "由歌賦街飲到去下亞厘畢道",
    "只是 moscato 未能亂我腳步",
    "",
    "由兩個月前等待今晚的來到",
    "都不敢把寄望定得太高",
    "但這約會後為何開始煩惱",
    "自你離開前的一個擁抱",
    "",
    "一杯 moscato　再一杯 moscato",
    "你說你明天不用上班多好",
    "由歌賦街飲到去下亞厘畢道",
    "聽著 Sinatra 的《我不會跳舞》",
    "",
    "和你設了暗號若想離開提早",
    "但沒設有暗號若想再進一步",
  ],
  [
    "My dear friend Ka Hei",
    "這次跟你別離",
    "曾每天跟你",
    "午飯後再睇戲",
    "",
    "如今你堅決遠離",
    "這一片傷心地",
    "My dear friend Ka Hei",
    "從此得我在 café",
    "",
    "My dear friend Ka Hei",
    "下次再見不知日期",
    "曾經因你",
    "明白鄰里關係的奧秘",
    "",
    "如今你堅決遠離",
    "落山道亦為你悲",
    "My dear friend Ka Hei",
    "這年紀闖闖也合理",
    "My dear friend Ka Hei",
    "這年紀不必太傷悲",
    "",
    "My dear friend Ka Hei",
    "我都沒再去 café",
  ],
  [
    "火炭麗琪　認識她第一星期",
    "她已拚命地叫你收皮",
    "她每份工都不過四星期",
    "像愛情一般　沒有頭沒有尾",
    "",
    "火炭麗琪　約她總話會出嚟",
    "臨去一刻五成會放飛機",
    "她確實有著某一種霸氣",
    "明天的事　今天她不會理",
    "",
    "她也有著自己的傷悲",
    "就是一般市面　唇膏不夠美",
  ],
  [
    "我食了第十支煙",
    "感覺是多麼的苦澀",
    "主要是新買的煙頭是天然",
    "為什麼要這麼天然？",
    "",
    "我遇見某種女子",
    "某種女子是多麼瘋癲",
    "瘋癲到　我也不想再痴纏",
    "為什麼要這麼痴纏？",
    "",
    "每個下午　我都在食煙",
    "但我沒有忘記　我是一個 rapper",
    "",
    "Yo check it out",
    "I wanna tell you a story",
    "About how the girls fool me",
    "Everybody give me a beat",
    "",
    "有晚喺公園坐　有個女仔行近",
    "想我教佢結他　問我肯唔肯",
    "我好認真　打量咗佢全身",
    "就話結他嚟講　其實我都仲學緊",
    "",
    "又有晚同個女仔好開心",
    "去到海邊傾偈　又有酒飲",
    "一流嘅情調同氣氛",
    "但就係可惜嗰晚　原來佢 m come",
    "",
    "If you go to the after party",
    "We can be company",
    "I know you only like my personality",
    "But you will never like my body",
    "You only like my personality",
    "But not my body",
    "Not my body",
    "",
    "我遇見某種女子",
    "想起我　決心食煙的初次",
    "因我不小心走進朋友討論的故事",
    "便要食支煙　轉身去出面",
    "",
    "離席　抽一支煙",
    "離席　抽一支煙",
    "離席　抽一支煙",
    "離席　抽一支煙",
  ],
  [
    "昨晚夢見你死了",
    "怨恨在懷裡消失了",
    "看著天空變白了",
    "雨下呀　燕子飛過來了",
    "",
    "細個半夜去公園坐",
    "大門原來是鎖緊咗",
    "年輕沒人可以管我",
    "笑笑一起便闖過",
    "",
    "你我沒有見很久了",
    "那年聖誕你失戀了",
    "你不面對　我放棄了",
    "心裡暗暗生氣了",
    "",
    "請原諒我",
    "年輕的我　不懂生命很多",
    "如果　一切就是最好的安排",
    "你當年難道有錯麼？",
    "",
    "請明白我",
    "無知的我　愛你很多",
    "如果　一切可以重頭來過",
    "無論怎樣過　只管盡情過",
    "",
    "昨晚夢見你死了",
    "一切都不緊要了",
    "雨白天空變清了",
    "數十燕子也歸家去了",
  ],
  [
    "我的煩惱不是愛人對我不夠真",
    "亦不是她對我流露蔑視的眼神",
    "No, non, いいえ",
    "不是朋友逐一結婚",
    "而是今天沒大麻在身",
    "而是今天沒大麻在身",
    "",
    "我的煩惱不是沒有照顧好父親",
    "亦不是看見他變成衰老的人",
    "No, non, いいえ",
    "不是憂慮無處棲身",
    "而是今天沒大麻在身",
    "而是今天沒大麻在身",
    "",
    "No, non, いいえ",
    "不是我沒變 gentleman",
    "No, non, いいえ",
    "不是沒學好法文",
    "No, non, いいえ",
    "不是選舉不知投哪人",
    "而是今天沒大麻在身",
    "",
    "我的煩惱不是每日起身就眼瞓",
    "亦不是想找人瞓已沒有人肯",
    "No, non, いいえ",
    "不是愛上朋友的女人",
    "而是今天沒大麻在身",
    "而是今天沒大麻在身",
    "而是今天沒大麻在身",
  ],
  null,
  [
    "這晚的一切是夢還是真",
    "像上天給我一晚的回贈",
    "想過一千遍重遇在電影中心",
    "終於等到這晚來臨",
    "",
    "你怕過了今晚我會更不開心",
    "但我說反正已夠不幸",
    "看你 IG 看到包括你所有親朋",
    "你說看我朋友看到像識了本人",
    "",
    "回家的一路上你笑笑的問",
    "猜你手機密碼去年的更新",
    "我並沒有猜中　然後過了一陣",
    "你按下是我的生辰",
    "",
    "偶爾也會想那些年的光陰",
    "為什麼他會從不過問",
    "或是他已知道有我這一個人",
    "就像那套《Age of Innocence》",
    "",
    "今天見你打火機遺留在我身",
    "像告訴我昨晚的都是真",
  ],
  [
    "多麼不想都已是個成人啊",
    "怎會帶女伴兩日一夜長洲消磨",
    "只有學生時期才這樣傻",
    "再去就像留班的中學課",
    "",
    "這個小島再不會發生什麼",
    "三輪單車與沙灘已把下午消磨",
    "晚飯後只不過是七點多",
    "還有一大個晚上怎麼過",
    "",
    "當初和誰到這球場看踢波",
    "現在眼前一班新青年燒烤唱歌",
    "他們到某年或有同樣痛楚",
    "今晚海風怎令人不難過",
    "",
    "三輪單車可帶我倆離開麼",
    "時間留在過去但又變了很多",
    "對上一次都問過到這裡是為何",
    "罰抄一千次還會再錯",
    "",
    "我以後不會到長洲過",
    "我以後不會到長洲過",
    "我以後不會到長洲過",
    "我以後不會到長洲過",
  ],
  [
    "今夜到干諾道中一起瞓",
    "這是我最可負擔的租金",
    "以後會否改變都不要問",
    "起碼這夜你我同行",
    "",
    "明天到登打士街那邊瞓",
    "明知半夜或會被嗌起身",
    "到處也是睡醒了的人",
    "只怕洗手間不太近",
    "",
    "失眠只為路燈",
    "防暴使你我心更近",
    "大會的咪可否細聲一陣？",
    "我聽不到愛人的聲音",
    "",
    "我將記得面前黑社會的佈陣",
    "曾經一晚煙霧使我看你更真",
    "還有龍和道站最前的女生",
    "草地跑過的腳印",
    "",
    "對不起這位市民",
    "我知我阻礙你　很不忿",
    "但我也是為這下一代人",
    "我也無愧於心",
    "",
    "明朝要清場我或會走人",
    "然後　晚上又來瞓",
  ],
  [
    "生於這一個年代",
    "沒有無緣無故的愛",
    "到處都佈滿了迫害",
    "無錢就等被劏開",
    "",
    "我要屌屌屌屌屌撚到你反西",
    "今晚你嚟唔嚟",
    "我要屌屌屌屌屌撚到你反西",
    "你無撚得話唔制",
    "",
    "邊撚個食咗我盒飯啲餸菜",
    "仲要我講聲唔該",
    "等我射返落你個菠蘿蓋",
    "射出我一生嘅悲哀",
    "",
    "今晚我的水哥　將要和嚴經理決戰",
    "這些年來　他受到的對待使人無法忍耐",
    "為了我們的將來　大家不要繼續托腮",
    "團結一致　屌佢老母臭化閪",
    "",
    "我要屌屌屌屌屌撚到你反西",
    "今晚你 嚟唔嚟（屌你個西）",
    "我要屌屌屌屌屌撚到你反西",
    "你無撚得話唔制",
    "",
    "我要屌屌屌屌屌撚到你反西",
    "今晚你 嚟唔嚟（屌你個西）",
    "我要屌屌屌屌屌撚到你反西",
    "你無撚得話唔制",
  ]
]

const a9list = {
  og: [
    "阿姆斯特丹夜機",
    "你說之後會找我",
    "一六八",
    "再殺一個人",
    "沖繩流浪貓",
    "百份之一機會",
    "彌敦道的一晚 good trip",
    "HEY HEY BABY",
    "你叫我譯一首德國歌詞",
    "心",
    "西湖沒有中秋",
    "以為和你熟",
  ],
  en: [
    "Night Flight to Amsterdam",
    "You Said We'd Be Back",
    "168",
    "Killed Off One More",
    "Okinawa Stray Cat",
    "One in a Hundred",
    "A Night of Good Trip Along Nathan Road",
    "HEY HEY BABY",
    "You Asked Me to Translate a German Song",
    "Heart",
    "No Mid-Autumn On West Lake",
    "Thought We Were Close"
  ]
}

const a9meta = [
  [
    2,12,
    'P', 'P', 'P,阿賢,阿科,銘銘', 'N', null
  ],
  [
    3,0,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,35,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫,銘銘', 'N', null
  ],
  [
    3,32,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫,Kit @ 22cats', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,39,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫', 'N', null
  ],
  [
    1,43,
    'Gustav Mahler', 'P', 'P,阿賢,阿科,夏日浪漫', null, {
      recite: 'N'
    }
  ],
  [
    2,57,
    'P', 'P', 'P,四方果', 'N', null
  ],
  [
    2,19,
    '陳輝陽', 'P', 'P,阿賢,阿科,銘銘', 'P,N', null
  ],
  [
    3,21,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫,銘銘', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,32,
    'N', 'N', 'P,N,阿賢,阿科,夏日浪漫,銘銘', 'N', null
  ],
  [
    2,7,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫', 'N', null
  ],
  [
    2,27,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫', 'N', {
      backup_vocal: 'Jackie'
    }
  ]
]

const a9lyrics = [
  [
    "七月十五號凌晨",
    "第一次去阿姆斯特丹",
    "我想快啲瞓著",
    "但兩點幾嗰陣",
    "突然有飛機餐食",
    "全機人都食緊",
    "好難一個人唔食",
    "於是我集中精神望窗外面",
    "當時經過緊個地方",
    "有好多一條條白色螢光嘅馬路",
    "中間有啲紅色點",
    "成個地面得兩隻色",
    "好似好科幻咁",
    "喺唔知邊度",
    "",
    "我諗起小學有次交功課",
    "題目係當父母唔俾自己睇電視",
    "作為子女應該點",
    "我寫「我會看窗外鄰家的電視」",
    "「雖然沒有聲音但好過沒有得看」",
    "結果班主任叫我對全班讀一次",
    "羞辱我",
    "但佢羞辱唔到我",
    "我細個係無感覺嘅人",
    "",
    "嗰日七月十四號",
    "我先轉機去德國　再去冰島",
    "我飲晒杯好難飲嘅白酒",
    "但都瞓唔著",
    "",
    "呢幾年我住厭咗土瓜灣",
    "有時搭小巴返去",
    "我特登唔叫落車",
    "等其他人嗌",
    "我先落車",
    "睇下會行幾遠路",
    "有次去到九龍城都未有人落",
    "我先忍唔住叫有落",
    "有時除咗買嘢食",
    "我可以全日唔駛講嘢",
    "",
    "去完旅行我會搬去深水埗",
    "深水埗有啲拎住籠雀",
    "戴住老夫子嘅帽",
    "好似清朝嘅人",
    "",
    "嗰日七月十四號",
    "上機前我上咗一陣網",
    "我已經有愛人",
    "香港無我嘅事",
  ],
  [
    "你在哪裡　我竟不知",
    "枉我們曾每日都相對",
    "你想著誰　為何流眼淚",
    "我竟也不再問一句",
    "",
    "呼呼呼呼　呼呼呼呼",
    "像風一樣的年歲",
    "呼呼呼呼　呼呼呼呼",
    "已不知被吹到哪裡",
    "",
    "我在哪裡　我也不知",
    "像被迫進了一個監獄裡",
    "只有過去　已沒能再聚",
    "目睹最美的事粉碎",
    "",
    "呼呼呼呼　呼呼呼呼",
    "像風一樣的年歲",
    "呼呼呼呼　呼呼呼呼",
    "已不知被吹到哪裡",
    "",
    "一生人很短",
    "但終點很遠",
    "任年月讓我再大多幾歲",
    "任年月伴我繼續老去",
  ],
  [
    "在嚴重喝醉酒的時候",
    "我才知道",
    "這年來我得罪過的親友",
    "其實是很有理由",
    "我來自一戰以後",
    "在最近喝醉酒的時侯",
    "還第一次感到有種愛在裡頭",
    "",
    "每天起床的時候",
    "都會有一句 stay happy 在響鬧裡頭",
    "有時都覺得絕望到不能再有",
    "還好我的仇家",
    "他比我更加荒謬",
    "我時常想著他",
    "作最真誠的詛咒",
    "",
    "午飯後在街上走",
    "走進裕民中心舊商場裡頭",
    "發現遊戲機中心",
    "於是入去玩個夠",
    "《雙截龍》是童年喜愛的打鬥",
    "為何一點感覺都沒有",
    "我想起初中 lunch hour 的時候",
    "",
    "那時的太陽和今天一樣",
    "讓我裇衫都濕透",
    "那年我開始想知",
    "世界為何還未到盡頭",
    "那年我開始想知",
    "世界為何還未到盡頭",
    "那年我開始想知",
    "世界為何還未到盡頭",
    "那年我開始想知",
    "世界為何還未到盡頭",
    "那年我開始想知",
    "世界為何還未到盡頭",
    "那年我開始想知",
    "世界為何還未到盡頭",
    "那年我開始想知",
    "世界為何還未到盡頭",
  ],
  [
    "有人話世間所有感情最終都會走向末路",
    "最好可以死喺三十七歲之前或者更早",
    "有理想嘅文青到某個階段開始變得深奧",
    "簡單嘅字句要有學術字眼先表達到",
    "每個作家都覺得自己嘅作品最純粹最樸素",
    "市面賣得好嘅一般都有危害社會嘅元素",
    "無人會諗自己嘅才能係咪需要進步",
    "但總會覺得自己嘅地位有待提高",
    "",
    "尤其是你為什麼可以這樣做",
    "你都知道我曾對你很好",
    "但我從不會表現憤怒",
    "某天先知道碰到你眼也不想一掃",
    "",
    "我想生活喺一千年前嗰陣時未有電腦",
    "唯一唔捨得嘅係冬天嘅時候無熱水爐",
    "近年我嘅生活開始無咁刺激無咁虛無",
    "以前深夜我會打俾人講我殺咗人一時糊塗",
    "但第日完全唔記得做乜我會咁做",
    "我懷念嗰陣時每日每夜都好 emo",
    "你係我嘅青春期永遠唔會再接觸到",
    "兩個人就咁永遠疏離都無話好定唔好",
    "",
    "尤其是你為什麼可以這樣做",
    "你都知道我曾對你很好",
    "但我從不會表現憤怒",
    "某天先知道碰到你眼也不想一掃",
  ],
  [
    "在沖繩黃昏後街上",
    "遇見隻貓 BB 神情憂傷",
    "慢慢走近到我身上",
    "好幾次把我差點搲傷",
    "",
    "我不曾遇過貓這樣",
    "玩了半天一直從後跟上",
    "最終走了睡前回想",
    "才確定那一刻你想我養",
    "",
    "但你不會講出所想",
    "令這悲傷一直生長",
    "見我決心走的模樣",
    "便轉身聞路邊花香",
    "",
    "你不曾像人類那樣",
    "習慣要解釋自己在網上",
    "講出了抵消一切哀傷",
    "而我都無法對你講出所想：",
    "「我是這裡的遊客家在異鄉」",
    "「你是唯一的貓我想過養」",
  ],
  [
    "所有呢度嘅禮物都唔值咁貴",
    "但因為係俾你所以都係抵",
    "你係部電影分唔到商業定文藝",
    "每次都咁刺激又一直咁襟睇",
    "",
    "世界上面已經無人令我著迷",
    "但我連魔性都俾你引發出嚟",
    "你問我有無約食飯同其他女仔",
    "我話有百份一機會令你唔開心我都唔制",
  ],
  [
    "每到晚上六七點",
    "我先會開始 weed",
    "再走上一架巴士",
    "坐上層的最前",
    "",
    "坐到去油麻地",
    "其中一個巴士站",
    "有架１號Ａ停咗喺前面",
    "嗰架巴士上層最後一行",
    "坐咗對幾靚嘅男女",
    "不停咁錫嚟錫去",
    "喺我面前",
    "",
    "兩個眯埋眼由咀錫到落條頸",
    "停咗幾秒　又不停咁錫",
    "佢哋前面無其他人",
    "我架巴士前排又得我一個人",
    "睇到有啲唔好意思",
    "所以有啲特別開心",
    "",
    "兩架巴士一路向前",
    "有時佢地行快啲會唔見一陣",
    "但去到燈位又貼返喺正前面",
    "好似我架巴士同佢哋架巴士",
    "喺彌敦道上面",
    "錫下停下的纏綿",
    "",
    "每到晚上六七點",
    "我先會開始 weed",
    "再走上一架巴士",
    "坐上層的最前",
    "",
    "喺土瓜灣出發",
    "搭６號Ｃ去朗豪坊睇戲",
    "呢段路我已經行到好熟",
    "燒賣　雪糕　迎面嘅人　恐怖片",
    "smoke咗之後　我吸晒",
    "所有的臉",
    "",
    "搭搭下我突然間記返起",
    "喺我幾歲嘅時候",
    "香港曾經有過一兩單交通意外",
    "坐巴士上層最前嘅乘客被撞死",
    "自始之後",
    "呢個位就俾啲大人講到好危險",
    "三十年嚟",
    "我好似失去咗理性",
    "每次上到上層都會自動坐後面",
    "直到 smoke 咗之後",
    "先揀最好嘅風景",
    "放在面前",
  ],
  [
    "有次同你搭長途巴士去瑞典",
    "巴士查飛嘅人問我另一半喺邊？",
    "當時我指住你",
    "你排緊喺我後面",
    "然後查飛嘅人話唔係",
    "係張飛嘅另一半喺邊",
    "",
    "手機裡螢幕放著是你照片",
    "當要看時間便看見你視線",
    "",
    "有晚開始傾極電話都唔收線",
    "然後每晚同你 video chat 到三點",
    "你話唔想瞓住",
    "驚瞓咗之後無得再見",
    "但我一眯埋眼只見到你的臉",
    "",
    "手機裡螢幕放著是你照片",
    "當要看時間便看見你視線",
    "手機裡螢幕放著是你照片",
    "當要看時間我便會碰電",
  ],
  [
    "你叫我幫你譯一首德國歌詞",
    "並沒有說是為什麼事",
    "那首歌說向外面世界奔馳",
    "渴望那一天將至",
    "",
    "祈求風和雨",
    "吹我到理想的遠處",
    "故土沒法跟隨我意願",
    "但為什麼終於",
    "穿過海灣來到老遠",
    "卻很想返回我的屋邨",
    "",
    "你叫我幫你譯一首德國歌詞",
    "像在訴說著你心願",
    "拍拖　出國　交友　工作　生兒",
    "難道不是為逃避寂寞不斷？",
    "",
    "祈求風和雨",
    "吹我返故鄉的某處",
    "異國沒法消除我睏倦",
    "但為什麼終於",
    "返到故鄉鞋也未轉",
    "卻很想離去我的屋邨",
    "但為什麼終於",
    "穿過海灣來到老遠",
    "卻很想返回我的屋邨",
  ],
  [
    "分手說再見也不易",
    "瀟灑的人也有流淚時",
    "明明知道　時候到了要變異",
    "突然消失了　擁抱都不要",
    "",
    "分手到底為什麼事",
    "是誰做錯令你我遲疑",
    "明明知道　無常總會有",
    "來的時候　說放手誰又可以？",
    "",
    "我把心打開　照星星月亮",
    "讓痛處眼淚盡流",
    "還望過去　你我沒有負累",
    "學會適當時要爭取",
    "",
    "我把心打開　照星星月亮",
    "讓痛處眼淚盡流",
    "祈願每個此刻　我更懂我",
    "重拾熱情像最初",
    "",
    "祈願每個此刻　我更懂我",
    "就算變幻很多",
    "",
    "願我心可放鬆過",
    "從今天當摯真的我"
  ],
  [
    "在這中秋的夜景",
    "湖面平靜",
    "與你去看滿月的倒影",
    "不為天上的星星",
    "不為手中的燈影",
    "聽你腳步的聲最動聽",
    "",
    "但這中秋的夜景",
    "為何這麼冷清",
    "並沒看見別的燈影",
    "只剩天上的星星",
    "不被看見的遠景",
    "照不清暗處的路徑",
  ],
  [
    "你替我選的那古著牛仔衣服",
    "是要讓我看來不太毒",
    "我卻每天聞起來想你才舒服",
    "但又怕聞到會麻木",
    "",
    "聽說世間所有情愛的事",
    "不過是浪漫的想像所致",
    "那我已知苦痛只不過是",
    "因我太夢幻的心智",
    "",
    "也說不出這算熟還是不熟",
    "能對望幾秒眉目",
    "但你眼中根本就從不算熟",
    "就算已訴盡我感觸",
    "",
    "聽說世間所有情愛的事",
    "不過是浪漫的想像所致",
    "與你那天做的每一件事",
    "也使我回味千百次",
    "",
    "與你那天做的每一件事",
    "也使我回味千百次",
    "每天也期待下一次",
  ]
]

const a10list = {
  og: [
    "詩歌舞街",
    "下了兩天雨",
    "喫煙席",
    "每次你走的時分",
    "因講了出來",
    "QUEEN",
    "宋代 SAD BOY",
    "20GB",
    "SABINA 之淚",
    "只收一半",
    "散步之年",
    "那陣時不知道",
    "今夜雪糕",
    "Ｋ同學",
    "煙",
    "燕",
  ],
  en: [
    "Sycamore Street",
    "Rained for Two Days",
    "Smoking Seat",
    "Every Time You Left",
    "When I Let It Out",
    "QUEEN",
    "Song Dynasty's SAD BOY",
    "20GB",
    "SABINA'S TEARS",
    "Just for Half Price",
    "Year of Strolling",
    "Didn't Know Then",
    "Ice Cream Tonight",
    "Classmate K",
    "Gas",
    "Swallow",
  ]
}

const a10meta = [
  [
    3,11,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,52,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,6,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    3,3,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,41,
    'P', 'P', 'P,蕭暢悅', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,43,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,53,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,57,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,20,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,18,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    3,18,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,27,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,27,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    3,0,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,30,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,1,
    'P', 'P', 'P', 'N', null
  ]
]

const a10lyrics = [
  [
    "那晚看 Russian Red 大角咀表演",
    "你與我竟會再遇見",
    "剛好我也到 bar 枱買酒才碰面",
    "完場與你去深水埗那邊",
    "剛過身的作家你介紹那年",
    "寫過只有散步我們才真正聊天",
    "",
    "再次聽你說故事",
    "聽你說剛生的嬰兒",
    "詩歌舞街地上有著光點閃閃",
    "聽你說外地歷險",
    "我卻像當天的少年",
    "但已是能接受你所有改變",
    "",
    "你說你最近愛聽古典",
    "你說你父母開始婚變",
    "邊聽邊講我忘了怎去 99 咖啡店",
    "說到我倆澳門過生日那年",
    "你為我訂了山上的酒店",
    "餐廳老闆對你彈唱結他一整天",
    "",
    "再次聽你說故事",
    "聽你說剛生的嬰兒",
    "走進萬發坐在暗光卡位窗邊",
    "聽你說外地歷險",
    "我卻像當天的少年",
    "但已是能控制我所有淚腺"
  ],
  [
    "像是太久不見",
    "久得不太自然",
    "像是某種訓練",
    "又過了數十天",
    "呀……",
    "久得不太自然",
    "呀……",
    "",
    "或是這種陰天",
    "令這愁緒浮面",
    "發現太深的掛念",
    "那是無可避免",
    "呀……",
    "那是無可避免",
    "呀……",
    "",
    "若我知你不想見",
    "像我那樣多",
    "那又有何不可？",
    "若你不知不覺生厭",
    "也未嘗不過 (不過）",
    "是最好的結果",
    "痛苦只我一個（一個）",
    "呀……",
    "痛苦只我一個（一個）",
    "呀……",
    "那又有何不可？",
    "那又有何不可？"
  ],
  [
    "一直知道更多",
    "令你一直失去更多",
    "真相告訴給你",
    "你因此不信我",
    "",
    "再會了那些從前片段",
    "你我不過見過數面",
    "",
    "一段日子已過",
    "再也沒去做過什麼",
    "不去工作不去拍拖",
    "不吸煙不吃水果",
    "",
    "四周也日麗風和",
    "卻有雨一直灑向我",
    "漫步到最遠咖啡座",
    "褪黑素幾晚沒效果",
    "",
    "再會了那些從前故事",
    "你亦不要太過懷念",
    "",
    "生命像一首老歌",
    "重複著某一樣痛楚",
    "Que será, será",
    "年老但仍未停播"
  ],
  [
    "每次到了你要走的時分",
    "我的心都會痛一陣",
    "總想有人會對你更貼心",
    "但又怕你不再接近",
    "",
    "世上總有人純粹而聰敏",
    "能夠明瞭這情感",
    "盼望你能被照顧得開心",
    "遙距與我共度餘生",
    "",
    "愛就似條河水般支分",
    "不由得流向他人",
    "這種抽離和這種痴心",
    "一秒即永恆（不留傷痕）",
    "",
    "世上總有人純粹而聰敏",
    "能夠明瞭這情感",
    "盼望你能被照顧得開心",
    "遙距與我共度餘生",
    "",
    "每次約會前",
    "我都先打掃房間",
    "不知這次你會否上來",
    "小心翼翼",
    "不留低之前任何人的髮根",
    "",
    "每次到了你要走的時分",
    "我的心都會痛一陣",
    "每次到了你要走的時分",
    "我的心都會痛一陣（不留傷痕）",
    "",
    "每次到了你要走的時分",
    "我的心都會痛一陣",
    "每次到了你要走的時分",
    "我的心都會痛一陣（不留傷痕）",
    "（不留傷痕）"
  ],
  [
    "因講了出來",
    "便會失去吸引力",
    "失去機會被愛",
    "因講了出來",
    "便會失去所有",
    "想像中的可愛",
    "",
    "寧願永遠",
    "永遠停留在",
    "沒太多往來",
    "不在時間上慷慨",
    "寧願永遠",
    "永遠停留在",
    "想像之內",
    "免去細節的傷害",
    "",
    "就這樣　在午夜　在馬路",
    "在網上　在購物　在腦內　就這樣愛",
    "就這樣　在被內　在散步",
    "在午睡　在吃飯　在發夢　就這樣愛",
    "和你去發展愛",
    "",
    "保持三兩日嘅 message",
    "報告睇過嘅電影",
    "交換一兩句影評",
    "間中一張食物照",
    "好睇嘅副刊 link",
    "你有冇去過淺水灣呢間露台餐廳？",
    "我喺日本嘅和歌展覽",
    "見到啲好短好脆弱嘅古詩",
    "例如有首係：",
    "「只因你說了一句『馬上就來』／",
    "　我在九月漫長的夜晚苦苦等待／",
    "　等到清晨月影浮出天外」",
    "又例如：",
    "「本來只有千年之齡的松樹／",
    "　今天被你的手一掂／",
    "　想必長活萬年」",
    "離開展覽前",
    "我都喺留言簿寫咗首：",
    "「不敢看到美好的詩句／",
    "　怕用手機影低時／",
    "　被保安捉去」"
  ],
  [
    "I have named you queen",
    "There are taller than you, taller",
    "There are purer than you, purer",
    "There are lovelier than you, lovelier",
    "But you are the queen",
    "",
    "我也喜歡被你這樣拒絕我",
    "我也喜歡被這種感覺折磨",
    "坐上情感過山車後座",
    "安然已不屬我（已不屬我）",
    "",
    "我是否應該找較平凡的那個",
    "但他不會給我甜甜的折磨",
    "每次鈴聲響起是你麼",
    "失去理智也不錯",
    "",
    "當你每次一在場",
    "體內似有河水聲響",
    "方圓十里植物突然生長",
    "溫柔包圍著我倆",
    "",
    "每次見面後",
    "我都會踩很久的單車",
    "走很多路",
    "你身上的味道飄來飄去",
    "昨晚之後",
    "全日又有很多香味",
    "",
    "但你始終未肯講再見不見我",
    "就讓我在這階段好好消磨",
    "往後如何也不再會這樣傻",
    "隨意歡喜略過",
    "",
    "當你每次一在場",
    "體內似有河水聲響",
    "方圓十里植物突然生長",
    "溫柔包圍著我倆"
  ],
  [
    "大千世界裡　我撇低你",
    "我撇低你　但又常想起",
    "大千世界裡　碰巧跟你",
    "就算只有　那一晚驚喜",
    "",
    "何時苦痛　何時便抽離",
    "平靜風雨裡　曾遇你",
    "",
    "大千世界裡　肉體紛飛",
    "情慾仿似　駕駛穿梭機",
    "大千世界裡　是他也是你",
    "曾被刺痛　隨時被勾起",
    "",
    "從前心跳　從未願抽離",
    "沿路風雨裡　誰是你",
    "",
    "平靜風雨裡　曾遇你",
    "",
    "千山同一月",
    "萬戶盡皆春",
    "千江有水千江月",
    "萬里無雲萬里天"
  ],
  [
    "如不是 Flickr 再一加價",
    "我也不會把八千張照片拿下",
    "迫使我重溫四年這段關係變化",
    "第一張相喺黃大仙開始",
    "你的眼耳口鼻",
    "是我生平遇過的最美",
    "想把一切都給你",
    "不設限期",
    "",
    "時光",
    "把各人的青春玩弄",
    "20GB 的重",
    "都不過是愛情的灰燼",
    "證實如何燒掉每張笑容",
    "",
    "這四年像是四十年",
    "要忘掉也是不容易",
    "",
    "黃大仙之後我話寄嘢俾你",
    "問你地址",
    "過兩日我貼咗張郵票",
    "喺戴住嘅冷帽上面",
    "去到你屋企門口",
    "我話我係嗰份郵件",
    "",
    "還有一些冬天郊區的照片",
    "第一次吃你煮的撈麵",
    "深夜投放電影在牆上過年",
    "",
    "時光",
    "把我們的青春玩弄",
    "20GB 的重",
    "都不過是愛情的灰燼",
    "證實如何燒掉每張笑容",
    "",
    "再來一次也不是那回事",
    "再來一次也不是那回事"
  ],
  [
    "應該早知道",
    "根本就不是",
    "應該早知道",
    "根本不會是",
    "對方沒任何表示",
    "純粹是沒有意思",
    "難道你認為是他不敢示意",
    "",
    "應該早知道（應該早知道）",
    "根本就不是（根本就不是）",
    "應該早知道（應該早知道）",
    "根本不會是（根本不會是）",
    "對方沒任何表示",
    "沉默像深海帶子",
    "情慾會令人突然喪失睿智",
    "",
    "心已經被佢偷走幾次",
    "誤解晒每個 post 每個 story",
    "過盡千帆皆不是",
    "根本冇個 post 係關你事",
    "",
    "但要逃脫亦不易",
    "繼續前行亦沒意思",
    "魂斷維也納他都不會在意",
    "",
    "應該早知道（根本就）",
    "應該早知道（應該早知道）"
  ],
  [
    "有次我咪同你講話",
    "我喺 IG 搵 PTGF 嘅",
    "有個覆返我話",
    "三千蚊一個鐘",
    "包 wet kiss, bj, ml",
    "同埋 bath tgt",
    "我有問你抵唔抵㗎嘛",
    "同埋好唔好每個星期都 call?",
    "其實呢  我係諗住你會話：",
    "",
    "「我只收你一半」",
    "「我也感到很悶」",
    "「我也想找個伴」",
    "「把這假期填滿」",
    "",
    "但係你都冇咁樣講",
    "你叫我快啲 call",
    "刺激下本地經濟",
    "我仲諗住",
    "一齊煮意粉食",
    "睇啲好耐冇睇嘅戲",
    "食雲呢拿味雪糕溝啲咖啡入去",
    "第二朝再煮早餐俾你",
    "（錢嗰波一你要 (?)，我無所謂㗎）",
    "（錢個幫胡 (?) 就得啦，吓）",
    "但係你都冇咁樣講：",
    "",
    "「我只收你一半」",
    "「我也感到很悶」",
    "「我也想找個伴」",
    "「把這宇宙填滿」",
    "（嚟吖，幫吓手喇，吓）",
    "",
    "後尾我過咗一半訂俾個 PTGF",
    "佢覆返我話等一陣",
    "同一個微笑嘅 emoji 之後",
    "就再搵唔到佢喇",
    "仆佢個臭街　頂佢個肺"
  ],
  [
    "靜靜過這天",
    "慢慢度這年",
    "沒有要去邊",
    "這是最好一年",
    "從沒走進這路線",
    "這是我剛剛發現",
    "某一塊肌肉像似",
    "得到首次鍛鍊",
    "",
    "有冇一種可能係",
    "咁啱行緊嘅呢一段",
    "係喺歷史嘅中段",
    "終點係喺好後面",
    "你係唔會見到",
    "但最終係會出現",
    "",
    "每晚看舊影片",
    "伊力盧馬冬天故事",
    "吃盡各種米線",
    "那是我的自由意志",
    "僅此而已",
    "還可以如此",
    "僅此而已",
    "還可以如此",
    "",
    "也會時常掛念那時",
    "掛念世界與你未病發前",
    "與你去到泰國夜市",
    "去到了一個攤檔街邊",
    "（那時）",
    "",
    "你話想買件飾物穿耳（那時）",
    "我問個檔主可唔可以平啲（那時）",
    "我見佢勉為其難咁話可以（那時）",
    "找錢嗰時我話唔駛",
    "我都係俾原本價錢（那時）",
    "你說你很有面子（那時）",
    "因我今晚的善意（那時）",
    "你說你很有面子（那時）",
    "因我今晚的善意（那時）",
    "（那時）",
    "你說你很有面子（那時）",
    "你說你很有面子（那時）",
  ],
  [
    "如像九十年代初",
    "照片裡海灘的印象",
    "那種下午沒有重量",
    "帶點迷惘奔向夕陽",
    "",
    "那年獨自去南洋",
    "一杯白咖啡的早上",
    "四周是可冒險方向",
    "尤加利樹葉的香",
    "",
    "那陣時不知道",
    "置身的日子都發亮",
    "眼光裡藏著的囂張",
    "往後已不再同樣",
    "",
    "圖書館木製的窗",
    "訓導假裝嚴肅的模樣",
    "還未有傾訴對象",
    "仍未覺得悲涼",
    "",
    "那陣時不知道",
    "每天在風光的戰場",
    "騎著馬去到邊疆",
    "所到之地全是賜賞",
    "",
    "Hoo 寂靜但不漫長",
    "Hoo 寂靜但不漫長",
    "Hoo 寂靜但不漫長",
    "Hoo 寂靜但不漫長",
    "La la la la la",
    "La la la la la",
    "La la la la la（那陣時不知道）"
  ],
  [
    "何も言うことはない",
    "Non ho niente da dire",
    "Ich habe nichts zu sagen",
    "No tengo nada que decir",
    "Je n'ai rien à dire",
    "",
    "有時很懊惱",
    "有時也感到驕傲",
    "有時不後悔曾這樣表達憤怒",
    "",
    "今夜我可以吃更多雪糕",
    "想像明天中午會被捕",
    "",
    "還有些心願未做",
    "還未去埃及希臘秘魯",
    "還未跟你曾相處得更好",
    "",
    "外面進行著的夜",
    "無窮的遠方",
    "無數的人們",
    "都和我有關",
    "",
    "今夜我可以走八千里路",
    "想像明天會特赦全部",
    "今夜我可以",
    "想像明天中午"
  ],
  [
    "如果可以重來　到太子午夜時候",
    "便會拖著你手不會似中學生那麼害羞",
    "你愛將警察當做背景自拍十張才夠",
    "路過的夫婦以為你來自韓國到此旅遊",
    "",
    "士多老細收錯錢說幾晚不夠人手",
    "酒吧街邊吧女拚命遊說我們飲啤酒",
    "",
    "望向星星是哪星球　它突然間飄走",
    "懷疑是航拍後　你竟除下口罩向它揮手",
    "問我想有什麼成就　我本不想活太久",
    "何以見到你後　我竟然會習武鍛鍊身手",
    "",
    "放狗的途人照舊　牠擒向另一女生膝頭",
    "圍觀到防暴對峙時也行近看下又走",
    "女生說家有三隻母狗　我說她那味散四周",
    "群眾都起哄話我　還要是你帶頭",
    "",
    "著拖鞋紅衫短袖　手執燒賣配清酒",
    "願這街坊裝被捕可免受暴動罪追究",
    "",
    "望向星星是哪星球　它突然間飄走",
    "懷疑是航拍後　你竟除下口罩向它揮手",
    "問我想有什麼成就　我本不想活太久",
    "何以這晚之後　我竟想去習武保衛地球",
    "",
    "何以見到你後　我竟然會習武鍛鍊身手"
  ],
  [
    "你說你不想遠離",
    "煙霧的滋味",
    "已看不到距離",
    "勝利的日期",
    "朦朧的希冀",
    "時代的奧秘",
    "",
    "你看到煙已霧起",
    "靈魂已準備",
    "誰願有這勇氣",
    "與肉體分離",
    "無謂的黑旗",
    "窒息的空氣",
    "",
    "已想像得到結尾",
    "熟悉的攻鼻",
    "茶餐廳電視機",
    "顛倒各樣是非",
    "幾代人也不理",
    "罪名卻給了你"
  ],
  [
    "海燕　寂寞令感覺都被騙",
    "你我沒太多的信念",
    "誰願為愛放棄能自由往返天邊",
    "",
    "海燕　靜默是今晚的盛宴",
    "從前有太多的纏綿",
    "純為限定你我能共誰喝酒吸煙",
    "",
    "預計不再相見",
    "過不到這試驗",
    "瘟疫正在蔓延",
    "誰有露台表演",
    "",
    "海燕　就讓日子給我沉澱",
    "京都之夜竟未發現",
    "日後重遊鴨川小店",
    "也許會很懷緬"
  ],
]

const a13list = {
  og: [
    "細胞",
    "某夜後台",
    "循環的夜",
    "LUNCH",
    "德州之戀",
    "你把我的",
    "嘔吐",
    "八點零五分",
    "我不適合聚會",
    "不要把冬天衫放回衣櫃",
  ],
  en: [
    "Cell",
    "Backstage Some Night",
    "Repeating Nights",
    "LUNCH",
    "Texas Romance",
    "You Take My",
    "Nausea",
    "08:05",
    "No Gathering for Me",
    "Don't Put Winter Clothes in the Closet",
  ]
}

const a13meta = [
  [
    2,30,
    'P', 'P', 'P,阿賢,阿科', 'N', null
  ],
  [
    2,54,
    'P', 'P', 'P,阿賢,阿科,eloobe', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,25,
    'P', 'P', 'P,阿賢,阿科', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,55,
    'P', 'P', 'P,阿賢,阿科,eloobe', 'P', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,43,
    'P', 'P', 'P,阿賢,阿科,蕭暢悅', 'N', {
      backup_vocal: 'Karena'
    }
  ],
  [
    2,25,
    'P', 'P', 'P,阿賢,阿科', 'N', null
  ],
  [
    3,20,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫,eloobe', 'N', null
  ],
  [
    2,32,
    'P', 'P', 'P,阿賢,阿科,', 'N', {
      poem: "Penny",
      recite: "Penny",
    }
  ],
  [
    2,7,
    'P', 'P', 'P,阿賢,阿科,eloobe', 'N', {
      backup_vocal: 'P'
    }
  ],
  [
    2,35,
    'P', 'P', 'P,阿賢,阿科', 'N', null
  ]
]

const a13lyrics = [
  [
    "這是硬仗一場",
    "生無可戀卻意志堅強",
    "我擁有了一切",
    "同時遇上了魔鬼",
    "讓我的免疫力",
    "施展一場魔法",
    "我為所有細胞負責",
    "",
    "我感到他們",
    "像小貓呼喚我",
    "我要給他們",
    "溫柔的撫摸",
    "",
    "這裡是借來我知道",
    "還不知歸還日數",
    "絕對的舒服是最令人難受",
    "才令人接受憂愁和苦酒",
    "",
    "世間降了四度",
    "你在我神聖國度",
    "",
    "我愛我所有細胞",
    "對不起我沒告訴你",
    "會好的會好的會好的",
  ], [
    "這是硬仗一場",
    "生無可戀卻意志堅強",
    "我擁有了一切",
    "同時遇上了魔鬼",
    "讓我的免疫力",
    "施展一場魔法",
    "我為所有細胞負責",
    "",
    "我感到他們",
    "像小貓呼喚我",
    "我要給他們",
    "溫柔的撫摸",
    "",
    "這裡是借來我知道",
    "還不知歸還日數",
    "絕對的舒服是最令人難受",
    "才令人接受憂愁和苦酒",
    "",
    "世間降了四度",
    "你在我神聖國度",
    "",
    "我愛我所有細胞",
    "對不起我沒告訴你",
    "會好的會好的會好的",
  ], [
    "這樣的問題",
    "只可以是個謎",
    "若你要問到底",
    "把黑夜都摧毀",
    "",
    "「喜不喜歡我？」",
    "「可否拖著我？」",
    "真的需要直接回答麼？",
    "",
    "答案在風裡",
    "線索在夜裡",
    "眼睛不早已傳遞訊息一堆？",
    "",
    "最後的事情",
    "留待路線說明",
    "每步都在決定",
    "下一站的風景",
    "",
    "散步的風格",
    "古典的想法",
    "已置身到無辦法掌握",
    "",
    "海風吹的你",
    "不真實的你",
    "有點想接近 也許只是好奇",
    "",
    "啦啦啦啦啦",
    "有把聲音說",
    "早晚亦會遠離",
    "",
    "啦啦啦啦啦",
    "「你只不過是對他有點好奇」",
    "「你只不過是對他有點好奇」",
  ], [
    "太多廢話亂我心神",
    "寧願這裡繼續有病菌",
    "不想見人",
    "但你新工作在我家附近",
    "為了和你食 lunch 才打一針",
    "講笑　也有一分真",
    "",
    "我被困在某種雲層（某種雲層）",
    "四周也是霧",
    "看不到路中心",
    "算是人類共同命運",
    "",
    "四月份是最殘忍",
    "你怎不知道我點諗",
    "當然這是不可能",
    "不要望我雙眼　會暈",
    "",
    "I only look at people when I'm drunk",
    "",
    "通山都是路人",
    "先知道是禮拜",
    "這座山已淪陷",
    "可否叫蛋糕甜品",
    "我想要食氣氛",
    "",
    "啦啦啦啦",
    "啦啦啦",
    "啦啦啦啦",
    "",
    "文藝是一種毒品",
    "幸福是絕對不幸",
    "我只不過想跟你呻",
    "不要嘗試解答我疑問",
    "",
    "我會對你好到不能",
    "方便日後沒有遺憾",
    "昨夜睡得很深層",
    "錯過了深夜安全的地震",
    "",
    "I only look at people when I'm drunk",
  ], [
    "一想到你這樣遙遠",
    "說回來至少要等一年",
    "三百六十五天",
    "你在黑夜時我在白天",
    "",
    "不過你未說這是遙遠",
    "或我未能填補你每天？",
    "但我知人類情感",
    "是無從計算",
    "",
    "但我還是會等",
    "厭倦無目的覓尋",
    "回憶不應當是紀念品",
    "",
    "共你愈談愈深",
    "怎不會被逐漸吸引？",
    "你還不知你內心是乾淨得很",
    "",
    "（ 男：）可唔可以問吓你啲嘢？",
    "（ 女：）你講吖",
    "（ 男：）你諗住幾時結婚？",
    "（ 女：）我冇諗過要結婚喎 你有嘅咩？",
    "（ 男：）我都冇人同我結",
    "（ 女：）咁即係如果我同你結 你就即刻結？",
    "（ 男：）係呀 不過最好等一年",
    "（ 女：）哈哈 你唔想網上結婚",
    "（ 男：）係 見咗面先",
    "（ 女：）咁我都想等四十歲先",
    "（ 男：）但係如果計劃定咁遠 好似冇咗意思喎",
    "（ 女：）咁 一係2028年2月22號啦",
    "（ 男：）咁我依家mark低 開始儲錢",
    "",
    "但我仍是會等",
    "接近無限的覓尋",
    "回憶不應當是戰利品",
    "",
    "共你愈談愈深",
    "世間仿似定在某時份",
    "我們不知不覺間在討論婚姻",
  ], [
    "你把　我的　各種　訊息",
    "都也　給予　別個　看的",
    "覺得　你跟　各位　更親",
    "跟我　一向　不算　很近",
    "",
    "我也只想回到我的山洞裡",
    "再也不想隨便相信人類",
    "我也只好放你進思念收集區",
    "這樣不會傷害誰",
    "",
    "最初　沒想　什麼　目的",
    "結果　也都　逐步　沉溺",
    "若果　我不　就此　絕跡",
    "最終　淪為　更糟　角色",
    "",
    "我也只想回到我的山洞裡",
    "再也不想隨便相信人類",
    "我也只好放你進思念收集區",
    "這樣不會傷害誰",
  ], [
    "你說最近看沙特的《嘔吐》",
    "但那主旨卻掌握不到",
    "「夜蒙住了燈光」「撲克牌亂糟」",
    "",
    "我也看這書每當苦惱",
    "我也不懂得它的全部",
    "每次看卻會對他除帽",
    "",
    "這種感覺不去表達也好",
    "想表達也找不到辭措",
    "",
    "每次派對我都會很早到",
    "預備了時間給盪失路",
    "遲到不應該感到自豪",
    "",
    "某次看到不好看的 show",
    "想起那時光跟你虛度",
    "聖誕睡著看的芭蕾舞",
    "",
    "彷彿新世紀正要來到",
    "彷彿因此使你要嘔吐",
    "",
    "看見退休的歌手逛港島",
    "他眷戀的風格已變味道",
    "下午是漫無目的潛逃",
    "",
    "給我一個肉身可擁抱",
    "給我一句詩走這段路",
    "「醉鄉路穩宜頻到」",
    "",
    "這種感覺不去表達也好",
    "想表達也找不到辭措",
    "彷彿新世紀正要來到",
    "彷彿因此使你要嘔吐"
  ], [
    "我也有過快樂",
    "在某幾個年份",
    "我已沒有抱恨",
    "什麼都不要緊",
    "不貪求永恆",
    "慶幸曾發生",
    "只感到好運",
    "就算曾很傷心",
    "",
    "（獨白）",
    "你話成個晏晝",
    "乜嘢都冇發生過",
    "你好難過",
    "我話唔係呀",
    "日落嘅時候",
    "你仲可以同佢講",
    "我愛你",
    "或者係 Bye Bye",
    "",
    "夢中我遺下了",
    "一些東西要緊",
    "日本某層飯店",
    "離開已是凌晨",
    "再醒來已經是",
    "八點零五分",
    "覺得還有需要",
    "回去再搜尋",
    "",
    "若干年之後",
    "我會重來",
    "把你抱入懷內",
    "如果你也是同樣",
    "把過去當成愛",
  ], [
    "我不適合聚會",
    "不知道聚會的規矩",
    "不知道帶什麼酒精",
    "蜜瓜　香料　毒品　火腿",
    "",
    "我不適合聚會",
    "不知道準時的樂趣",
    "不知道說什麼笑話",
    "是非　天氣　社會　過去",
    "",
    "我不知怎聚會",
    "吸煙的習慣已失去",
    "再不知怎隨意轉身到露台去",
    "",
    "我不適合人類",
    "超過半小時的派對",
    "靈魂在這裡　沒有同類",
  ], [
    "不要把冬天衫太早放回衣櫃",
    "冬天已經走了這只是你認為",
    "那夜之間聽見太多悲痛的故仔",
    "更悲痛的故仔原諒我不想再提",
    "",
    "不要把冬天衫太早放回衣櫃",
    "春天已經到了北風還會再嚟",
    "世上太多苦況才令你成為集體",
    "你若離開谷底亦會失去安慰",
    "",
    "不要把幸福的相都放上網傳遞",
    "世界都知道了並不代表一切",
    "極盛大的婚禮轉眼已變前妻",
    "能夠埋藏心底才算是最珍貴",
    "",
    "不要覺得旁人成功是你的關係",
    "每次你的失敗歸咎旁人問題",
    "你要是不敢看見他人的光輝",
    "四季都只會被冬天包圍",
  ]
]

// I Also don't know (live)

const a11list = {
  og: [
    "讓我搭一班會爆炸的飛機 - Live",
    "美麗新香港 - Live",
    "奇人的離職 - Live",
    "每次你走的時分 - Live",
    "你說之後會找我 - Live",
    "喫煙席 - Live",
    "QUEEN - Live",
    "宋代 SAD BOY - Live",
    "20GB - Live",
    "SABINA 之淚 - Live",
    "悲傷的採購 - Live",
    "只因當時太緊張 - Live",
    "介乎法國與旺角的詩意 - Live",
    "只收一半 - Live",
    "因講了出來 - Live",
    "彌敦道的一晚 goodtrip - Live",
    "下亞厘畢道 - Live",
    "麥記最後一夜 - Live",
    "Ｋ同學 - Live",
    "煙 - Live",
    "今宵多珍重 - Live",
    "時代的孩子 節錄 - Live",
    "心 - Live",
    "散步之年 - Live",
    "那陣時不知道 - Live",
    "詩歌舞街 - Live",
    "土瓜灣情歌 - Live",
    "年輕的茶餐廳老闆娘 - Live",
    "以為和你熟 - Live",
    "和陳五 msn - Live",
    "rm1210 - Live",
    "terence 的心事 - Live",
    "失落沮喪歌 - Live",
  ],
  en: [
    "Let Me Get On an Exploding Plane - Live",
    "Brand New Hong Kong - Live",
    "Weirdo's Resignation - Live",
    "Every Time You Left - Live",
    "You Said We'd Be Back - Live",
    "Smoking Seat - Live",
    "QUEEN - Live",
    "Song Dynasty's SAD BOY - Live",
    "20GB - Live",
    "SABINA 之淚 - Live",
    "Sad Purchase - Live",
    "Because I Was Too Nervous at That Time - Live",
    "Poetics - Something Between Montparnasse and Mongkok - Live",
    "Just for Half Price - Live",
    "When I Let It Out - Live",
    "A Night of Good Trip Along Nathan Road - Live",
    "Lower Albert Road - Live",
    "The Last Night at McDonald's - Live",
    "Classmate K - Live",
    "Smoke - Live",
    "Farewell - Live",
    "Children of the Age (Excerpt) - Live",
    "Heart - Live",
    "Year of Strolling - Live",
    "Didn't Know Then - Live",
    "Sycamore Street - Live",
    "To Kwa Wan Ballad - Live",
    "Cha Chaan Teng's Young Boss Lady - Live",
    "Thought We Were Close - Live",
    "MSN with Ng Chan - Live",
    "rm1210 - Live",
    "Terence's Worries - Live",
    "The Lost and Frustration Song - Live",
  ]
}

const a11meta = [
  [
    2,49,
    'P', 'P', 'P,N,阿賢,阿科', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,24,
    'P', 'P', 'P,N,阿賢,阿科,陳銘澤 @ 在草地上', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,0,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,51,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,5,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,13,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,22,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,15,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,0,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,37,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,58,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,24,
    "P", "P", null, "N", {
      backup_vocal: 'Karena'
    }
  ], [
    2,21,
    'P', 'P', 'P,阿賢,阿科', 'N,P,Jackie,Sot Kanin', {
      backup_vocal: 'Karena'
    }
  ], [
    2,51,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,41,
    'P', 'P', 'P,蕭暢悅', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,4,
    'P', 'P', 'P,四方果', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,32,
    'P', 'P', 'P,夏日浪漫', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,40,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,56,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,55,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,53,
    "LMF", "LMF", "P", "N", {
      backup_vocal: "Karena"
    }
  ], [
    1,35,
    'P', 'Wislawa Szymborska', 'P', 'Karena', null
  ], [
    2,27,
    'N', 'N', 'P,N,阿賢,阿科,夏日浪漫,銘銘', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,12,
    'P', 'P', 'P', 'N', {
      backup_vocal: "Karena"
    }
  ], [
    3,1,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,15,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,32,
    'P', 'P', 'P,N,阿賢,阿科', 'N', {
      backup_vocal: 'Karena',
    }
  ], [
    3,5,
    'P', 'P', 'P,N,阿賢,阿科', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,9,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,38,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,11,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,55,
    'P', 'P', 'P', 'Porpor Channel,Kit @ 22cats,Ling ling ling,N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,0,
    "P", "P", null, "N", {
      backup_vocal: 'Karena'
    }
  ]
]

const a11lyrics = [
  [
    "讓我搭一班會爆炸的飛機",
    "去到台灣之前被炸死",
    "讓我喝一杯會吐血的香檳",
    "在喝醉之前可斷氣",
    "",
    "我以後也不會再打電話給你",
    "再沒法對電話唱Ｋ",
    "你要是孤單便去找好友睇戲",
    "他們一定會氹返你",
    "",
    "讓我搭一班會爆炸的飛機",
    "回到香港之前被炸死",
    "讓我喝一杯會吐血的香檳",
    "在喝醉之前擁抱你",
    "",
    "我以後也不會再打電話給你",
    "再沒法對電話唱Ｋ",
    "你要是孤單便去找好友睇戲",
    "他們一定會氹返你",
    "",
    "讓我搭一班會爆炸的飛機",
    "回到香港之前被炸死",
    "讓我喝一杯會吐血的香檳",
    "在喝醉之前擁抱你"
  ],
  [
    "自你決定要走以後",
    "沒人知我有多難受",
    "再沒勇氣在派對逗留",
    "踫見朋友不敢問候",
    "",
    "自你遠去不再回頭",
    "青山綠水不再依舊",
    "再沒有早起身的理由",
    "再沒可安睡的枕頭",
    "",
    "這世界只有一種鄉愁",
    "就是沒有你的時候",
    "這香港已不是我的地頭",
    "就當我在外地旅遊",
    "",
    "自美夢再不可尋求",
    "我便學會清風兩袖",
    "命運要令我學識奮鬥",
    "但我只懂得荒謬",
    "",
    "這世界只有一種鄉愁",
    "是你不在身邊的時候",
    "這香港已不是我的地頭",
    "就當我在外地飄流",
    "",
    "我知已走到盡頭",
    "為何還要擔憂？",
    "這世界也不是我的地頭",
    "就當我在宇宙飄流"
  ],
  [
    "到最後你都係要走",
    "我沒有挽留",
    "到最後你都係要分手",
    "你沒有回頭",
    "我沒有強求",
    "你在我眼前",
    "我在你背後",
    "看你離開帶走你的小狗",
    "",
    "到最後我都沒有開口",
    "我只有垂頭",
    "到最後我都沒有跳樓",
    "我只有淚流",
    "對於愛情",
    "對於友情",
    "我都會任由",
    "任由它自己飄走",
  ],
  [
    "每次到了你要走的時分",
    "我的心都會痛一陣",
    "總想有人會對你更貼心",
    "但又怕你不再接近",
    "",
    "世上總有人純粹而聰敏",
    "能夠明瞭這情感",
    "盼望你能被照顧得開心",
    "遙距與我共度餘生",
    "",
    "愛就似條河水般支分",
    "不由得流向他人",
    "這種抽離和這種痴心",
    "一秒即永恆（不留傷痕）",
    "",
    "世上總有人純粹而聰敏",
    "能夠明瞭這情感",
    "盼望你能被照顧得開心",
    "遙距與我共度餘生",
    "",
    "每次約會前",
    "我都會先打掃房間",
    "不知這次你會否上來",
    "小心翼翼",
    "不留低之前任何人的髮根",
    "",
    "每次到了你要走的時分",
    "我的心都會痛一陣",
    "每次到了你要走的時分",
    "我的心都會痛一陣（不留傷痕）",
    "",
    "每次到了你要走的時分",
    "我的心都會痛一陣",
    "每次到了你要走的時分",
    "我的心都會痛一陣（不留傷痕）",
    "（不留傷痕）"
  ],
  [
    "你在哪裡　我竟不知",
    "枉我們曾每日都相對",
    "你想著誰　為何流眼淚",
    "我竟也不再問一句",
    "",
    "呼呼呼呼　呼呼呼呼",
    "像風一樣的年歲",
    "呼呼呼呼　呼呼呼呼",
    "已不知被吹到哪裡",
    "",
    "我在哪裡　我也不知",
    "像被迫進了一個監獄裡",
    "只有過去　已沒能再聚",
    "目睹最美的事粉碎",
    "",
    "呼呼呼呼　呼呼呼呼",
    "像風一樣的年歲",
    "呼呼呼呼　呼呼呼呼",
    "已不知被吹到哪裡",
    "",
    "一生人很短",
    "但終點很遠",
    "任年月讓我再大多幾歲",
    "任年月伴我繼續老去"
  ],
  [
    "一直知道更多",
    "令你一直失去更多",
    "真相告訴給你",
    "你因此不信我",
    "",
    "再會了那些從前片段",
    "你我不過見過數面",
    "",
    "一段日子已過",
    "再也沒去做過什麼",
    "不去工作不去拍拖",
    "不吸煙不吃水果",
    "",
    "四周也日麗風和",
    "卻有雨一直灑向我",
    "漫步到最遠咖啡座",
    "褪黑素幾晚沒效果",
    "",
    "再會了那些從前故事",
    "你亦不要太過懷念",
    "",
    "生命像一首老歌",
    "重複著某一樣痛楚",
    "Que será, será",
    "年老但仍未停播"
  ],
  [
    "I have named you queen",
    "There are taller than you, taller",
    "There are purer than you, purer",
    "There are lovelier than you, lovelier",
    "But you are the queen",
    "",
    "我也喜歡被你這樣拒絕我",
    "我也喜歡被這種感覺折磨",
    "坐上情感過山車後座",
    "安然已不屬我",
    "",
    "我是否應該找較平凡的那個",
    "但他不會給我甜甜的折磨",
    "每次鈴聲響起是你麼",
    "失去理智也不錯",
    "",
    "當你每次一在場",
    "體內似有河水聲響",
    "方圓十里植物突然生長",
    "溫柔包圍著我倆",
    "",
    "每次見面後",
    "我都會踩很久的單車",
    "走很多路",
    "你身上的味道飄來飄去",
    "昨晚之後",
    "全日又有很多香味",
    "",
    "但你始終未肯講再見不見我",
    "就讓我在這階段好好消磨",
    "往後如何也不再會這樣傻",
    "隨意歡喜略過",
    "",
    "當你每次一在場",
    "體內似有河水聲響",
    "方圓十里植物突然生長",
    "溫柔包圍著我倆"
  ],
  [
    "大千世界裡　我撇低你",
    "我撇低你　但又常想起",
    "大千世界裡　碰巧跟你",
    "就算只有　那一晚驚喜",
    "",
    "何時苦痛　何時便抽離",
    "平靜風雨裡　曾遇你",
    "",
    "大千世界裡　肉體紛飛",
    "情慾仿似　駕駛穿梭機",
    "大千世界裡　是他也是你",
    "曾被刺痛　隨時被勾起",
    "",
    "從前心跳　從未願抽離",
    "沿路風雨裡　誰是你",
    "",
    "平靜風雨裡　曾遇你",
    "",
    "千山同一月",
    "萬戶盡皆春",
    "千江有水千江月",
    "萬里無雲萬里天"
  ],
  [
    "如不是 Flickr 再一加價",
    "我也不會把八千張照片拿下",
    "迫使我重溫四年這段關係變化",
    "第一張相喺黃大仙開始",
    "你的眼耳口鼻",
    "是我生平遇過的最美",
    "想把一切都給你",
    "不設限期",
    "",
    "時光",
    "把各人的青春玩弄",
    "20GB 的重",
    "都不過是愛情的灰燼",
    "證實如何燒掉每張笑容",
    "",
    "這四年像是四十年",
    "要忘掉也是不容易",
    "",
    "黃大仙之後我話寄嘢俾你",
    "問你地址",
    "過兩日我貼咗張郵票",
    "喺戴住嘅冷帽上面",
    "去到你屋企門口",
    "我話我係嗰份郵件",
    "",
    "還有一些冬天郊區的照片",
    "第一次吃你煮的撈麵",
    "深夜投放電影在牆上過年",
    "",
    "時光",
    "把我們的青春玩弄",
    "20GB 的重",
    "都不過是愛情的灰燼",
    "證實如何燒掉每張笑容",
    "",
    "再來一次也不是那回事",
    "再來一次也不是那回事"
  ],
  [
    "應該早知道（應該早知道）",
    "根本就不是（根本就不是）",
    "應該早知道（應該早知道）",
    "根本不會是（根本不會是）",
    "對方沒任何表示",
    "純粹是沒有意思",
    "難道你認為是他不敢示意",
    "",
    "應該早知道（應該早知道）",
    "根本就不是（根本就不是）",
    "應該早知道（應該早知道）",
    "根本不會是（根本不會是）",
    "對方沒任何表示",
    "沉默像深海帶子",
    "情慾會令人突然喪失睿智",
    "",
    "心已經被佢偷走幾次",
    "誤解晒每個 post 每個 story",
    "過盡千帆皆不是",
    "根本冇個 post 係關你事",
    "",
    "但要逃脫亦不易",
    "繼續前行亦沒意思",
    "魂斷維也納他都不會在意",
    "",
    "應該早知道（應該早知道）",
    "應該早知道（應該早知道）"
  ],
  [
    "訂了一季　又到下季",
    "為何人大了就要成為工作的奴隸",
    "最愛作的不可發揮",
    "我的感覺　逐漸流逝",
    "從前曾話過要如何欣賞世界的美麗",
    "現在只懂得放假去消費",
    "",
    "如果繼續這樣",
    "我還是我嗎？",
    "我還喜歡我嗎？",
    "或者我應該死去吧",
    "總好過變得更差",
    "如果繼續這樣",
    "你還願意嗎？",
    "和我奏著結他？",
    "日子不應該這樣吧",
    "而現實就是這樣可怕",
    "",
    "如果繼續這樣",
    "我還是我嗎？",
    "我還喜歡我嗎？",
    "或者我應該死去吧",
    "總好過變得更差",
    "如果繼續這樣",
    "你還願意嗎？",
    "和我奏著結他？",
    "日子不應該這樣吧",
    "而現實就是這樣可怕"
  ],
  [
    "只因當時我太緊張",
    "令你覺得我很異常",
    "看到你令我有很多遐想",
    "最好以後都這樣",
    "",
    "那次不小心掂了你手掌",
    "令我覺得很有營養",
    "與你出街是我的夢想",
    "人生充滿新方向",
    "",
    "別人說你很囂張",
    "但是我覺得你很善良",
    "我已沒有心機再細想",
    "怎麼告訴你真相？",
    "",
    "別人說你太囂張",
    "但是我覺得你很正常",
    "我已沒有心情再去聯想",
    "怎麼能令你心癢？",
    "",
    "只因當時我太慌張",
    "沒發揮到我的擅長",
    "我最想與你去數綿羊",
    "每夜美夢很歡暢",
    "",
    "結果我與你去了足球埸",
    "然後再逛白購商場",
    "順便看了一些明星相",
    "但願你沒高修養",
    "",
    "別人說你很囂張",
    "但是我覺得你很善良",
    "我已沒有心機再細想",
    "怎麼告訴你真相？",
    "",
    "別人說你太囂張",
    "但是我覺得你很漂亮",
    "我已沒有心情再去聯想",
    "怎麼能令你心癢？",
    "",
    "我似漸變抑鬱與感傷",
    "都不能令你歡暢"
  ],
  [
    "俾我再約多你一次",
    "再約多一次　再多一次",
    "你我這件事　回到公司",
    "不會有人知",
    "",
    "俾我再交換多一次",
    "交換多一次　換禮物的心意",
    "放進去雪櫃　用黃色的膠袋包起",
    "不會有人懷疑",
    "",
    "其實我不願離開這公司",
    "全都因為對你太鍾意",
    "",
    "俾我再講多一次",
    "再講多一次　我的心事",
    "我最愛你是　介乎法國與旺角的詩意",
    "",
    "俾我再送多你一次",
    "再送多一次　到你家多一次",
    "送到去門前　再行多一圈",
    "再兜多一轉",
    "",
    "其實我不願離開這公司",
    "全都因為對你太鍾意",
    "",
    "其實我心願離開這公司",
    "然之後一同與你搞生意",
    "誰都知是對你太鍾意"
  ],
  [
    "有次我咪同你講話",
    "我喺 IG 搵 PTGF 嘅",
    "有個覆返我話",
    "三千蚊一個鐘",
    "包 wet kiss, bj, ml",
    "同埋bath tgt",
    "我有問你抵唔抵㗎嘛",
    "同埋好唔好每個星期都 call?",
    "其實呢  我係諗住",
    "我係諗住你會話：",
    "",
    "「我只收你一半」",
    "「我也感到很悶」",
    "「我也想找個伴」",
    "「把這假期填滿」",
    "「我只收你一半」",
    "「我也感到很悶」",
    "「我也想找個伴」",
    "「把這宇宙填滿」",
    "",
    "但係你都冇咁樣講",
    "你叫我快啲 call",
    "刺激下本地經濟",
    "我仲諗住",
    "一齊煮意粉食",
    "睇啲好耐冇睇嘅戲",
    "食雲呢拿味雪糕溝啲咖啡入去",
    "第二朝再煮早餐俾你",
    "但係你都冇咁樣講：",
    "",
    "「我只收你一半」",
    "「我也感到很悶」",
    "「我也想找個伴」",
    "「把這假期填滿」",
    "「我只收你一半」",
    "「我也感到很悶」",
    "「我也想找個伴」",
    "「把這宇宙填滿」",
  ],
  [
    "（跳舞吧）",
    "",
    "因講了出來",
    "便會失去吸引力",
    "失去機會被愛",
    "因講了出來",
    "便會失去所有",
    "想像中的可愛",
    "",
    "寧願永遠",
    "永遠停留在",
    "沒太多往來",
    "不在時間上慷慨",
    "寧願永遠",
    "永遠停留在",
    "想像之內",
    "免去細節的傷害",
    "",
    "就這樣　在午夜　在馬路",
    "在網上　在購物　在腦內　就這樣愛",
    "就這樣　在被內　在散步",
    "在午睡　在吃飯　在發夢　就這樣愛",
    "和你去發展愛",
    "",
    "保持三兩日嘅 message",
    "報告睇過嘅電影",
    "交換一兩句影評",
    "間中一張食物照",
    "好睇嘅副刊 link",
    "你有冇去過淺水灣呢間露台餐廳？",
    "我喺日本嘅和歌展覽",
    "見到啲好短好脆弱嘅古詩",
    "例如有首係：",
    "「只因你說了一句『馬上就來』／",
    "　我在九月漫長的夜晚苦苦等待／",
    "　等到清晨月影浮出天外」",
    "又例如：",
    "（就這樣　在午夜　在馬路",
    "在網上　在購物　在腦內　就這樣愛）",
    "「本來只有千年之齡的松樹／",
    "　今天被你的手一掂／",
    "　想必長活萬年」",
    "（就這樣　在被內　在散步",
    "在午睡　在吃飯　在發夢　就這樣愛）",
    "離開展覽前",
    "我都喺留言簿寫咗首：",
    "「不敢看到美好的詩句／",
    "　怕用手機影低時／",
    "　被保安捉去」"
  ],
  [
    "每到晚上六七點",
    "我先會開始 weed",
    "再走上一架巴士",
    "坐上層的最前",
    "",
    "坐到去油麻地",
    "其中一個巴士站",
    "有架１號Ａ停咗喺前面",
    "嗰架巴士上層最後一行",
    "坐咗對幾靚嘅男女",
    "不停咁錫嚟錫去",
    "喺我面前",
    "",
    "兩個眯埋眼由咀錫到落條頸",
    "停咗幾秒　又不停咁錫",
    "佢哋前面無其他人",
    "我架巴士前排又得我一個人",
    "睇到有啲唔好意思",
    "所以有啲特別開心",
    "",
    "兩架巴士一路向前",
    "有時佢地行快啲會唔見一陣",
    "但去到燈位又貼返喺正前面",
    "好似我架巴士同佢哋架巴士",
    "喺彌敦道上面",
    "錫下停下的纏綿",
    "",
    "每到晚上六七點",
    "我先會開始 weed",
    "再走上一架巴士",
    "坐上層的最前",
    "",
    "喺土瓜灣出發",
    "搭６號Ｃ去朗豪坊睇戲",
    "呢段路我已經行到好熟",
    "燒賣　雪糕　迎面嘅人　恐怖片",
    "smoke咗之後　我吸晒",
    "所有的臉",
    "",
    "搭搭下我突然間記返起",
    "喺我幾歲嘅時候",
    "香港曾經有過一兩單交通意外",
    "坐巴士上層最前嘅乘客被撞死",
    "自始之後",
    "呢個位就俾啲大人講到好危險",
    "三十年嚟",
    "我好似失去咗理性",
    "每次上到上層都會自動坐後面",
    "直到 smoke 咗之後",
    "先揀最好嘅風景",
    "放在面前"
  ],
  [
    "和你兩個月前每日 inbox 傾訴",
    "初次約會要設定一種暗號",
    "如你我碰面後若想離開提早",
    "用暗號避免直接令對方失措",
    "",
    "一杯 moscato　再一杯 moscato",
    "你教我講冰島語如何問好",
    "由歌賦街飲到去下亞厘畢道",
    "只是 moscato 未能亂我腳步",
    "",
    "由兩個月前等待今晚的來到",
    "都不敢把寄望定得太高",
    "但這約會後為何開始煩惱",
    "自你離開前的一個擁抱",
    "",
    "一杯 moscato　再一杯 moscato",
    "你說你明天不用上班多好",
    "由歌賦街飲到去下亞厘畢道",
    "聽著 Sinatra 的《我不會跳舞》",
    "",
    "和你設了暗號若想離開提早",
    "但沒設有暗號若想再進一步"
  ],
  [
    "就在那夜送你歸家以後",
    "你再打給我說　你沒有上樓",
    "沿著我背後一路走　因見面不敢開口",
    "「明天要返英國」　電話裡淚流",
    "",
    "就到家的我即轉身回頭",
    "寒夜跟你到麥記　朱古力當是酒",
    "我若全力追究　你也會來去自由",
    "那種金牛座特性你全沒有",
    "",
    "麥記的最後一夜",
    "白燈、廣東歌與露宿者",
    "想起廟街某夜 相士說你屬於那些",
    "「矛盾波動的千里馬」般狂野",
    "",
    "就在八月知你住我家附近之後",
    "那夜經過大角咀你細訴小時候",
    "然後我們隔一兩週 便結伴公園夜遊",
    "潛入「嶺南之風」看不見四周",
    "",
    "但是我亦走不進你心裡頭",
    "某夜親吻到似變了男女朋友",
    "但是你從不愛拖手　只是放手進我衣袖",
    "代表只想取暖　是否？",
    "",
    "麥記的最後一夜",
    "白燈、廣東歌與露宿者",
    "這惡俗的不捨　比一起更好過些",
    "才是你遠走的主因　或者？",
    "",
    "麥記的最後一夜",
    "橙汁、朱古力與露宿者",
    "合照留下一些 轉地方睡最後今夜",
    "完結這一場冬天的狂野",
    "",
    "電捲風馳　從此各自上車"
  ],
  [
    "如果可以重來　到太子午夜時候",
    "便會拖著你手不會似中學生那麼害羞",
    "你愛將警察當做背景自拍十張才夠",
    "路過的夫婦以為你來自韓國到此旅遊",
    "",
    "士多老細收錯錢說幾晚不夠人手",
    "酒吧街邊吧女拚命遊說我們飲啤酒",
    "",
    "望向星星是哪星球　它突然間飄走",
    "懷疑是航拍後　你竟除下口罩向它揮手",
    "問我想有什麼成就　我本不想活太久",
    "何以見到你後　我竟然會習武鍛鍊身手",
    "",
    "放狗的途人照舊　牠擒向另一女生膝頭",
    "圍觀到防暴對峙時也行近看下又走",
    "女生說家有三隻母狗　我說她那味散四周",
    "群眾都起哄話我　還要是你帶頭",
    "",
    "著拖鞋紅衫短袖　手執燒賣配清酒",
    "願這街坊裝被捕可免受暴動罪追究",
    "",
    "望向星星是哪星球　它突然間飄走",
    "懷疑是航拍後　你竟除下口罩向它揮手",
    "問我想有什麼成就　我本不想活太久",
    "何以這晚之後　我竟想去習武保衛地球",
    "",
    "問我想有什麼成就　我本不想活太久",
    "何以見到你後　我竟然會習武鍛鍊身手"
  ],
  [
    "你說你不想遠離",
    "煙霧的滋味",
    "已看不到距離",
    "勝利的日期",
    "朦朧的希冀",
    "時代的奧秘",
    "",
    "你看到煙已霧起",
    "靈魂已準備",
    "誰願有這勇氣",
    "與肉體分離",
    "無謂的黑旗",
    "窒息的空氣",
    "",
    "已想像得到結尾",
    "熟悉的攻鼻",
    "茶餐廳電視機",
    "顛倒各樣是非",
    "幾代人也不理",
    "罪名卻給了你"
  ],
  [
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又未必會係噉",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又另外一個樣",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又未必會係噉",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又另外一個樣",
    "",
    "你有冇埋怨自己生出嚟嗰陣時",
    "唔係含住條金鎖匙　係一條生銹鎖匙",
    "你老豆冇本事　冇錢養妻活兒",
    "你呀嫲就啋　你個衰仔",
    "生你出嚟都係多餘　唔該",
    "你要記住人生嘅大門係要靠你雙手去打開",
    "掂嗰陣時就唔駛見風駛盡𢃇",
    "耷嗰陣時唔好怨天怨地",
    "",
    "可以試下去諗條路唔係一定要噉走",
    "要壞嘅話唔一定要食煙飲酒",
    "唔係一定要耷低頭做人隻狗",
    "可以抬起頭自己行每一步路",
    "唔駛畀人睇死你係一個冇用嘅細路",
    "自己去做做到自己就當然最好",
    "做唔到都千祈唔好放棄自己",
    "鎅手跳樓飲滴露蕩失路",
    "",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又未必會係噉",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又另外一個樣",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又未必會係噉",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又另外一個樣",
  ],
  [
    "所有你的、我們的、你們的",
    "日常和夜間事務，",
    "都是政治的事務。",
    "",
    "你說的話，有政治的回音，",
    "你的沉默，訴說著許多話語，",
    "橫著看豎著看都是政治性的。",
    "甚至當你走入森林，",
    "你也踏著政治的步伐，",
    "走在政治的地面上。",
    "",
    "非政治的詩也是政治的，",
    "天空中懸掛著月亮，",
    "那已經不只是月球。",
    "活著或是死亡，這是個問題。",
    "甚麼樣的問題，回答吧親愛的。",
    "那是個政治的問題。",
    "",
    "（──辛波絲卡《橋上的人們》）"
  ],
  [
    "分手說再見也不易",
    "瀟灑的人也有流淚時",
    "明明知道　時候到了要變異",
    "突然消失了　擁抱都不要",
    "",
    "分手到底為什麼事",
    "是誰做錯令你我遲疑",
    "明明知道　無常總會有",
    "來的時候　說放手誰又可以？",
    "",
    "我把心打開　照星星月亮",
    "讓痛處眼淚盡流",
    "還望過去　你我沒有負累",
    "學會適當時要爭取",
    "",
    "我把心打開　照星星月亮",
    "讓痛處眼淚盡流",
    "祈願每個此刻　我更懂我",
    "重拾熱情像最初",
    "",
    "祈願每個此刻　我更懂我",
    "就算變幻很多",
    "",
    "願我心可放鬆過",
    "從今天當摯真的我"
  ],
  [ 
    "靜靜過這天",
    "慢慢度這年",
    "沒有要去邊",
    "這是最好一年",
    "從沒走進這路線",
    "這是我剛剛發現",
    "某一塊肌肉像似",
    "得到首次鍛鍊",
    "",
    "有冇一種可能係",
    "咁啱行緊嘅呢一段",
    "係喺歷史嘅中段",
    "終點係喺好後面",
    "你係唔會見到",
    "但最終係會出現",
    "",
    "(Ah ah ah ah ah)",
    "(Ah ah ah ah ah)",
    "",
    "每晚看舊影片",
    "伊力盧馬冬天故事",
    "吃盡各種米線",
    "那是我的自由意志",
    "僅此而已",
    "還可以如此",
    "僅此而已",
    "還可以如此",
    "",
    "也會時常掛念那時",
    "掛念世界與你未病發前",
    "與你去到泰國夜市",
    "去到了一個攤檔街邊",
    "（那時）",
    "",
    "你話想買件飾物穿耳（那時）",
    "我問個檔主可唔可以平啲（那時）",
    "我見佢勉為其難咁話可以（那時）",
    "找錢嗰時我話唔駛",
    "我都係俾原本價錢（那時）",
    "你說你很有面子（那時）",
    "因我今晚的善意（那時）",
    "你說你很有面子（那時）",
    "因我今晚的善意（那時）",
    "（你說你很有面子）",
    "（因我今晚的善意）",
    "（你說你很有面子）",
    "（因我今晚的善意）",
  ],
  [
    "如像九十年代初",
    "照片裡海灘的印象",
    "那種下午沒有重量",
    "帶點迷惘奔向夕陽",
    "",
    "那年獨自去南洋",
    "一杯白咖啡的早上",
    "四周是可冒險方向",
    "尤加利樹葉的香",
    "",
    "那陣時不知道",
    "置身的日子都發亮",
    "眼光裡藏著的囂張",
    "往後已不再同樣",
    "",
    "圖書館木製的窗",
    "訓導假裝嚴肅的模樣",
    "還未有傾訴對象",
    "仍未覺得悲涼",
    "",
    "那陣時不知道",
    "每天在風光的戰場",
    "騎著馬去到邊疆",
    "所到之地全是賜賞",
    "",
    "Hoo 寂靜但不漫長",
    "Hoo 寂靜但不漫長",
    "Hoo 寂靜但不漫長",
    "Hoo 寂靜但不漫長",
    "",
    "La la la la la",
    "La la la la la",
    "La la la la la（那陣時不知道）",
    "La la la la la",
    "La la la la la",
    "La la la la la",
    "La la la la la（那陣時不知道）",
    "La la la la la",
    "La la la la la",
    "La la la la la",
    "La la la la la",
    "La la la la la",
  ],
  [
    "那晚看 Russian Red 大角咀表演",
    "你與我竟會再遇見",
    "剛好我也到 bar 枱買酒才碰面",
    "完場與你去深水埗那邊",
    "剛過身的作家你介紹那年",
    "寫過只有散步我們才真正聊天",
    "",
    "再次聽你說故事",
    "聽你說剛生的嬰兒",
    "詩歌舞街地上有著光點閃閃",
    "聽你說外地歷險",
    "我卻像當天的少年",
    "但已是能接受你所有改變",
    "",
    "你說你最近愛聽古典",
    "你說你父母開始婚變",
    "邊聽邊講我忘了怎去 99 咖啡店",
    "說到我倆澳門過生日那年",
    "你為我訂了山上的酒店",
    "餐廳老闆對你彈唱結他一整天",
    "",
    "再次聽你說故事",
    "聽你說剛生的嬰兒",
    "走進萬發坐在暗光卡位窗邊",
    "聽你說外地歷險",
    "我卻像當天的少年",
    "但已是能控制我所有淚腺"
  ],
  [
    "這一個下雨的夜晚",
    "你是否正在空閒",
    "我此刻仍留在土瓜灣",
    "想著未來怎麼辦",
    "只希望沙中線的那個站",
    "可以起得更慢",
    "再貴的租我已不能負擔",
    "",
    "這一個下雨的夜晚",
    "我是否仍在你心間",
    "已有多少年沒有上班",
    "煩惱為何卻未減",
    "只想歲月過得平淡",
    "知這是最困難",
    "願你我能留在黑暗中多一陣間",
    "",
    "這一個下雨的夜晚",
    "我躺在沙發矇矓間",
    "收音機說北韓已射出導彈",
    "地球另一邊被炸爛",
    "所有的事都不夠虛幻",
    "因現實未夠最爛",
    "要到絕望才望見希望在人間",
    "",
    "到了某天你我不再浪漫",
    "有不喜歡的工要返",
    "你會否便記起曾笑得這麼燦爛"
  ],
  [
    "這是我最後一次",
    "來吃午飯吃到四點",
    "以後或不會再見",
    "到底我也是高傲的成年",
    "",
    "已考慮不知幾多遍",
    "才決定給你這信件",
    "寫了電話等你致電",
    "明知你看了只會笑一百遍",
    "",
    "這些年裡面",
    "你都知我很少到別的店",
    "但若你最終給我來電",
    "你可知道這感覺會點",
    "",
    "像一支小蠟燭的光點",
    "讓漆黑的房滿佈光線",
    "像打開考試試卷發現",
    "突然所有答案都看得見",
    "",
    "對上的一次冬至",
    "來吃晚飯吃到八點",
    "但你卻沒有出現",
    "出現我也不敢看你正面",
    "",
    "已幻想不知幾多遍",
    "旅行有你在我身邊",
    "多次見你髮型轉變",
    "又是為吸引那個他的視線",
    "",
    "這些年裡面",
    "你都知我很少到別的店",
    "但若你最終給我來電",
    "你可知道這感覺會點",
    "",
    "像一支小蠟燭的光點",
    "讓漆黑的房滿佈光線",
    "像打開考試試卷發現",
    "突然所有答案都看得見",
    "",
    "這些年裡面",
    "我也不知道我可以去邊",
    "但若你最終給我來電",
    "我可給你一整個的春天"
  ],
  [
    "你替我選的那古著牛仔衣服",
    "是要讓我看來不太毒",
    "我卻每天聞起來想你才舒服",
    "但又怕聞到會麻木",
    "",
    "聽說世間所有情愛的事",
    "不過是浪漫的想像所致",
    "那我已知苦痛只不過是",
    "因我太夢幻的心智",
    "",
    "也說不出這算熟還是不熟",
    "能對望幾秒眉目",
    "但你眼中根本就從不算熟",
    "就算已訴盡我感觸",
    "",
    "聽說世間所有情愛的事",
    "不過是浪漫的想像所致",
    "與你那天做的每一件事",
    "也使我回味千百次",
    "",
    "與你那天做的每一件事",
    "也使我回味千百次",
    "每天也期待下一次"
  ],
  [
    "我愛你　我要帶你返屋企",
    "我不要等你",
    "我不要在 MSN 等你",
    "我要你　我要撟住你的手臂",
    "像那晚撟住你的手臂",
    "閉著眼行了一條街",
    "你說這是測驗對對方信心的遊戲",
    "",
    "這個世界已沒有將來",
    "我在這裏被悶到炸開",
    "我要與你流亡到海外",
    "我說我們不如一同去到越南食魚蛋",
    "我說我們不如一同去到月球釣泥猛",
    "我說我們不如一同去到去到去到去到……",
    "",
    "其實我知道你不開心",
    "其實我知道你不想一個人",
    "其實我都不想一個人",
    "我說我們不如我們不如結婚",
    "但結婚之後要分開住分開訓",
    "然後你問「咁點解要結婚？」",
    "我說和不太熟的人結婚應該會很開心",
    "",
    "這個世界已沒有將來",
    "這裏永遠也沒法變改",
    "我要與你流亡到海外",
    "或者我們都不需要真的戀愛",
    "或者我們只不過需要間唔中去一次諾士佛臺",
    "或者我們只不過需要一種新的戀愛"
  ],
  [
    "或者是過了今年　或者是過了明年",
    "你說會與他結為夫妻　永遠不變",
    "最好是今年　我說不要等明年",
    "既然你已說好　從那天起我們不再見",
    "",
    "然後我步入中年　然後我步入老年",
    "我或已經忘記　有否到過你婚宴",
    "而我不想忘記那天　我們遇見的那年",
    "那年吃過幾多薯片　看過幾多套港產片？",
    "",
    "知你們不會改變　你們計劃了多年",
    "就趁這之前　我仍願繼續發展",
    "然後我會等你來電　不會給他發現",
    "就算你為兒女改名　我都會給意見"
  ],
  [
    "我已經中了你愛情的箭",
    "我正等你為我拉開褲鏈",
    "你的眼神猶如兩支火箭",
    "眨一眨我便會飛到上天",
    "是你　是你　是你",
    "是你先可以令我沒法睡眠",
    "是你　是你　是你",
    "是你將愛的感覺再次燃點",
    "",
    "我的少男情懷已早改變",
    "但你卻會令我再紅了臉",
    "我不可以被你見太多遍",
    "我怎可以被你知我發癲",
    "是你　是你　是你",
    "是你先可以令我沒法睡眠",
    "是你　是你　是你",
    "是你將愛的感覺再次燃點",
    "",
    "是你　是你　是你",
    "是你先可以令我沒法睡眠",
    "是你　是你　是你",
    "是你將愛的感覺再次燃點",
    "",
    "One more time",
    "Whoa",
    "One more time",
    "",
    "是你　是你　是你",
    "是你先可以令我沒法睡眠",
    "是你　是你　是你",
    "是你將愛的感覺再次燃點"
  ],
  [
    "我又有心事",
    "自從看了太宰治",
    "我問誰",
    "誰都不會在意死亡的意義",
    "",
    "世界像似崩裂",
    "我倆都無可救治",
    "我但求",
    "乘風飛到別處再重新開始",
    "",
    "願你可以給我寫首詩",
    "紀念我們約會過六次（十次）",
    "",
    "我沒法想像",
    "看過的童話故事",
    "有陣時",
    "成長不過是個悲劇的開始",
    "",
    "我做過的事",
    "沒太多是有意義",
    "接近誰",
    "誰都不會快樂這些我都知",
    "",
    "願我可變歡笑的天使",
    "這樣你便願看數十次",
    "",
    "願我可變優雅的詩篇",
    "這樣你便願看數十遍"
  ],
]

// TASTE OF TEARS (LIVE)

const a12list = {
  og: [
    "你叫我譯一首德國歌詞 - Live",
    "我們一起離開吧 - Live",
    "愛情 disabled - Live",
    "露體狂小丁 - Live",
    "美麗新香港 - Live",
    "再殺一個人 - Live",
    "今天沒大麻在身 - Live",
    "西西弗斯之歌 - Live",
    "今夜雪糕 - Live",
    "宅女，上街吧 - Live",
    "失業抗爭歌 - Live",
    "牛頭角青年 - Live",
    "今宵多珍重 - Live",
    "吳小姐 - Live",
    "Ｋ同學 - Live",
    "香港香港 - Live",
    "廣州足浴一夜 - Live",
    "驗孕的下晝 - Live",
    "九龍公園游泳池 - Live",
    "詩歌舞街 - Live",
    "下了兩天雨 - Live",
    "五點鐘去天光墟 - Live",
    "失落沮喪歌 - Live",
  ],
  en: [
    "You Asked Me to Translate a German Song - Live",
    "Let's Leave Together - Live",
    "Love Disabled - Live",
    "Siu Ding The Exhibitionist - Live",
    "Brand New Hong Kong - Live",
    "Killed Off One More - Live",
    "No Weed on Me Today - Live",
    "The Hymn of Sisyphus - Live",
    "Ice Cream Tonight - Live",
    "Geeky Girls, Hit the Streets - Live",
    "Anthem of the Unemployment Resistance - Live",
    "Youths of Ngau Tau Kok - Live",
    "Farewell - Live",
    "Miss Ng - Live",
    "Classmate K - Live",
    "Hong Kong Hong Kong - Live",
    "A Night of GuangZhou Foot Massage - Live",
    "Afternoon of Pregnancy Testing - Live",
    "Kowloon Park Swimming Pool - Live",
    "Sycamore Street - Live",
    "Rained for Two Days - Live",
    "To Dawn Market at 5AM - Live",
    "The Lost and Frustration Song - Live",
  ]
}

const a12meta = [
  [
    3,8,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫,銘銘', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,11,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,18,
    'P,陳寧', 'P', 'P,N,阿賢,阿科', 'N', {
      backup_vocal: 'Karena',
      recite: 'N'
    }
  ], [
    2,55,
    '何山', 'P', '何山', 'P,N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,16,
    'P', 'P', 'P,N,阿賢,阿科,陳銘澤 @ 在草地上', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    3,37,
    'P', 'P', 'P,阿賢,阿科,夏日浪漫,Kit @ 22cats', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,11,
    'P', 'P', 'P,阿賢,阿科', 'N', {
      backup_vocal: 'Karena',
    }
  ], [
    2,1,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,23,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,19,
    "P", "P", "P", "N", {
      backup_vocal: 'Karena'
    }
  ], [
    2,28,
    'P', 'P', 'P', 'P,阿賢,Thomas,Sot Kanin', {
      backup_vocal: 'Karena'
    }
  ], [
    2,31,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena',
    }
  ], [
    3,7,
    "LMF", "LMF", "P", "N", {
      backup_vocal: 'Karena'
    }
  ], [
    3,20,
    "P", "P", "P", "N", {
      backup_vocal: 'Karena'
    }
  ], [
    3,15,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,25,
    '許日元,劉岳靈', '許日元,劉岳靈', '許日元,劉岳靈', 'N', {
      backup_vocal: 'Karena',
    }
  ], [
    2,20,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena',
    }
  ], [
    2,35,
    'P', 'P', 'P,阿賢,阿科,銘銘', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,59,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,58,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    2,52,
    'P', 'P', 'P', 'N', {
      backup_vocal: 'Karena'
    }
  ], [
    4,3,
    'P', 'P', 'P,N,阿賢,阿科', 'P,N', {
      backup_vocal: 'Karena'
    }
  ], [
    1,59,
    "P", "P", null, "N", {
      backup_vocal: 'Karena'
    }
  ]
]

const a12lyrics = [
  [
    "你叫我幫你譯一首德國歌詞",
    "並沒有說是為什麼事",
    "那首歌說向外面世界奔馳",
    "渴望那一天將至",
    "",
    "祈求風和雨",
    "吹我到理想的遠處",
    "故土沒法跟隨我意願",
    "但為什麼終於",
    "穿過海灣來到老遠",
    "卻很想返回我的屋邨",
    "",
    "你叫我幫你譯一首德國歌詞",
    "像在訴說著你心願",
    "拍拖　出國　交友　工作　生兒",
    "難道不是為逃避寂寞不斷？",
    "",
    "祈求風和雨",
    "吹我返故鄉的某處",
    "異國沒法消除我睏倦",
    "但為什麼終於",
    "返到故鄉鞋也未轉",
    "卻很想回去我的屋邨",
    "但為什麼終於",
    "穿過海灣來到老遠",
    "卻很想返回我的屋邨"
  ],
  [
    "(Ah-ah ah-ah ah-ah ah-ah)",
    "(Ah-ah ah-ah ah-ah ah-ah)",
    "",
    "我想變做個木偶",
    "我想變做個石頭",
    "我想變做山丘",
    "我想變做小狗",
    "",
    "點先可以沒憂愁？",
    "點先可以沒淚流？",
    "點先可以沒內疚？",
    "點先可以自由？",
    "",
    "(Ah-ah ah-ah ah-ah ah-ah)",
    "(Ah-ah ah-ah ah-ah ah-ah)",
    "",
    "我想帶你去歐洲",
    "我想帶你去月球",
    "想帶你環遊宇宙",
    "會否把生命看透？",
    "",
    "那天你睡著顫抖",
    "我的心裡在淚流",
    "上天為何不保佑？",
    "為什麼要我們接受？",
    "",
    "(Ah-ah ah-ah ah-ah)",
    "",
    "我家不在這星球",
    "太深業力的地球",
    "每當我望見星宿",
    "我都不想逗留"
  ],
  [
    "當我向政府申請津貼時職員看著我然後",
    "問我申請津貼到底是基於一個什麼的理由",
    "(love disabled)",
    "當我搭巴士向司機展示我張優惠證時候",
    "司機看著我彷彿想問我究竟是什麼因由",
    "love disabled  love disabled",
    "love disabled  love disabled",
    "",
    "當我想找一個好友得到心靈上的解救",
    "總是沒有一個可以找得到我心靈的入口",
    "(love disabled)",
    "當我見到戲院的觀眾開始投入到淚流",
    "我都想知道故事有什麼值得大家憂愁",
    "love disabled  love disabled",
    "love disabled  love disabled",
    "",
    "這個世界是否真的有這種 love disabled？",
    "心裡的這個問號 誰又能解答得到？",
    "",
    "love disabled  love disabled",
    "love disabled  love disabled",
    "",
    "耶穌說的愛是無條件的、獻身的，",
    "奧修說的愛是能量的互動、是自由的、無束縛的，",
    "昆德拉說的愛是機遇的、偶然的、命定的，",
    "高達說的愛是刺激的、好玩的、有今生沒來世的、哲學的，",
    "(love disabled  love disabled)",
    "小津安二郎說的愛是溫柔的、隱藏的、非愛的，",
    "(love disabled  love disabled)",
    "畢卡索說的愛是經驗的、性慾的、美好的，",
    "夏卡爾說的愛是聖潔的、救贖的、唯一的。",
    "L'amour, mes amants, mon amour, aimer.",
    "愛情，愛人們，我的愛，去愛。",
    "而我將要說的是,",
    "L'impossibilité d'aimer dans notre temps.",
    "我們時代的愛無能。"
  ],
  [
    "(Ooh ooh ooh ooh)",
    "(Ooh ooh ooh ooh)",
    "(Ooh ooh ooh ooh)",
    "(Ooh ooh ooh ooh)",
    "",
    "人們說你要離開這陣營",
    "我們無不覺得吃驚",
    "因為這不是你的個性",
    "你生來是露體狂小丁",
    "",
    "我都知道我們不是很有型",
    "我都知道我們不年青",
    "這一場革命",
    "最終無人取勝",
    "但請你（請你）",
    "請你　留低一起作見證",
    "(Ooh ooh ooh ooh)",
    "(Ooh ooh ooh ooh)",
    "(Ooh ooh ooh ooh)",
    "(Ooh ooh ooh ooh)",
    "",
    "這裡的人最終都不會明",
    "世界永沒法安靜",
    "我們還有多少酒剩",
    "給你露體狂小丁",
    "",
    "我都知道我們不是很有型",
    "我都知道我們不年青",
    "這一場革命",
    "最終無人取勝",
    "但請你（請你）",
    "請你　留低一起作見證",
    "(Ooh ooh ooh ooh)",
    "(Ooh ooh ooh ooh)",
    "(Ooh ooh ooh ooh)",
    "(Ooh ooh ooh ooh)",
  ],
  [
    "自你決定要走以後",
    "沒人知我有多難受",
    "再沒勇氣在派對逗留",
    "踫見朋友不敢問候",
    "",
    "自你遠去不再回頭",
    "青山綠水不再依舊",
    "再沒有早起身的理由",
    "再沒可安睡的枕頭",
    "",
    "這世界只有一種鄉愁",
    "就是沒有你的時候",
    "這香港已不是我的地頭",
    "就當我在外地旅遊",
    "",
    "自美夢再不可尋求",
    "我便學會清風兩袖",
    "命運要令我學識奮鬥",
    "但我只懂得荒謬",
    "",
    "這世界只有一種鄉愁",
    "是你不在身邊的時候",
    "這香港已不是我的地頭",
    "就當我在外地飄流",
    "",
    "我知已走到盡頭",
    "為何還要擔憂？",
    "這世界也不是我的地頭",
    "就當我在宇宙飄流"
  ],
  [
    "有人話世間所有感情最終都會走向末路",
    "最好可以死喺三十七歲之前或者更早",
    "有理想嘅文青到某個階段開始變得深奧",
    "簡單嘅字句要有學術字眼先表達到",
    "每個作家都覺得自己嘅作品最純粹最樸素",
    "市面賣得好嘅一般都有危害社會嘅元素",
    "無人會諗自己嘅才能係咪需要進步",
    "但總會覺得自己嘅地位有待提高",
    "",
    "尤其是你為什麼可以這樣做",
    "你都知道我曾對你很好",
    "但我從不會表現憤怒",
    "某天先知道碰到你眼也不想一掃",
    "",
    "我想生活喺一千年前嗰陣時未有電腦",
    "唯一唔捨得嘅係冬天嘅時候無熱水爐",
    "近年我嘅生活開始無咁刺激無咁虛無",
    "以前深夜我會打俾人講我殺咗人一時糊塗",
    "但第日完全唔記得做乜我會咁做",
    "我懷念嗰陣時每日每夜都好 emo",
    "你係我嘅青春期永遠唔會再接觸到",
    "兩個人就咁永遠疏離都無話好定唔好",
    "",
    "尤其是你為什麼可以這樣做",
    "你都知道我曾對你很好",
    "但我從不會表現憤怒",
    "某天先知道碰到你眼也不想一掃",
  ],
  [
    "我的煩惱不是愛人對我不夠真",
    "亦不是她對我流露蔑視的眼神",
    "No, non, いいえ",
    "不是朋友逐一結婚",
    "而是今天沒大麻在身",
    "而是今天沒大麻在身",
    "",
    "我的煩惱不是沒有照顧好父親",
    "亦不是看見他（變成衰老的人）",
    "No, non, いいえ",
    "不是憂慮無處棲身",
    "而是今天沒大麻在身",
    "而是今天沒大麻在身",
    "",
    "No, non, いいえ",
    "不是我沒變 gentleman",
    "No, non, いいえ",
    "不是沒學好法文",
    "No, non, いいえ",
    "不是選舉不知投哪人",
    "而是今天沒大麻在身",
    "",
    "(No, non, いいえ)",
    "(No, non, いいえ)",
    "",
    "我的煩惱不是每日起身就眼瞓",
    "亦不是想找人瞓已沒有人肯",
    "No, non, いいえ",
    "不是愛上朋友的女人",
    "而是今天沒大麻在身",
    "而是今天沒大麻在身",
    "而是今天沒大麻在身"
  ],
  [
    "呢年嚟我有幾份兼職　其中嘅一份",
    "係幫啲馬迷落注　喺電話投注中心",
    "喺裡面我識唔到朋友　返工食飯都係一個人",
    "我已經悶到抽筋　但要維生",
    "我諗到一個方法抗衡",
    "",
    "開閘前半分鐘　總有好急嘅客人",
    "落注嘅時侯夾雜粗口　聽到我一嚿雲",
    "但佢哋愈係忟　我就愈係斯文",
    "我話：「麻煩你重複一次」",
    "搞到夠鐘開閘　佢就問候我屋企人",
    "",
    "佢鬧我正一懵撚　話要叫個經理出嚟問",
    "我都好有禮貌咁回應：「先生，麻煩你等等」",
    "我既緊張又興奮　同時又扮晒殷勤",
    "喺呢個心情咁複雜嘅搏鬥裡面",
    "我開始搵到工作嘅快感",
    "",
    "希臘神話有個故事",
    "講述西西弗斯受到諸神嘅懲罰",
    "要喺地獄不斷推一塊巨石上山",
    "上到山頂　塊巨石又會自己碌返落山腳",
    "佢每日都要重複呢一種徒勞無功嘅工作",
    "直到永恆",
    "",
    "後人有一個講法",
    "話諸神並唔係用「推石頭」嚟懲罰西西弗斯",
    "而係用觀念",
    "用「我永世都要推石頭實在太慘」嘅呢個觀念",
    "西西弗斯知道自己改變唔到命運",
    "佢唯一可以做嘅　就係繼續推石頭",
    "直到有一日",
    "佢發現佢可以蔑視呢嘅命運",
    "甚至用享受呢個過程嚟去否定諸神對佢嘅懲罰",
    "於是　佢感覺到自己係快樂嘅"
  ],
  [
    "何も言うことはない",
    "Non ho niente da dire",
    "Ich habe nichts zu sagen",
    "No tengo nada que decir",
    "Je n'ai rien à dire",
    "",
    "有時很懊惱",
    "有時也感到驕傲",
    "有時不後悔曾這樣表達憤怒",
    "",
    "今夜我可以吃更多雪糕",
    "想像明天中午會被捕",
    "",
    "還有些心願未做",
    "還未去埃及希臘秘魯",
    "還未跟你曾相處得更好",
    "",
    "外面進行著的夜",
    "無窮的遠方",
    "無數的人們",
    "都和我有關",
    "",
    "今夜我可以走八千里路",
    "想像明天會特赦全部",
    "今夜我可以",
    "想像明天中午"
  ],
  [
    "你錯過了反高鐵",
    "你錯過了保衛天星碼頭",
    "你錯過了包圍禮賓府",
    "你錯過了反世貿",
    "",
    "宅女　上街吧",
    "這個政府靠得過嗎？",
    "宅女　上街吧",
    "你要未來繼續任由人魚肉嗎？",
    "",
    "不要再說你沒有仔吼",
    "你對你的生活都沒有要求",
    "你只關心工作已經不夠",
    "你本來可以結識一個俊俏的八十後",
    "趁你現在還可以補救",
    "不要再信媒體中的主流",
    "佢哋只會著眼肢體的爭鬥",
    "不會理會暴力的政策才是因由",
    "",
    "宅女　上街吧",
    "這個政府靠得過嗎？",
    "宅女　上街吧",
    "功能組別還不夠醜惡嗎？",
    "",
    "我們已錯過了美好的六十年代",
    "錯過了胡士托",
    "反戰學運的精彩",
    "現在理想主義已逐漸歸來",
    "但你為何還不變改？",
    "",
    "你錯過了反高鐵",
    "我錯過了保衛天星碼頭",
    "有時我也會很悔疚",
    "但願你的醒覺",
    "不會是在多年以後"
  ],
  [
    "來到世上的意義是為了抗衡",
    "為了正義必要時犯法抗爭",
    "這是場戰爭但勝利是不可能",
    "犯法為彰顯法制不公允",
    "",
    "全世界也在欺壓著社會低下層",
    "年過五十低技能無法翻身",
    "大財團最終亦會炒哂你啲人",
    "小政府唔會理你幾窮困",
    "",
    "來到最後只有最後的搖滾精神",
    "我們有四五六七八九十個人",
    "這是場戰爭但勝利是不可能",
    "犯法為彰顯法制不公允"
  ],
  [
    "牛頭角都玩厭了",
    "我們還可以去邊？",
    "全世界都有暴動的青年",
    "但香港幾時先出現？",
    "",
    "再過春天　再過秋天",
    "這裡都不會改變",
    "或者永遠都不變",
    "如果我們都只願做旁觀的青年",
    "",
    "牛頭角的日出都看厭",
    "時間不站在你身邊",
    "沉迷過的樂隊解散了",
    "還記得穿橙色衫的阿 Tim",
    "",
    "「在吐煙」　「在發癲」",
    "首歌叫〈究竟應該點？〉",
    "事隔差不多二十年",
    "不知你是否已不是個「討厭的青年」？",
    "",
    "而我想寫首法文歌叫「Jamais」",
    "「永遠不會」的意思",
    "« Jamais je ne t'ai dit que je t'aimerai toujour »",
    "",
    "過了秋天　再過冬天",
    "我都回不到你身邊",
    "然後這一首歌的最尾一句叫「adieu」",
    "意思是「永遠的再見」"
  ],
  [
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又未必會係噉",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又另外一個樣",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又未必會係噉",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又另外一個樣",
    "",
    "你有冇埋怨自己生出嚟嗰陣時",
    "唔係含住條金鎖匙　係一條生銹鎖匙",
    "你老豆冇本事　冇錢養妻活兒",
    "你呀嫲就啋　你個衰仔",
    "生你出嚟都係多餘　唔該",
    "你要記住人生嘅大門係要靠你雙手去打開",
    "掂嗰陣時就唔駛見風駛盡𢃇",
    "耷嗰陣時唔好怨天怨地",
    "",
    "Unprecedented though this moment in history may be",
    "We have the utmost confidence in the abilities",
    "And resilience of the Hong Kong people",
    "Britain learnt long ago",
    "That Hong Kong people know best",
    "What is good for Hong Kong",
    "",
    "可以試下去諗條路唔係一定要噉走",
    "要壞嘅話唔一定要食煙飲酒",
    "唔係一定要耷低頭做人隻狗",
    "可以抬起頭自己行每一步路",
    "唔駛畀人睇死你係一個冇用嘅細路",
    "自己去做做到自己就當然最好",
    "做唔到都千祈唔好放棄自己",
    "鎅手跳樓飲滴露蕩失路",
    "",
    "It has been the greatest honour and privilege of my life",
    "To share your home for five years",
    "And to have some responsibility for your future",
    "Now Hong Kong people are to run Hong Kong",
    "That is the promise and that is the unshakable destiny",
    "",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又未必會係噉",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又另外一個樣",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又未必會係噉",
    "今宵多珍重（今宵多珍重）",
    "今宵多珍重　聽日又另外一個樣",
  ],
  [
    "吳小姐畢業之後",
    "做咗銀行十幾年",
    "最近佢單身",
    "罷工嗰日自己一個去彌敦道",
    "黃昏嘅時候",
    "成條馬路都係黑衫嘅年輕人行嚟行去",
    "尖沙咀同其他區已經放咗催淚彈",
    "佢一路行",
    "一路睇吓 live",
    "聽吓條街啲人講點部署",
    "",
    "無幾耐天開始黑",
    "行到豉油街",
    "經過兩間日本嘢",
    "佢都好想入去食住涼冷氣",
    "不過咁好似幫襯咗無罷市嘅食店",
    "最後佢猶豫咗一陣",
    "都係喺小販檔買咗兩個桔返示威區",
    "坐喺十字街口嘅馬路邊",
    "一路食一路有啲涼風",
    "",
    "就在她坐下的那個瞬間",
    "她竟有前所未有的嘆",
    "",
    "食食吓有人問佢要唔要水",
    "有人問佢去唔去深水埗警署聲援",
    "有個女仔指住太子方向",
    "話一直行就會到",
    "吳小姐走咗幾步覺得太遠",
    "都係搭地鐵",
    "",
    "去到現場",
    "有人叫佢唔好行咁前",
    "然後 佢第一次",
    "親眼見到催淚彈",
    "佢跟住大家",
    "向後褪　開遮　踎低　行返前",
    "來來回回",
    "幾粒催淚彈之後",
    "佢有種從未試過嘅感覺",
    "",
    "時間突然好慢",
    "周圍好靜",
    "佢覺得呢個瞬間",
    "最前線嘅人係自己",
    "後面用士巴拿拆緊鐵欄嘅人又係自己",
    "旁邊鬧緊街坊唔好影相嘅人又係自己",
    "遠處坐緊監嘅人又係自己",
    "",
    "無幾耐大家話轉場去另一個警署",
    "吳小姐坐喺紅綠燈下面食咗支煙",
    "",
    "就在她坐下的那個瞬間",
    "她竟有說不出的感嘆",
    "",
    "最後防暴衝出嚟",
    "吳小姐狂跑嘅時候",
    "竟然跑得快過晒所有嘅人",
    "佢決定　以後都出嚟"
  ],
  [
    "如果可以重來　到太子午夜時候",
    "便會拖著你手不會似中學生那麼害羞",
    "你愛將警察當做背景自拍十張才夠",
    "路過的夫婦以為你來自韓國到此旅遊",
    "",
    "士多老細收錯錢說幾晚不夠人手",
    "酒吧街邊吧女拚命遊說我們飲啤酒",
    "",
    "望向星星是哪星球　它突然間飄走",
    "懷疑是航拍後　你竟除下口罩向它揮手",
    "問我想有什麼成就　我本不想活太久",
    "何以見到你後　我竟然會習武鍛鍊身手",
    "",
    "放狗的途人照舊　牠擒向另一女生膝頭",
    "圍觀到防暴對峙時也行近看下又走",
    "女生說家有三隻母狗　我說她那味散四周",
    "群眾都起哄話我　還要是你帶頭",
    "",
    "著拖鞋紅衫短袖　手執燒賣配清酒",
    "願這街坊裝被捕可免受暴動罪追究",
    "",
    "望向星星是哪星球　它突然間飄走",
    "懷疑是航拍後　你竟除下口罩向它揮手",
    "問我想有什麼成就　我本不想活太久",
    "何以這晚之後　我竟想去習武保衛地球",
    "",
    "何以見到你後　我竟然會習武鍛鍊身手"
  ],
  [
    "北角　再到上環",
    "無論去哪裏",
    "我內裏有種渴望",
    "有說話我要去講",
    "",
    "我會說",
    "香港",
    "還未到對岸",
    "船上再跌盪",
    "再去走一趟",
    "",
    "大會堂",
    "昔日海港",
    "怎可會被困於一角　太寂寞",
    "決定最終相信直覺",
    "",
    "望上天",
    "雲層沒有形狀",
    "期待盼望",
    "似轉空檔",
    "",
    "望上天",
    "雲層沒有形狀",
    "旗在飄盪",
    "看見它升降",
    "",
    "我會說",
    "香港",
    "還未到對岸",
    "隨時受到踫撞",
    "你我要抵抗",
    "",
    "潮汐海浪",
    "驟雨輕輕的降",
    "別停別退後別回望",
  ],
  [
    "人生是什麼？",
    "我今晚不知道喎",
    "只想你不要難過",
    "甚麼都不算甚麼",
    "",
    "明天我不會來過",
    "快樂不捨得用太多",
    "只知我今晚如何過",
    "明天你已忘記我",
    "",
    "你話你未離開過廣州",
    "你話你未見過海",
    "你問香港係唔係好繁榮",
    "你問香港嘅海係點",
    "你話舊年返鄉下種田之後覺得",
    "人應該留喺城市",
    "我話香港有啲年青人開始學種田",
    "你話香港咁繁榮",
    "點會有人去種田",
    "",
    "努力又是什麼？",
    "如果目標已經出錯",
    "這裡有沒有人清楚",
    "越努力越錯得多",
    "",
    "人生是什麼？",
    "我今晚不知道喎",
    "這裡有沒有人可",
    "將一切告訴我"
  ],
  [
    "當我知這一次或會改變我前程",
    "應要繼續我路拒絕命運的邀請",
    "或已到某個階段應該順應天命",
    "為你的到來而高興",
    "",
    "想你應將得到眾多姨姨的垂青",
    "想你會使我努力　為這關係打拼",
    "縱是我未認清你母親的個性",
    "但婚姻無關愛情反正",
    "",
    "不會責備你若你什麼都跟我傾",
    "包括青春期會遇到的各種類型",
    "世界要你努力去考取功名",
    "但是真誠才是最大本領",
    "",
    "總會有很多事令你失去熱情",
    "總在青春過後留不住舊髮型",
    "只願驗孕結果能在此刻呈陰性",
    "不負如來不負卿"
  ],
  [
    "我喜歡九龍公園游泳池",
    "那個戲水池有個瀑布位置",
    "瀑布下站著能忘記煩惱事",
    "每個星期我都會去一次",
    "",
    "我喜歡九龍公園游泳池",
    "那裡我不再執著一些往事",
    "我原是世間其中的粒子",
    "如何沖擊我都可以",
    "",
    "就在那時我變得不再幼稚",
    "就在那時我想重新開始",
    "二百年後這裡什麼也都不是",
    "宇宙裡有什麼不是暫時？",
    "",
    "就在我要離開這個瀑布時",
    "我突然游得更加輕易",
    "晚上折射到池底的燈光很美　但是",
    "我都要離去不留戀到八時"
  ],
  [
    "那晚看 Russian Red 大角咀表演",
    "你與我竟會再遇見",
    "剛好我也到 bar 枱買酒才碰面",
    "完場與你去深水埗那邊",
    "剛過身的作家你介紹那年",
    "寫過只有散步我們才真正聊天",
    "",
    "再次聽你說故事",
    "聽你說剛生的嬰兒",
    "詩歌舞街地上有著光點閃閃",
    "聽你說外地歷險",
    "我卻像當天的少年",
    "但已是能接受你所有改變",
    "",
    "你說你最近愛聽古典",
    "你說你父母開始婚變",
    "邊聽邊講我忘了怎去 99 咖啡店",
    "說到我倆澳門過生日那年",
    "你為我訂了山上的酒店",
    "餐廳老闆對你彈唱結他一整天",
    "",
    "再次聽你說故事",
    "聽你說剛生的嬰兒",
    "走進萬發坐在暗光卡位窗邊",
    "聽你說外地歷險",
    "我卻像當天的少年",
    "但已是能控制我所有淚腺"
  ],
  [
    "像是太久不見",
    "久得不太自然",
    "像是某種訓練",
    "又過了數十天",
    "(Ah-ah)",
    "久得不太自然",
    "(Ah-ah)",
    "",
    "或是這種陰天",
    "令這愁緒浮面",
    "發現太深的掛念",
    "那是無可避免",
    "(Ah-ah)",
    "那是無可避免",
    "(Ah-ah)",
    "",
    "若我知你不想見",
    "像我那樣多",
    "那又有何不可？",
    "若你不知不覺生厭",
    "也未嘗不過 (不過）",
    "是最好的結果",
    "痛苦只我一個（一個）",
    "Ah-ah",
    "痛苦只我一個（一個）",
    "Ah-ah",
    "那又有何不可？",
    "那又有何不可？"
  ],
  [
    "do do do...",
    "",
    "我最近有一個夢想",
    "係同朋友五點鐘去天光墟",
    "我經已好耐無同朋友出去",
    "而且去一個未去過嘅地區",
    "",
    "但我點可能五點鐘就起身出去？",
    "除非我十點鐘開始入睡",
    "但香港有無人可以十點鐘入睡？",
    "除非喺屋企無端端飲醉",
    "",
    "Terence、阿雪、Peter 同阿 Cza",
    "你哋會陪我去嗎？",
    "之後我哋再去飲早茶",
    "你哋都應該會陪我啩",
    "",
    "do do do...",
    "",
    "但我最大嘅一個夢想",
    "其實已一早失去",
    "香港都正在死去",
    "我都經已不再唏噓",
    "",
    "世界叫人類堅持落去",
    "世界叫窮人奮鬥落去",
    "但我淨係可以繼續喺度",
    "無聊落去",
    "",
    "我有一隻公仔",
    "兩年嚟同我朝夕相對",
    "我有段日子唔想出街",
    "主要都係因為佢",
    "",
    "但我最近發現",
    "佢唔再講嘢食嘢同埋飲水",
    "而我亦都開始瞓唔著",
    "就算飲到好醉",
    "",
    "do do do...",
    "",
    "我諗緊好唔好等到五點鐘",
    "然後就去天光墟",
    "我諗緊瞓唔著係咪因為",
    "以前做過一啲事太衰",
    "",
    "我起返身聽鄧麗君",
    "聽佢唱到呢幾句：",
    "「不知天上宮闕 今夕是何年",
    "我欲乘風歸去……」",
    "",
    "do do do..."
  ],
  [
    "(I love you too)",
    "",
    "我又有心事",
    "自從看了太宰治",
    "我問誰",
    "誰都不會在意死亡的意義",
    "",
    "世界像似崩裂",
    "我倆都無可救治",
    "我但求",
    "乘風飛到別處再重新開始",
    "",
    "願你可以給我寫首詩",
    "紀念我們約會過六次",
    "",
    "我沒法想像",
    "看過的童話故事",
    "有陣時",
    "成長不過是個悲劇的開始",
    "",
    "我做過的事",
    "沒太多是有意義",
    "接近誰",
    "誰都不會快樂這些我都知",
    "",
    "願我可變歡笑的天使",
    "這樣你便願看數十次",
    "",
    "(I love you, take care)"
  ],
]

// singing in the telephone

const lka1list = {
  og: [
    "debbie, don't sing this song alone",
    "josephine's shop",
    "柯德莉，關於風景",
    "victor, fly me to stafford",
    "coka, i'm fine",
    "伊，關於眉"
  ],
  en: [
    "Debbie, Don't Sing This Song Alone",
    null,
    null,
    null,
    null,
    "Thou, About Eyebrows"
  ],
}

const lka1meta = [
  [
    0,0,
    'P', 'P,N,Josephine,Jerry,Tim', 'P', 'N', null
  ],
  [
    0,0,
    'P', 'P', 'P', 'N', null
  ],
  [
    0,0,
    'P', 'P,N', 'P', 'N', null
  ],
  [
    0,0,
    'P', 'P', 'P', 'N', null
  ],
  [
    0,0,
    'P', 'P', 'P', 'N', null
  ],
  [
    0,0,
    'P', 'P', 'P', 'N', null
  ]
]

const lka1lyrics = [
  [
    "Where are you?",
    "Oh my dear Debbie",
    "Don't you know me?",
    "I am your little P",
    "Yeah...",
    "",
    "I dance alone",
    "You just stand near Jerry",
    "Never give me fantasy",
    "Oh my darling Debbie",
    "",
    "Oh my Debbie, can you imagine",
    "If one day I disappear",
    "And will you cry for me?",
    "Will you miss me?",
    "",
    "Oh my Debbie, you make me crazy",
    "If you still talk with Jerry",
    "I could not sleep",
    "How could I sleep?",
    "",
    "Na…",
    "",
    "Oh my Debbie, Jerry laughed at me",
    "He said that I was in a dream",
    "Do you think it's a dream to me?",
    "Oh my dear Debbie",
    "",
    "I have a dream",
    "Holding hands with Debbie",
    "Kissing her lips",
    "Touching her skin",
    "",
    "Where are you?"
  ],
  null,
  null,
  null,
  null,
  [
    "如果你過份缺乏集體精神",
    "我會陪你離棄日日遠飛",
    "如果你每夜缺乏合適睡眠",
    "我會讓你抱著　抱着我的唱機",
    "好風光明媚",
    "播着陳綺貞的悠悠長假期",
    "",
    "除非你渴望是一雙粗的眉",
    "我永遠也沒花讓你驚喜",
    "難保某日發揮出犧牲精神",
    "我懶理不理　會決心劫機",
    "法官不留餘地　大會堂處死",
    "那天風光明媚",
    "",
    "我會在死前串通護衞種植著一雙粗的眉",
    "一雙粗的眉",
    "再去被分成碎片滴著血肉地",
    "給刀槍分裂　給玻璃分割",
    "法西斯的美麗遇著希治閣"
  ]
]

// when i listen to the field mice

const lka2list = {
  og: [
    'When I Listen To The Field Mice',
    'When I Listen To The Field Mice (Catalog Mix)'
  ],
  en: [
    'When I Listen To The Field Mice',
    'When I Listen To The Field Mice (Catalog Mix)'
  ],
}

const lka2meta = [
  [
    2,40,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,30,
    'P', 'P', 'P', 'N', null
  ]
]

const lka2lyrics = [
  [
    "Still remember the songs of the field mice",
    "You used to sing to me at night",
    "Sometimes I think we can be together",
    "You make me feel love's like flowers",
    "I have a catalogue of everything we did",
    "I still remember everything you need",
    "I wanna write this love song for you",
    "I wanna tell you how I love you",
    "",
    "'Cause you",
    "You deserve a major 7 chord",
    "And I",
    "I just can't stop feeling so lost",
    "",
    "Remember once me and you're in a bus",
    "I kissed your lips and no one saw us",
    "I didn't know where the bus's going",
    "I didn't know where we are going",
    "You told me you like major 7",
    "So I start writing songs with these chords",
    "But don't you know these chords're about love",
    "Which will never happen between us",
    "",
    "'Cause you",
    "You deserve a major 7 chord",
    "And I",
    "I just can't stop feeling so lost",
    "",
    "So I use an E minor chord",
    "After a D major 7",
    "So I use an E minor chord",
    "To stop myself feeling lost"
  ],
  [
    "Still remember the songs of the field mice",
    "You used to sing to me at night",
    "Sometimes I think we can be together",
    "You make me feel love's like flowers",
    "I have a catalogue of everything we did",
    "I still remember everything you need",
    "I wanna write this love song for you",
    "I wanna tell you how I love you",
    "",
    "'Cause you",
    "You deserve a major 7 chord",
    "And I",
    "I just can't stop feeling so lost",
    "",
    "Remember once me and you're in a bus",
    "I kissed your lips and no one saw us",
    "I didn't know where the bus's going",
    "I didn't know where we are going",
    "You told me you like major 7",
    "So I start writing songs with these chords",
    "But don't you know these chords're about love",
    "Which will never happen between us",
    "",
    "'Cause you",
    "You deserve a major 7 chord",
    "And I",
    "I just can't stop feeling so lost",
    "",
    "So I use an E minor chord",
    "After a D major 7",
    "So I use an E minor chord",
    "To stop myself feeling lost"
  ]
]

// gi gi leung is dead

const lka3list = {
  og: [
    'gi gi leung is dead',
    'washide',
    '你的微笑像朵花 (guitar version)',
    'dee, it may all end tomorrow (guitar version)'
  ],
  en: [
    null,
    'Washide',
    "You Smile Like A Blossom (Guitar Version)",
    'Dee, It May All End Tomorrow (Guitar Version)'
  ]
}

const lka3meta = [
  [
    1,16,
    'P', 'P', 'P', 'N', null
  ],
  [
    1,29,
    'P', 'P', 'P', 'N', null
  ],
  [
    2,43,
    'P', 'P', 'P', 'P', null
  ],
  [
    1,19,
    'P', 'P', 'P', 'N', null
  ]
]

const lka3lyrics = [
  null,
  // [
  //   "Shing shung wolowoshi madine",
  //   "Hung gu wo lu wo du wadero",
  //   "Shing shung wa nan de na woshibe",
  //   "Shing shung la wan",
  //   "Wala washide",
  //   "Shing shung wolowoshi madine",
  //   "Hung gu wo lu wo du wadero",
  //   "Shing shung wa nan de na woshibe",
  //   "Shing shung la wan",
  //   "Wala washide",
  //   "Shing shung la wan",
  //   "Wala washide",
  //   "Shing shu la wan le wa le wa le do",
  //   "Shing shu la wan"
  // ],
  null,
  [
    "已經過了兩星期",
    "沒有與你食雪梨",
    "我與你確係離奇",
    "年紀相差五十里",
    "",
    "那天你要去外地",
    "剩下我獨自皺眉",
    "你的個性最迷離",
    "讓我不可以忘記",
    "",
    "願明日能再見",
    "你迷人的嘴臉",
    "我現時就快要",
    "找你對住我笑",
    "",
    "你知嗎　你最迷人",
    "是你回望帶笑眼神",
    "那刻已被你迷魂",
    "望你可給我熱吻",
    "",
    "可惜我並沒有運",
    "看著你我會頭暈",
    "怎可以做你情人",
    "為了你甘願受困",
    "",
    "願明日能再見",
    "你迷人的嘴臉",
    "我現時就快要",
    "找你對住我笑",
    "",
    "願明日就會見",
    "你迷人的嘴臉",
    "我現時就要叫",
    "想起你就要笑"
  ],
  [
    "But how can I forget?",
    "But how can I forget your tender smile?",
    "But how can you forgive?",
    "But how can you forgive my unstable mind?",
    "One day we may meet again",
    "One day we may kiss again very splendid",
    "One day you may get married",
    "One day I may be buried, very smiley"
  ]
]

// Zoo is sad, people are cruel
// https://elefant.com/album/5819/zoo-is-sad-people-are-cruel

const lka4list = {
  og: [
    "Edward, Had You Ever Thought That the End of the World Would Come On 20.9.01?",
    "在動物園散步才是正經事",
    "Gi Gi Leung Is Dead",
    "Victor, Fly Me to Stafford",
    "Leo, Are You Still Jumping Out of Windows in Expensive Clothes?",
    "I Don't Know How to Download Good AV Like Iris Does",
    "你的微笑像朵花",
    "Josephine's Shop",
    "When I Listen To The Field Mice",
    "山頂．公仔．波板糖",
    "My Little Banana",
    "只因當時太緊張",
    "You Don't Wanna Be My Girlfriend, Phoebe",
    "Dee, It May All End Tomorrow"
  ],
  en: [
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    "Mountaintop, Doll, Lollypop",
    null,
    null,
    null,
    null,
  ]
}

const lka4meta = [
  [
    2,19,
    "P", "P", null, "N", null
  ],
  [
    2,17,
    "P", "P", null, "N", null
  ],
  [
    1,18,
    "P", "P", null, "N", null
  ],
  [
    2,16,
    "P", "P", null, "N", null
  ],
  [
    2,12,
    "P", "P", null, "N", null
  ],
  [
    2,28,
    "P", "P", null, "N", null
  ],
  [
    2,40,
    "P", "P", null, "N", null
  ],
  [
    2,48,
    "P", "P", null, "N", null
  ],
  [
    2,40,
    "P", "P", null, "P,N", null
  ],
  [
    2,17,
    "P,AMK", "P", "P,AMK", "P,N", null
  ],
  [
    2,1,
    "N", "馬克斯（Marcus）", null, "N", null
  ],
  [
    2,37,
    "P", "P", null, "N", null
  ],
  [
    2,20,
    "P", "P", null, "N", null
  ],
  [
    1,46,
    "P", "P,N", null, "N", null
  ]
]

const lka4lyrics = [
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  [
    "斷斷續續點點感覺你的臉在我心裏打轉",
    "日日夜夜都想跟你到山頂遠看風景",
    "我送你的公仔在你房中暗凝視著你",
    "偶爾碰到的目光真的竟能弄碎了波板糖",
    "怎知你渴望快遠走",
    "心中佈滿幻化虛構",
    "不知哪日會可了解",
    "hey hey hey hey",
    "",
    "斷斷續續點點感覺你的臉在我心裏打轉",
    "日日夜夜都想跟你到山頂遠看風景",
    "我送你的公仔在你房中暗凝視著你",
    "偶爾碰到的目光真的竟能弄碎了波板糖",
    "怎知你渴望快遠走",
    "心中滲透幻化虛構",
    "不知哪日會可了解",
    "hey hey hey hey", 
  ],
  null,
  null,
  null,
  null,
]

// Stay As Sweet As You Are

const lka5list = {
  og: [
    "debbie, don't send this song to others",
    "dee, it may all end tomorrow",
    "dolly and bb will not wake up anymore",
    "edward, had you ever thought that the end of the world would come on 20.9.01?",
    "frankie wants to be alone",
    "gi gi leung is dead",
    "i don't know how to download good av like iris does",
    "白田購物中心",
    "japan 實瓜",
    "je pense à toi",
    "josephine's shop",
    "leo, are you still jumping out of windows in expensive clothes?",
    "my little banana",
    "my little k and j",
    "victor, fly me to stafford",
    "when i listen to the field mice",
    "you don't wanna be my girlfriend, phoebe",
  ],
  en: [
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
    null,
  ]
}

const lka5meta = [
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
]

const lka5lyrics = [
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
  null,
]

// Hum Ching for You

const colablist = {
  og: [
    "浪漫九龍塘",
    "浪漫九龍城",
    "公司裡的愛琴海",
    "畢業變成失業",
    "阿煩",
    "在動物園散步才是正經事",
    "離開是為了回來",
    "悲傷的採購",
    "when the party is over, i miss my dear pornstar (demo)",
    "see you there"
  ],
  en: [
    null,
    "Romantic Kowloon City",
    "Love Sea in The Office",
    "Graduation Equals Jobless",
    "Ah Fan",
    "The OK Thing to Do on Sunday Afternoon Is to Toddle in the Zoo",
    "Leaving Is for Coming Back",
    "Sad Purchase",
    "When The Party Is Over, I Miss My Dear Pornstar (demo)",
    "See You There",
  ],
}

const colabmeta = [
  null,
  [
    2,28,
    "P", "林一峰", "林一峰", "林一峰"
  ],
  [
    1,45,
    "林一峰", "林一峰", "P", "N,P,Sot Kanin"
  ],
  [
    2,20,
    "P", "P", "林一峰", "林一峰,P,N", null
  ],
  [
    1,41,
    "林一峰", "林一峰", "P", "P,N", null
  ],
  [
    3,1,
    "P", "P", "林一峰", "林一峰", null
  ],
  [
    3,17,
    "林一峰", "林一峰", "P", "P,N", null
  ],
  [
    3,2,
    "P", "P", "林一峰", "林一峰", null
  ],
  [
    1,27,
    "P", "P", "P", "P", null
  ],
  [
    2,32,
    "林一峰", "林一峰", "林一峰", "林一峰", null
  ]
]

const colablyrics = [
  null,
  [
    "空姐的化妝很勁",
    "同行機師制服也很型",
    "黑與白與印巴籍",
    "降落起飛滿了沖勁",
    "",
    "通過寨城講經驗",
    "每段路感覺刺激新鮮",
    "巴士多到有些亂",
    "宋王臺古跡位置不遠",
    "",
    "吃飯宵夜　又有出口店",
    "富豪酒店有好光景",
    "放學打機學會些本領",
    "就算悶　嗌飛機也很高興",
    "就算壞　始終也算生性",
    "",
    "我坐巴士路過九龍城",
    "昨日啟德已變冷清",
    "寨城消失　記憶給鏟平",
    "各路牙醫不再顯本領",
    "對著公園嘆息",
  ],
  [
    "lu lu-lu-lu lu-lu lu",
    "lu lu-lu-lu lu-lu-lu",
    "lu lu-lu-lu lu-lu lu",
    "lu-lu-lu-lu-lu-lu-lu lu-lu lu",
    "lu-lu-lu-lu lu-lu lu",
    "lu-lu-lu-lu-lu-lu-lu lu-lu lu",
    "",
    "lu-lu-lu lu-lu-lu",
    "lu-lu-lu lu-lu-lu",
    "lu-lu-lu-lu-lu-lu-lu lu-lu-lu",
    "",
    "lu lu-lu-lu lu-lu lu",
    "lu lu-lu-lu lu-lu-lu",
    "lu lu-lu-lu lu-lu lu",
    "lu-lu-lu-lu-lu-lu-lu lu-lu-lu",
    "lu-lu-lu-lu lu-lu lu",
    "lu-lu-lu-lu-lu-lu-lu lu-lu-lu",
    "",
    "lu-lu-lu lu-lu-lu",
    "lu-lu-lu lu-lu-lu",
    "lu-lu-lu-lu-lu-lu-lu lu-lu-lu",
    "",
    "lu-lu-lu lu-lu-lu",
    "lu-lu-lu lu-lu-lu",
    "lu-lu-lu-lu-lu-lu-lu lu-lu-lu",
  ],
  [
    "一畢業就等於失業",
    "一畢業就等於失業",
    "",
    "Shu-bi du-bi",
    "Shu-bi du-bi-du-ba",
    "Shu-bi du-bi",
    "Shu-bi du-bi-du-ba",
    "",
    "七個月見咗三十九份工",
    "搵份工仲難過搵個窿",
    "我今日只想留在家中",
    "Shu-bi du-bi",
    "Shu-bi du-bi-du-ba (ba-ba)",
    "Shu-bi du-bi",
    "Shu-bi du-bi-du-ba",
    "",
    "見工見到入咗去東涌",
    "點知份工要去掘山窿",
    "我今日只想留在家中",
    "",
    "(du du du du du du du du du)",
    "聽見同學逐個搵到工",
    "(du du du du du du du du du)",
    "但我仍在尋尋覓覓中",
    "(du du du du du du du du-du-du)",
    "沒有人比我更加燶",
    "(du du du du du du du)",
    "不幸正與我結伴中",
    "",
    "Shu-bi du-bi",
    "Shu-bi du-bi-du-ba (ba)",
    "Shu-bi du-bi",
    "Shu-bi du-bi-du-ba (ba-ba)",
    "",
    "嘿　又話見啱份工以為好輕鬆",
    "因為份工其實基本上係無人工",
    "我今日只想留在家中",
    "",
    "Shu-bi du-bi",
    "Shu-bi du-bi-du-ba (he)",
    "Shu-bi du-bi",
    "Shu-bi du-bi-du-ba (he)",
    "Shu-bi du-bi",
    "Shu-bi du-bi-du-ba (he)",
    "Shu-bi du-bi",
    "Shu-bi du-bi-du-ba (ba)",
    "",
    "一畢業就已經失業",
    "一畢業就已經失業",
    "一畢業就已經失業",
    "一畢業就已經失業",
  ],
  [
    "一年一年　你又再出現",
    "一年一年　你又再出現",
    "那天相見在水平線",
    "有天出現在天堂見",
    "失蹤了就最好",
    "永不相見就最好",
    "你出現不遲不早",
    "帶來無限煩惱",
    "",
    "雖然很煩　我亦愛找麻煩",
    "怎樣都煩　我自有空間",
    "那天相見在水平線",
    "有天出現在天堂見",
    "睇不到誰都好",
    "仍在風雨中趕路",
    "在地上畫滿記號",
    "有天可　再擁抱",
  ],
  [
    "我間中仍會想我們會見面",
    "在那間紅磡近黃埔的商店",
    "你若然還記得那諾言",
    "曾說今天我們流浪到海邊",
    "",
    "你照片留在一封情信裡面",
    "每一天仍是照舊看它一遍",
    "我仍然還記得那笑臉",
    "曾在當天我們流浪到夏天",
    "",
    "我都曾寫你在每天的日記",
    "但放棄你卻像再沒可能夢想",
    "像一個沉悶的獨唱",
    "",
    "你照片留在一封情信裡面",
    "每一天仍是照舊看它一遍",
    "兩年前遊過的動物園",
    "來到今天我們流浪到哪邊？",
    "",
    "我都曾寫你在每天的日記",
    "但放棄你卻像再沒可能夢想",
    "像一個沉悶的獨唱",
    "",
    "但放棄你卻像再沒可能夢想",
    "像失去甜蜜的合唱"
  ],
  [
    "明天就要飛走",
    "朋友都輕鬆奉上祝福　旅途愉快",
    "留低是個選擇",
    "離開亦未算放開一切壓抑",
    "如愛上一個人　一起上路",
    "才驟覺尚有一些心野",
    "行程未想好",
    "起點經已太遠",
    "",
    "人進進出出在我生命",
    "季節更改幻變天地　歲月流過",
    "遺憾若是放不下",
    "仍可學習去感激得到過的",
    "如放棄一個人　孤身上路",
    "才驟覺尚有一些心軟",
    "忙著讓傷口盡快風乾",
    "沿路風光沒心機細看",
    "明明是太軟弱",
    "偏裝作硬朗",
    "",
    "倘若結束為了重新開始",
    "乾掉了酒為了重新斟滿",
    "旅途落泊能讓記憶穩固",
    "偶然下雨懷念陽光可愛",
    "歡樂太短為了回憶千次",
    "失掉瑰寶為了尋找安慰",
    "我離別你全為太專心愛",
    "怕忘掉了世間的色彩",
    "離開　是為了回來",
    "",
    "從機艙機望向跑道",
    "迅速身處萬尺高空　我留下你",
    "城市跟天空大概一樣",
    "容納著敵友從不知道偏心",
    "曾遇上幾個人　一心靠近",
    "才驟覺尚有一些心癮",
    "離別再上路",
    "不管遠或近",
    "",
    "倘若結束為了重新開始",
    "乾掉了酒為了重新斟滿",
    "旅途落泊能讓記憶穩固",
    "偶然下雨懷念陽光可愛",
    "歡樂太短為了回憶千次",
    "失掉瑰寶為了尋找安慰",
    "我離別你全為太專心愛",
    "怕忘掉了世間的色彩",
    "離開　是為了回來",
  ],
  [
    "訂了一季　又到下季",
    "為何人大了就要成為工作的奴隸",
    "最愛作的不可發揮",
    "我的感覺　逐漸流逝",
    "從前曾話過要如何欣賞世界的美麗",
    "現在只懂得放假去消費",
    "",
    "如果繼續這樣",
    "我還是我嗎？",
    "我還喜歡我嗎？",
    "或者我應該死去吧",
    "總好過變得更差",
    "如果繼續這樣",
    "你還願意嗎？",
    "和我奏著結他？",
    "日子不應該這樣吧",
    "而現實就是這樣可怕",
    "",
    "如果繼續這樣",
    "我還是我嗎？",
    "我還喜歡我嗎？",
    "或者我應該死去吧",
    "總好過變得更差",
    "如果繼續這樣",
    "你還願意嗎？",
    "和我奏著結他？",
    "日子不應該這樣吧",
    "而現實就是這樣可怕",
    "",
    "日子不應該這樣吧",
    "而現實就是這樣　可怕",
  ],
  [
    "When the party is over, I miss my dear porn star",
    "When the party is over, I miss my dear porn star",
    "Do you have any boyfriend in Beijing?",
    "Do you have any boyfriend in Beijing?",
    "I don't think you have boyfriend in Beijing",
    "And I am the same",
    "",
    "I can only sing \"Sha la la\"",
    "Because I can't sing Putonghua",
    "I can only sing \"Sha La La\"",
    "Because I don't know Putonghua",
    "You will be queen of indie in Beijing",
    "I will be Hong Kong indie king",
    "Oh what am I saying?",
    "I'm just kidding",
    "",
    "Sha la la la la la la la la la la la",
    "Sha la la la la la la la la la la la",
    "I know tomorrow you will be leaving",
    "And your \"mushroom head\" is going",
    "And your post punk music is going",
    "Back to Beijing"
  ],
  [
    "la-la-la la-la-la la-la-la la-la-la-la",
    "la-la-la la-la-la la-la-la la-la-la-la",
    "la-la-la la-la-la la-la-la la-di-la-la",
    "la-la-la-la-la",
    "la-la-la",
    "",
    "la la-la-la la-la-la la-la-la la-la-la-la",
    "la-la la-la-la la-la-la la-la-la la-la-la-la",
    "la-la-la la-la-la la-la-la la-la-la-la",
    "la-la-la-la-la",
    "See you there",
    "",
    "la-la la la-la",
    "la-la-la-la la-la-la-la",
    "la-la-la-la la-la-la-la-la-lah-ah",
    "",
    "(la-la-la)",
    "(la-la-la)",
    "(la-la-la)",
    "(la la la la)",
    "",
    "la la-la-la la-la-la la-la-la la-la-la-la",
    "(la-la-la la-la-la la-la-la la-la-la)",
    "la-la la-la-la la-la-la la-la-la la-la-la-la",
    "(la-la-la-la)",
    "la-la la-la-la la-la-la la-la-la la-la-la-la",
    "(ah-ha ah-ha-ah)",
    "la-la-la-la-la",
    "See you there",
    "",
    "la-la-la la-la-la la-la-la la-la-la-la",
    "la-la-la-la-la",
    "See you there",
  ]
]

//
//  Singles
//

// zh, en?, yr, meta?, lyrics?

const singles = [
  [
    "你把我的",
    null,
    2022,
    [
      2, 25,
      "P", "P", "P,阿賢,阿科", "N", null
    ],
    [
      "你把　我的　各種　訊息",
      "都也　給予　別個　看的",
      "覺得　你跟　各位　更親",
      "跟我　一向　不算　很近",
      "",
      "我也只想回到我的山洞裡",
      "再也不想隨便相信人類",
      "我也只好放你進思念收集區",
      "這樣不會傷害誰",
      "",
      "最初　沒想　什麼　目的",
      "結果　也都　逐步　沉溺",
      "若果　我不　就此　絕跡",
      "最終　淪為　更糟　角色",
      "",
      "我也只想回到我的山洞裡",
      "再也不想隨便相信人類",
      "我也只好放你進思念收集區",
      "這樣不會傷害誰",
    ],
    "published"
  ],
  [
    "嘔吐",
    null,
    2022,
    [
      3, 20,
      "P", "P", "P,阿賢,阿科,夏日浪漫,eloobe", "N", null
    ],
    [
      "你說最近看沙特的《嘔吐》",
      "但那主旨卻掌握不到",
      "「夜蒙住了燈光」「撲克牌亂糟」",
      "",
      "我也看這書每當苦惱",
      "我也不懂得它的全部",
      "每次看卻會對他除帽",
      "",
      "這種感覺不去表達也好",
      "想表達也找不到辭措",
      "",
      "每次派對我都會很早到",
      "預備了時間給盪失路",
      "遲到不應該感到自豪",
      "",
      "某次看到不好看的 show",
      "想起那時光跟你虛度",
      "聖誕睡著看的芭蕾舞",
      "",
      "彷彿新世紀正要來到",
      "彷彿因此使你要嘔吐",
      "",
      "看見退休的歌手逛港島",
      "他眷戀的風格已變味道",
      "下午是漫無目的潛逃",
      "",
      "給我一個肉身可擁抱",
      "給我一句詩走這段路",
      "「醉鄉路穩宜頻到」",
      "",
      "這種感覺不去表達也好",
      "想表達也找不到辭措",
      "彷彿新世紀正要來到",
      "彷彿因此使你要嘔吐"
    ],
    "published"
  ],
  [
    "LUNCH",
    null,
    2022,
    [
      2, 55,
      "P", "P", "P,阿賢,阿科,eloobe", "P,N", {
        backup_vocal: "Karena"
      }
    ],
    [
      "太多廢話亂我心神",
      "寧願這裡繼續有病菌",
      "不想見人",
      "但你新工作在我家附近",
      "為了和你食 lunch 才打一針",
      "講笑（也有一分真）",
      "",
      "我被困在某種雲層（某種雲層）",
      "四周也是霧",
      "看不到路中心",
      "算是人類共同命運",
      "",
      "四月份是最殘忍",
      "你怎不知道我點諗",
      "當然這是不可能",
      "不要望我雙眼　會暈",
      "",
      "I only look at people when I'm drunk",
      "",
      "通山都是路人",
      "先知道是禮拜",
      "這座山已淪陷",
      "可否叫蛋糕甜品",
      "我想要食氣氛",
      "",
      "啦啦啦啦",
      "啦啦啦",
      "啦啦啦啦",
      "",
      "文藝是一種毒品",
      "幸福是絕對不幸",
      "我只不過想跟你呻",
      "不要嘗試解答我疑問",
      "",
      "我會對你好到不能",
      "方便日後沒有遺憾",
      "昨夜睡得很深層",
      "錯過了深夜安全的地震",
      "",
      "I only look at people when I'm drunk",
    ],
    "published"
  ], [
    "那陣時不知道",
    null,
    2021,
    null, null,
    "published"
  ], [
    "因講了出來",
    null,
    2021,
    null, null, 
    "published"
  ], [
    "因講了出來 (OTAKU MOBB Remix)",
    "When I Let It Out (OTAKU MOBB Remix)",
    2021,
    [ 
      2, 42,
      'P,OTAKU MOBB', 'P,OTAKU MOBB', 'P,蕭暢悅', 'N,OTAKU MOBB', {
        backup_vocal: 'Karena'
      }
    ],
    [
      "因講了出來",
      "便會失去吸引力",
      "失去機會被愛",
      "因講了出來",
      "便會失去所有",
      "想像中的可愛",
      "",
      "寧願永遠",
      "永遠停留在",
      "沒太多往來",
      "不在時間上慷慨",
      "寧願永遠",
      "永遠停留在",
      "想像之內",
      "免去細節的傷害",
      "",
      "I got six xannies 喺我枱面",
      "Pop it, yet I can't sleep",
      "Bought myself some Yat Pit, Margielas",
      "But I still feel lonely",
      "I'm lonely",
      "Girl 你只係個概念　係個畫面",
      "未曾見過面 but ask your friends they all heard about me",
      "O-T-K I'm reppin'",
      "Your DMs, I'm waitin'",
      "八個鐘　一個星期",
      "限時動態　\"Yo where's it at?\"",
      "\"Which 餐廳？\"　\"Nice movie!\"",
      "Boy he wack, he issa goofy",
      "But all good, he can have you",
      "我閉上眼　和你過生活",
      "This is my life, yeh, this is my song",
      "即使結局一樣　一樣 sorrow",
      "給我幾分鐘擁抱你和疼痛",
      "我醒來　你死去　我不能說出來 I'm waiting tomorrow",
      "I'm waiting tomorrow",
      "OTAKU MOBBIN'",
      "lil airport",
      "和你去發展愛",
      "Tonight I need no 羽田愛",
      "no no no no no",
      "",
      "（就這樣　在午夜　在馬路）",
      "（在網上　在購物　在腦內　就這樣愛）",
      "（寧願永遠）",
      "（永遠停留在）",
      "（就這樣　在被內　在散步）",
      "（在午睡　在吃飯　在發夢　就這樣愛）",
      "（沒太多往來）",
      "（不被對方掩蓋）",
    ],
    "published"
  ], [
    "散步之年",
    null,
    2021,
    null, null,
    "published"
  ], [
    "每次你走的時分",
    null,
    2021,
    null, null,
    "published"
  ], [
    "今夜雪糕",
    "Ice Cream Tonight",
    2019,
    [ 
      2, 27,
      'P', 'P', 'P', 'N', {
        backup_vocal: 'Karena'
      }
    ],
    [
      "何も言うことはない",
      "Non ho niente da dire",
      "Ich habe nichts zu sagen",
      "No tengo nada que decir",
      "Je n'ai rien à dire",
      "",
      "有時很懊惱",
      "有時也感到驕傲",
      "有時不後悔曾這樣表達憤怒",
      "",
      "今夜我可以吃更多雪糕",
      "想像明天中午會被捕",
      "",
      "還有些心願未做",
      "還未去埃及希臘秘魯",
      "還未跟你曾相處得更好",
      "",
      "揭開手套用胎記相認嘅朋友",
      "逾越嘅喜悅",
      "地鐵售票機上面嘅一堆散銀",
      "推完鐵馬流嘅汗",
      "最後一班列車嘅車門",
      "",
      "今夜我可以走八千里路",
      "想像明天會特赦全部",
      "今夜我可以",
      "想像明天中午"
    ],
    "public"
  ], [
    "吳小姐",
    "Miss Ng",
    2019,
    [ 
      3, 5,
      "P", "P", "P", "N", null
    ],
    [
      "吳小姐畢業之後",
      "做咗銀行十幾年",
      "最近佢單身",
      "罷工嗰日自己一個去彌敦道",
      "黃昏嘅時候",
      "成條馬路都係黑衫嘅年輕人行嚟行去",
      "尖沙咀同其他區已經放咗催淚彈",
      "佢一路行",
      "一路睇吓 live",
      "聽吓條街啲人講點部署",
      "",
      "無幾耐天開始黑",
      "行到豉油街",
      "經過兩間日本嘢",
      "佢都好想入去食住涼冷氣",
      "不過咁好似幫襯咗無罷市嘅食店",
      "最後佢猶豫咗一陣",
      "都係喺小販檔買咗兩個桔返示威區",
      "坐喺十字街口嘅馬路邊",
      "一路食一路有啲涼風",
      "",
      "就在她坐下的那個瞬間",
      "她竟有前所未有的嘆",
      "",
      "食食吓有人問佢要唔要水",
      "有人問佢去唔去深水埗警署聲援",
      "有個女仔指住太子方向",
      "話一直行就會到",
      "吳小姐走咗幾步覺得太遠",
      "都係搭地鐵",
      "",
      "去到現場",
      "有人叫佢唔好行咁前",
      "然後 佢第一次",
      "親眼見到催淚彈",
      "佢跟住大家",
      "向後褪　開遮　踎低　行返前",
      "來來回回",
      "幾粒催淚彈之後",
      "佢有種從未試過嘅感覺",
      "",
      "時間突然好慢",
      "周圍好靜",
      "佢覺得呢個瞬間",
      "最前線嘅人係自己",
      "後面用士巴拿拆緊鐵欄嘅人又係自己",
      "旁邊鬧緊街坊唔好影相嘅人又係自己",
      "遠處坐緊監嘅人又係自己",
      "",
      "無幾耐大家話轉場去另一個警署",
      "吳小姐坐喺紅綠燈下面食咗支煙",
      "",
      "就在她坐下的那個瞬間",
      "她竟有說不出的感嘆",
      "",
      "最後防暴衝出嚟",
      "吳小姐狂跑嘅時候",
      "竟然跑得快過晒所有嘅人",
      "佢決定　以後都出嚟",
    ],
    "public"
  ], [
    "宅女，上街吧",
    "Geeky Girls, Hit the Streets",
    2010,
    [
      2, 15,
      "P", "P", "P", "P", null,
    ],
    [
      "你錯過了反高鐵",
      "你錯過了保衛天星碼頭",
      "你錯過了包圍禮賓府",
      "你錯過了反世貿",
      "",
      "宅女　上街吧",
      "這個政府靠得過嗎？",
      "宅女　上街吧",
      "你要未來繼續任由人魚肉嗎？",
      "",
      "不要再說你沒有仔吼",
      "你對你的生活都沒有要求",
      "你只關心工作已經不夠",
      "你本來可以結識一個俊俏的八十後",
      "趁你現在還可以補救",
      "不要再信媒體中的主流",
      "佢哋只會著眼肢體的爭鬥",
      "不會理會暴力的政策才是因由",
      "",
      "宅女　上街吧",
      "這個政府靠得過嗎？",
      "宅女　上街吧",
      "功能組別還不夠醜惡嗎？",
      "",
      "我們已錯過了美好的六十年代",
      "錯過了胡士托",
      "反戰學運的精彩",
      "現在理想主義已逐漸歸來",
      "但你為何還不變改？",
      "",
      "你錯過了反高鐵",
      "其實我都錯過了保衛天星碼頭",
      "有時我也會很悔疚",
      "但願你的醒覺",
      "不會是在多年以後",
    ],
    "public"
  ], [
    "加多利大廈滲水事件",
    "Water Seepage Incident at Kadoorie Mansion",
    2010,
    [
      1, 50,
      "P", "P", "P", "P", null,
    ],
    [
      "我要告訴你知",
      "事關這新聞未算太多人知",
      "這是有關天花板滲水的事",
      "地點是發生在太子",
      "",
      "住喺加多利大廈的鄔女士",
      "零六年開始過滲水的日子",
      "零七年經過地台蓄水的測試",
      "確實滲水由上層單位所致",
      "",
      "樓上原來係屋宇署前署長蔡宇略嘅屋企地址",
      "佢一直話滲水唔關佢事",
      "職員上嚟幾次都唔俾人入屋做測試",
      "四年浸水　就係咁俾蔡宇略拖遲",
      "鄔女士慨嘆讀咁多書　點解會讀到失去良知",
      "",
      "六零年加入政府獲資助讀到博士",
      "D5 人工月入十五萬唔止",
      "退休前又曾被投訴濫權假公濟私",
      "用公帑評估清拆僭建物　但嗰個係自己嘅地址",
      "",
      "滲水可以滲足四年　係你本事",
      "政府淨係識依足指示辦事",
      "但依足指示辦四年都沒有成事",
      "這個社會就是要逼人起義",
      "",
      "這是滲水帶來的啟示",
      "再等落去嗰個先至係白痴",
      "請和你的朋友分享這件事",
      "不要介意旋律同其他歌似",
    ],
    "public"
  ], [
    "關於前度和男孩食飯的事",
    "About My Ex Dating A Youngster",
    2010,
    [
      2, 33,
      "P", "P", "P", "P", null
    ],
    [
      "我聽講我的一個前度女朋友約會",
      "與一個比佢年幼的男孩",
      "而那餐晚飯裡",
      "男孩講了許多許多往事",
      "但大多數只關於考試和放榜的事",
      "",
      "Oh no 我的前度",
      "佢聽到一半　心裡已不滿　覺得悶",
      "過了一小時　像過了一天",
      "晚飯後二人從此不再見",
      "",
      "男孩　世界發生了這麼多事",
      "你為何只講考試",
      "你可以講沙特和波娃的愛情故事",
      "又或者講李秋霞的咸濕歌詞",
      "又或者講 The Smiths 與 Morrissey",
      "文藝少女一般都會鍾意",
      "而遇上這種素食的文藝女士",
      "食飯時　你大可以",
      "慨嘆既然人類知道食魚翅是殘忍時",
      "為何食豬牛羊又可以？",
      "提出類似的質疑",
      "佢哋會覺得你有意思",
      "男孩　你下次可以試",
      "",
      "男孩　其實我剛才只是講笑",
      "當一個中女約會你時",
      "已經是退而求其次",
      "你講什麼佢都不會介意",
      "最後覺得悶 純粹係你不夠靚仔所致",
      "世界就是這樣諷刺",
      "其實所謂的文藝女士",
      "我見過　最終都一樣是",
      "想找個靚仔而已",
    ],
    "unlisted"
  ], [
    "告別吧，香港",
    "Farewell, Hong Kong",
    2010,
    [
      2, 11,
      "P", "P", "P", "P", null,
    ],
    [
      "當我走上街　條街很靜",
      "我看見人們微笑　但沒笑聲",
      "我感覺到太陽熾熱　但沒光明",
      "嗰日係五月十六號",
      "黑色星期天　但天晴",
      "",
      "當我走入車廂　沒有人覺得悲傷",
      "晚上十時半廣播後　我開始幻想",
      "我看得見草原　我看得見花",
      "我感覺到軍隊　我感覺到人們熟睡去",
      "",
      "告別吧　香港",
      "告別吧　再見",
      "再會了　係噉",
      "再會吧　再約",
      "拜拜　拜拜",
      "再會　拜拜",
    ],
    "unlisted"
  ], [
    "我愛郊野，但不愛派對",
    "I Love the Country But Not the Party",
    2009,
    [
      1, 29,
      "P,Vicky Leong", "P", null, "P", null
    ],
    [
      "I love the country",
      "But not the party",
      "If you wanna celebrate",
      "Don't even think of calling me",
      "You know it's not easy",
      "Pretending to be happy",
      "There're still many good guys doing time",
      "Can't you see?",
      "",
      "Rich guys get richer",
      "Poor guys get poorer",
      "In a party, there are many wankers and losers",
      "I'd celebrate Christmas Day",
      "I'd celebrate Easter Sunday",
      "But not this party of your sixtieth birthday",
      "No, not this party of your sixtieth birthday",
    ],
    "public"
  ], [
    "前度，去你的",
    "Ex, To Hell With You",
    2009,
    [
      1, 39,
      null, "P", null, null, {
        poem: "阿雪",
        recite: "阿雪",
      }
    ],
    [
      "前度　去你的",
      "",
      "在分開的第212天",
      "我聽到一些關於我們的傳言",
      "我沒有故作大方",
      "隨即將你從 facebook 及 msn 中刪除掉",
      "以為你不會再在我眼前出現",
      "但你那些礙眼的遺物就在我的跟前",
      "心裡不禁低聲罵了一句　去你的",
      "沒錯　去你的",
      "去你的壞品味",
      "去你的飄忽不定",
      "去你的表裡不一",
      "去你的不三不四九唔搭八",
      "去你的虛假的甜言蜜語",
      "去你的嘔心的英雄主義",
      "去你的從不欣賞我為你寫的一字一句",
      "去你的居然背著我偷偷愛上別人",
      "去你的浪費了我兩年光陰",
      "去你的",
      "前度　去你的",
    ],
    "public"
  ], 
]

// zh, en, publisher, 
// yr, [trackNo]
// tracks, [disc]
const featureAlbumList = [
  [
    "LMFXXYEARFEST",
    "LMFXXYEARFEST",
    "Cactus Creation", "Cactus Creation",
    2019,
    [7], null,
    "hk"
  ], [
    "達明一代",
    "Tat Ming Pair Tribute Album",
    "環球唱片", "Universal Music",
    2017,
    [8], [1],
    "hk"
  ], [
    "everyone remembers the orange",
    "everyone remembers the orange",
    "維港唱片", "Habour Records",
    2014,
    [2, 7, 10, 14, 16, 19], null,
    "hk"
  ], [
    "Space Escapade (Aventura Espacial) Unit 1 - Destination: Pluto Sector 68",
    "Space Escapade (Aventura Espacial) Unit 1 - Destination: Pluto Sector 68",
    "Elefant Records", "Elefant Records",
    2010,
    [2], [2],
    "international",
  ], [
    "Happy New Year 2008",
    "Happy New Year 2008",
    "Elefant Records", "Elefant Records",
    2008,
    [4], null,
    "international",
  ], [
    "Poptastic Conversation China - 奇妙對話中國",
    "Poptastic Conversation China",
    "Fly Fast Records", "Fly Fast Records", 
    2008,
    [8], [1],
    "international",
  ], [
    "花吃了那女孩電影原聲帶",
    "Candy Rain OST",
    "亞神音樂娛樂", "Asia Muse Entertainment",
    2008,
    [1, 8], [2, 2],
    "tw"
  ], [
    "Little Darla Has a Treat for You, Volume 25: Endless Summer 2007-08",
    "Little Darla Has a Treat for You, Volume 25: Endless Summer 2007-08",
    "Darla Records", "Darla Records",
    2007,
    [8], [1],
    "international",
  ], [
    "We Wish You An Indie X'mas",
    "We Wish You An Indie X'mas",
    "維港唱片", "Habour Records",
    2007,
    [6], null,
    "hk",
  ], [
    "2007 小草地２年級同學會",
    "2007 Grassland Second Grade Reunion",
    "默契音樂", "Silent Agreement",
    2007,
    [5], null,
    "tw",
  ], [
    "草地音樂合輯",
    "Grass Fest Music Compilation",
    "默契音樂", "Silent Agreement",
    2005,
    [2, 8], null,
    "tw",
  ], [
    // http://sandeechan.com/HomePage/music/is_Cht.php
    "IS #3",
    "IS #3",
    "荒島網路科技", "Isle Networks",
    2005,
    [4], null,
    "tw",
  ], [
    "FC5-118 Hitoradio.com 開站紀念盤",
    "FC5-118 Hitoradio.com Launch Commemorative Disc",
    "台北之音", "Hitoradio",
    2005,
    [4], [1],
    "tw",
  ], [
    // https://room40.bandcamp.com/album/add-to-friends
    "Add To Friends",
    "Add To Friends",
    "Someone Good", "Someone Good",
    2006,
    [17], null,
    "international",
  ], [
    "Come Out and Play", 
    "Come Out and Play",
    "89628 / the PANIC", "89628 / the PANIC",
    2004,
    [12], [2],
    "hk"
  ], [
    "In the Name of AMK",
    "In the Name of AMK",
    "維港唱片", "Habour Records",
    2004,
    [6], null,
    "hk"
  ]
]

const feature = [
  [
    "今宵多珍重",
    "Farewell",
    [
      2, 40,
      "LMF", "LMF", "P", "N", null
    ],
    [
      "今宵多珍重　聽日又未必會係噉",
      "今宵多珍重　聽日又另外一個樣",
      "今宵多珍重　聽日又未必會係噉",
      "今宵多珍重　聽日又另外一個樣",
      "",
      "你有冇埋怨自己生出嚟嗰陣時",
      "唔係含住條金鎖匙　係一條生銹鎖匙",
      "你老豆冇本事　冇錢養妻活兒",
      "你呀嫲就啋　你個衰仔",
      "生你出嚟都係多餘　唔該",
      "你要記住人生嘅大門係要靠你雙手去打開",
      "掂嗰陣時就唔駛見風駛盡𢃇",
      "耷嗰陣時唔好怨天怨地",
      "",
      "Unprecedented though this moment in history may be",
      "We have the utmost confidence in the abilities",
      "And resilience of the Hong Kong people",
      "Britain learnt long ago",
      "That Hong Kong people know best",
      "What is good for Hong Kong",
      "",
      "可以試下去諗條路唔係一定要噉走",
      "要壞嘅話唔一定要食煙飲酒",
      "唔係一定要耷低頭做人隻狗",
      "可以抬起頭自己行每一步路",
      "唔駛畀人睇死你係一個冇用嘅細路",
      "自己去做做到自己就當然最好",
      "做唔到都千祈唔好放棄自己",
      "鎅手跳樓飲滴露蕩失路",
      "",
      "It has been the greatest honour and privilege of my life",
      "To share your home for five years",
      "And to have some responsibility for your future",
      "Now Hong Kong people are to run Hong Kong",
      "That is the promise and that is the unshakable destiny",
      "",
      "今宵多珍重　聽日又未必會係噉",
      "今宵多珍重　聽日又另外一個樣",
      "今宵多珍重　聽日又未必會係噉",
      "今宵多珍重　聽日又另外一個樣",
      "今宵多珍重"
    ],
    "published"
  ], [
    "你望我望",
    "You Look I Look",
    [
      3, 6,
      "劉以達", "陳少琪", "P", "N", null,
    ],
    [
      "寂寞在舞中消逝　漫漫樂韻交替",
      "人們仍為舞會著迷　琴弦嘹亮徹底",
      "站在伴侶的背后　靜靜疊上雙手",
      "無從明白你的念頭　為何凝視我喝酒",
      "我望你望我面　只隔一線",
      "你望我望你面　心里束縛再加添",
      "",
      "站在待應的背后　默默換了杯酒",
      "情人仍在你心逗留　為何仍愿意去偷",
      "我望你望我面　只隔一線",
      "你望我望你面　心里束縛再加添",
      "",
      "別賣弄你的漂亮　別望著我欣賞",
      "為何呆著似一道墻　為何情欲正擴張",
      "我望某夜某地　可碰到你",
      "放任某夜某地　一切束縛也不理"
    ],
    "published"
  ], [
    "my little banana",
    null,
    null, null,
    "published"
  ], [
    "春天在車廂裡",
    null,
    null, null,
    "published"
  ], [
    "debbie, don't send this song to others",
    null,
    null, null,
    "published"
  ], [
    "藍白紅風格練習",
    null,
    null, null,
    "published"
  ], [
    "milan",
    null,
    null, null,
    "published"
  ], [
    "印度",
    null,
    null, null,
    "published"
  ], [
    // Space Escapade
    "Edward, Had You Ever Thought That the End of the World Would Come on 20.9.01?",
    null,
    null, null,
    "published"
  ], [
    // Happy New Year 2008
    "阿波馬草結婚了",
    null,
    null, null,
    "published"
  ], [
    // Poptastic Conversation China
    "Ｄ釘硬Ｄ'",
    "D Deng Ang D",
    [
      1, 31,
      null, null, null, null, null,
    ],
    null,
    "published"
  ], [
    // 花吃了那女孩
    "讓我搭上一班會爆炸的飛機",
    "Let Me Get On an Exploding Plane",
    [
      2,21,
      'P', 'P', 'P,N,阿賢,阿科', 'N', null
    ],
    [
      "讓我搭一班會爆炸的飛機",
      "去到台灣之前被炸死",
      "讓我喝一杯會吐血的香檳",
      "在喝醉之前可斷氣",
      "",
      "我以後也不會再打電話給你",
      "再沒法對電話唱Ｋ",
      "你要是孤單便去找好友睇戲",
      "他們一定會氹返你",
      "",
      "讓我搭一班會爆炸的飛機",
      "回到香港之前被炸死",
      "讓我喝一杯會吐血的香檳",
      "在喝醉之前擁抱你",
      "",
      "我以後也不會再打電話給你",
      "再沒法對電話唱Ｋ",
      "你要是孤單便去找好友睇戲",
      "他們一定會氹返你",
      "",
      "讓我搭一班會爆炸的飛機",
      "回到香港之前被炸死",
      "讓我喝一杯會吐血的香檳",
      "在喝醉之前可斷氣",
    ],
    "published"
  ], [
    // 花吃了那女孩
    "和陳五 msn",
    null,
    null, null,
    "published"
  ], [
    // Little Darla
    "Coka, I'm Fine",
    null,
    null, null,
    "published"
  ], [
    // We Wish You An Inde X'mas
    "悲傷的採購 (Digilick Remix)",
    "Sad Purchase (Digilick Remix)",
    [
      4, 32,
      'P', 'P', 'P,Digilick', 'N', null,
    ],
    [
      "訂了一季　又到下季",
      "為何人大了就要成為工作的奴隸",
      "最愛作的不可發揮",
      "我的感覺　逐漸流逝",
      "從前曾話過要如何欣賞世界的美麗",
      "現在只懂得放假去消費",
      "",
      "如果繼續這樣",
      "我還是我嗎？",
      "我還喜歡我嗎？",
      "或者我應該死去吧",
      "總好過變得更差",
      "如果繼續這樣",
      "你還願意嗎？",
      "和我奏著結他？",
      "日子不應該這樣吧",
      "而現實就是這樣可怕",
      "",
      "如果繼續這樣",
      "我還是我嗎？",
      "我還喜歡我嗎？",
      "或者我應該死去吧",
      "總好過變得更差",
      "如果繼續這樣",
      "你還願意嗎？",
      "和我奏著結他？",
      "日子不應該這樣吧",
      "而現實就是這樣",
      "",
      "可怕"
    ],
    "published"
  ], [
    // 2007 小草地２年級同學會
    "我愛官恩娜，都不及愛你的哨牙",
    null, 
    null, null,
    "published"
  ], [
    // 草地音樂合輯
    "在動物園散步才是正經事",
    null,
    null, null,
    "published"
  ], [
    // 草地音樂合輯
    "victor, fly me to stafford",
    null,
    null, null,
    "published"
  ], [
    // IS #3
    "victor, fly me to stafford (remix)",
    "victor, fly me to stafford (remix)",
    [
      4, 25,
      "P", "P", "Mike McGuire", "N", null
    ],
    [
      "This is the last song I write for you",
      "This is the last night I think of you",
      "Your name is Victor Ching, how do you do?",
      "",
      "I phoned you this morning you couldn't hear me",
      "I sent you letter you couldn't receive",
      "It's my favorite game, but I have changed",
      "",
      "It's too late to say I miss you Victor Ching",
      "And you are on your way to Stafford in UK",
      "I know it's too late to say I'm fond of you",
      "You've got a girl from Singapore who you didn't like before",
      "",
      "I love you, I need you",
      "I love you, I need you",
      "I love you, I need you",
      "I love you, I need you",
      "",
      "I love you when you no longer love me",
      "I need you when you no longer need me",
      "You wanted me, but you have changed",
      "",
      "It's too late to say I miss you Victor Ching",
      "And you are on your way to Stafford in UK",
      "I know it's too late to say I'm fond of you",
      "You've got a girl from Singapore who you didn't like before",
      "",
      "It's too late to say I miss you Victor Ching",
      "And you are on your way to Stafford in UK",
      "I know it's too late to say I'm fond of you",
      "You've got a girl from Singapore who you didn't like before",
      "",
      "I need you"
    ],
  ], [
    // FC5-118 Hitoradio.com 開站紀念盤
    "I Don't Know How To Download Good AV Like Iris's",
    "I Don't Know How To Download Good AV Like Iris's",
    [
      2,26,
      "P", "P", null, "N", null
    ],
    [
      "I wanna watch some AV and you got plenty",
      "I'm so dry, I'm so dry, I'm so dry",
      "I enjoy watching girls and boys in uniforms",
      "Send them to me, oh Iris, send them to me",
      "",
      "I don't know any others who has plenty of AV",
      "Please send those good AV to me",
      "",
      "I don't know how to download good AV like yours",
      "I don't know, I don't know, I don't know",
      "You know I like those AV telling no story",
      "Only you, only you, you know me",
      "",
      "I don't know any others who has plenty of AV",
      "please send those good AV to me",
      "",
      "(Send me good AV)",
      "(Please send me good AV)",
      "(Saw some AV)",
      "(AV, got plenty)",
      "(I want some good AV)",
      "(AV telling no story)",
      "(Send me, send me)",
      "(I wanna watch some AV)",
      "(Yeah I watch AV)",
      "(I know you got plenty)",
      "(AV)",
      "(AV)"
    ],
    "published"
  ], [
    // Add To Friends
    "Leo, Are You Still Jumping Out Of The Windows In Expensive Clothes?",
    "Leo, Are You Still Jumping Out Of The Windows In Expensive Clothes?",
    [
      2,9,
      "P", "P", null, "N", null
    ],
    [
      "I hope you can see what I can see",
      "I hope you can hear what I can hear",
      "Because it's not easy",
      "Because it's not easy",
      "",
      "I hope you can feel what I can feel",
      "I hope you believe what I believe",
      "Because I feel lonely",
      "Because I'm so lonely",
      "",
      "I don't wanna go home",
      "I still can recall",
      "All the days in F.4 we played in our school",
      "Now I stay in Mei Foo",
      "But where are you?",
      "Leo, where are you?",
      "",
      "I hope you can taste what I can taste",
      "I hope you can think what I can think",
      "Because I'm so crazy",
      "Because I'm so crazy",
      "",
      "'Cause I know you know what I don't know",
      "I know you can think what I can't think",
      "Because you are special",
      "Because you're so special",
      "",
      "How I wish you were here",
      "So I called your home",
      "But your mom said that you've gone somewhere",
      "I have never heard before",
      "Why didn't you call me",
      "When you leave here?",
      "",
      "And all the things changed",
      "After we repeated a year in school"
    ],
    "published"
  ], [
    // Come Out and Play
    "Tim, Do You Really Want to Make A Film?",
    "Tim, Do You Really Want to Make A Film?",
    [ 
      3, 9,
      "P", "P", "P", "P,N", null,
    ],
    [
      "係呢個太陽充沛嘅早上",
      "我哋有位叫阿Ｐ的聽眾",
      "點咗首歌畀阿 Tim 話：",
      "",
      "Tim, if you do wanna make a film",
      "Why don't you start to write a script?",
      "Do you just wanna tell everybody",
      "That you plan to make a film?",
      "'Cause you want to show your existence",
      "Let the world knows who's Tim",
      "You even don't try to pick up a pen",
      "To write anything",
      "",
      "But I do wanna see you",
      "Can make a film that belongs to you",
      "And I know you are more than this",
      "It's time to start it and make a smile",
      "'Cause I do wanna see you",
      "Can make a film that belongs to you",
      "I've been waiting so long",
      "You have to start it within this summer",
      "",
      "Every day in this summer is like Sunday",
      "You catch up with your friends every night and day",
      "Plan to date Candy in Causeway Bay",
      "Though she doesn't really wanna see your face",
      "Go shopping from Monday to Friday",
      "And watching VCDs every Saturday",
      "Telling me your story will be okay",
      "But no action until the end of this holiday",
      "",
      "Tim, if I said I feel happy",
      "That sometimes you're worse than me",
      "Would you think that I have low self-esteem",
      "And even more disguise than you?",
      "Will you be very angry?",
      "Do you still wanna talk to me?",
      "Maybe you just think",
      "No one understand you in this world",
      "",
      "But I do wanna see you",
      "Can make a film that belongs to you",
      "And I know you are more than this",
      "It's time to start it and make a smile",
      "'Cause I do wanna see you",
      "Can make a film that belongs to you",
      "I've been waiting so long",
      "You have to start it within this summer",
      "",
      "La la la la",
      "La la la la",
      "La la la",
    ],
    "published"
  ], [
    // In the Name of AMK
    "山頂．公仔．波板糖",
    null,
    null, null,
    "published",
  ]
]

const albumSongs = [
  a1list, a1meta, a1lyrics,
  a2list, a2meta, a2lyrics,
  a3list, a3meta, a3lyrics,
  a4list, a4meta, a4lyrics,
  a5list, a5meta, a5lyrics,
  a6list, a6meta, a6lyrics,
  a7list, a7meta, a7lyrics,
  a8list, a8meta, a8lyrics,
  a9list, a9meta, a9lyrics,
  a10list, a10meta, a10lyrics,
  a11list, a11meta, a11lyrics,
  a12list, a12meta, a12lyrics,
  a13list, a13meta, a13lyrics
].concat([
  lka1list, lka1meta, lka1lyrics,
  lka2list, lka2meta, lka2lyrics,
  lka3list, lka3meta, lka3lyrics,
  lka4list, lka4meta, lka4lyrics,
  lka5list, lka5meta, lka5lyrics,
], [
  colablist, colabmeta, colablyrics
])

//
// Helper Func
//

const getNames = (str) => {
  if (!str) return null
  return str.split(',').map(name => {
      switch (name) {
        case 'P': return '林阿Ｐ'
        case 'N': return 'Nicole'
        default: return name
      }
    })
}

const getOthers = (obj) => {
  if (!obj) return {}
  return Object.fromEntries(
    Object.entries(obj).map(([key, val]) => {
      return [key, getNames(val)]
    })
  )
}

const slugify = (str) => {
  str = str.trim();
  str = str.toLowerCase();

  // remove accents, swap ñ for n, etc
  const from = "ｋｄ２åàáãäâèéëêìíïîòóöôùúüûñç·/_,:;";
  const to   = "kd2aaaaaaeeeeiiiioooouuuunc------";

  for (let i = 0; i < from.length; i++)
    str = str.replace(new RegExp(from.charAt(i), "g"), to.charAt(i));

  return str
    .replace(/[^a-z0-9_\u3400-\u9FBF\uFF0C -]/g, "") // remove invalid chars
    .replace(/\s+/g, "-") // collapse whitespace and replace by -
    .replace(/-+/g,  "-") // collapse dashes
    .replace(/^-+/,  "")  // trim - from start of text
    .replace(/-+$/,  "")  // trim - from end of text
    .replace(/\uFF0C/g, "-"); // replace ，
}

//
// Main
//

const fs = require('fs')

let tracks = [];
let albums = [];

for (let i = 0; i < albumList.length; i++) {
  const trackNames  = albumSongs[3*i]
  const trackMetas  = albumSongs[3*i + 1]
  const trackLyrics = albumSongs[3*i + 2]
  const [aName, aNameEN, aYear, aTrackNo, aType, aLabel, aLabelEN] = albumList[i]

  let trackId = 0;
  let currAlbum = {
    name: aName,
    slug: slugify(aName),
    date: aYear,
    type: 'album',
    edition: aType,
    track_no: aTrackNo,
    label: aLabel,
    tracks: [],
    translation: {
      en: {
        name: aNameEN,
        label: aLabelEN,
      }
    }
  }

  trackNames.og.forEach((name, idx) => {

    const _meta   = trackMetas[idx]
    const _lyrics = trackLyrics[idx]

    const _idx  = tracks.findIndex(s => s.slug === slugify(name))

    const trackInfo = {
      slug: slugify(aName),
      name: aName,
      disc: 1,
      track: trackId + 1
    }

    const albumTrackInfo = {
      disc: 1,
      track: trackId + 1
    }

    if (!trackNames.en[trackId]) {
      tracks[_idx].album.push(trackInfo)
      currAlbum.tracks.push({
        name: tracks[_idx].name,
        slug: tracks[_idx].slug,
        ...albumTrackInfo
      })
      ++trackId
      return
    }

    const slug = _idx !== -1 
      ? slugify(`${name} (${aYear})`) 
      : slugify(name)
    tracks.push({
      name,
      slug,
      album: [trackInfo],
      duration: _meta[0]*60 + _meta[1],
      credits: {
        composer:   getNames(_meta[2]),
        lyricist:   getNames(_meta[3]),
        arranger:   getNames(_meta[4]),
        lead_vocal: getNames(_meta[5]),
        ...(getOthers(_meta[6]) ?? {})
      },
      lyrics: _lyrics,
      translation: {
        en: {
          name: trackNames.en[trackId]
        }
      }
    })
    currAlbum.tracks.push({
      name, 
      slug, 
      ...albumTrackInfo
    })
    ++trackId
  })

  albums.push(currAlbum)
}

singles.forEach((song, idx) => {

  const [name, nameEN, year, meta, lyrics, edition] = song

  const _idx = tracks.findIndex(s => s.slug === slugify(name))

  const aName   = name
  const aNameEN = nameEN

  const currAlbum = {
    name: aName,
    slug: slugify(aName),
    date: year,
    type: 'single',
    edition,
    track_no: 1,
    label: 'my little airport',
  }

  const trackInfo = {
    slug: slugify(aName),
    name: aName,
    disc: 1,
    track: 1
  }

  const albumTrackInfo = {
    disc: 1,
    track: 1,
  }

  if (!nameEN) {
    // Push new album into track
    tracks[_idx].album.push(trackInfo)
    // Create new album
    albums.push({
      ...currAlbum,
      tracks: [{
        name: tracks[_idx].name,
        slug: tracks[_idx].slug,
        ...albumTrackInfo
      }],
      translation: {
        en: {
          name: tracks[_idx].translation.en.name
        }
      }
    })
    return
  }

  const slug = _idx !== -1 
    ? slugify(`${name} (${year})`) 
    : slugify(name)
  tracks.push({
    name,
    slug,
    album: [trackInfo],
    duration: meta[0]*60 + meta[1],
    credits: {
      composer:   getNames(meta[2]),
      lyricist:   getNames(meta[3]),
      arranger:   getNames(meta[4]),
      lead_vocal: getNames(meta[5]),
      ...(getOthers(meta[6]) ?? {})
    },
    lyrics: lyrics,
    translation: {
      en: {
        name: nameEN
      }
    }
  })
  albums.push({
    ...currAlbum,
    tracks: [{
      name: name,
      slug: slug,
      ...albumTrackInfo
    }],
    translation: {
      en: {
        name: aNameEN,
        label: 'my little airport'
      }
    }
  })
})

let currTrack = 0;
featureAlbumList.forEach((_album, idx) => {

  const [aName, aNameEN, aLabel, aLabelEN, aYear, aTracks, aDisc, aEdition] = _album

  const currAlbum = {
    name: aName,
    slug: slugify(aName),
    date: aYear,
    type: 'compilation',
    edition: aEdition,
    track_no: aTracks.length, // TODO
    tracks: [],
    label: aLabel
  }

  aTracks.forEach((_trackNo, tidx) => {
    const [name, nameEN, meta, lyrics, edition] = feature[currTrack]
    const _idx  = tracks.findIndex(s => s.slug === slugify(name))
    
    const trackInfo = {
      slug: slugify(aName),
      name: aName,
      disc: aDisc?.[tidx] ?? 1,
      track: _trackNo,
    }

    const albumTrackInfo = {
      disc: aDisc?.[tidx] ?? 1, 
      track: _trackNo,
    }

    if (!nameEN) {
      // Push new album into track
      tracks[_idx].album.push(trackInfo)
      // Push track into currAlbum
      currAlbum.tracks.push({
        name: tracks[_idx].name,
        slug: tracks[_idx].slug,
        ...albumTrackInfo
      })
    } else {
      // Create new track
      const slug = _idx !== -1 
        ? slugify(`${name} (${year})`) 
        : slugify(name)
      tracks.push({
        name,
        slug,
        album: [trackInfo],
        duration: meta[0]*60 + meta[1],
        credits: {
          composer:   getNames(meta[2]),
          lyricist:   getNames(meta[3]),
          arranger:   getNames(meta[4]),
          lead_vocal: getNames(meta[5]),
          ...(getOthers(meta[6]) ?? {})
        },
        lyrics: lyrics,
        translation: {
          en: {
            name: nameEN
          }
        }
      })
      currAlbum.tracks.push({
        name,
        slug,
        ...albumTrackInfo
      })
    }
    currTrack++;
  })

  // Create new album
  albums.push({
    ...currAlbum,
    translation: {
      en: {
        name: aNameEN,
        label: aLabelEN
      }
    }
  })

})

const tracksFile = fs.createWriteStream('tracks.json')
tracksFile.on('error', (err) => console.error(err))
tracksFile.write(JSON.stringify(tracks, undefined, 2))
tracksFile.close()

const albumsFile = fs.createWriteStream('albums.json')
albumsFile.on('error', (err) => console.error(err))
albumsFile.write(JSON.stringify(albums, undefined, 2))
albumsFile.close()